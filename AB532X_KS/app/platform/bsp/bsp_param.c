...
{
	uint8_t state_buf[12];
	
	if (reboot == param_exit_state_read()) {
		printf("flash is same, return!!!\n");
		return;
	}
	
	if (reboot) {
		memcpy(state_buf, "MAS+MCU+RBT&", 12);
		printf ("set extern mcu reboot;\n");
	}
	else {
		memset(state_buf, 0xff, 12);
		printf ("set extern mcu frist poweron;\n");
	}
    param_write((u8 *)state_buf, PARAM_EXIT_STATE, 12);
    param_sync();
}
...
bool param_exit_state_read(void)
{
    ...
    param_read((u8 *)&state_buf, PARAM_EXIT_STATE, 12);
    if(memcmp(state_buf, "MAS+MCU+RBT&", 12) == 0) {
    	ret = 1;
    	printf("extern mcu reboot!!!\n");
    }
    else {
    	ret = 0;
    	printf("extern mcu frist poweron!!!\n");
    }

...
{
    GPIOBDE |= BIT(4);  GPIOBDIR |= BIT(4); GPIOBFEN &= ~BIT(4);
    GPIOBPD |= BIT(4);
    sys_irq_init(IRQ_PORT_VECTOR,0, port_isr);
    PORTINTEN |= BIT(12);
    PORTINTEDG &= ~BIT(12);  //falling edge;
    WKUPEDG &= ~BIT(4);     //falling edge
    WKUPCON = BIT(4) | BIT(16);  //falling edge wake iput //wakeup int en

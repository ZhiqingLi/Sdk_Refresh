...
extern void SlaveBluetoothStateDet(void);
#endif
...
//	SysGotoPowerDown();
  ...
  SwUartTxInit(SWUART_GPIO_PORT_A, 25, 115200);
	EnableSwUartAsFuart(TRUE);
	APP_DBG("Debug uart is SWUART_GPIO_PORT_A25\n");
 #else
	GpioFuartRxIoConfig(DEBUG_RX_PORT);
	GpioFuartTxIoConfig(DEBUG_TX_PORT);
	OSDBGUartInit(115200, 8, 0, 1);
	APP_DBG("Debug uart is FUART_GPIO_PORT_C3,C4\n");
 #endif
  OsSetDebugFlag(1);
#else
	OsSetDebugFlag(0);
...
#endif
#ifdef FUNC_WIFI_EN
		   || (IsWiFiSoundRemindPlaying() || !gSys.MuteFlag)
#endif
#ifdef FUNC_KEY_BEEP_SOUND_EN
       || !IsBeepSoundEnd()
#endif

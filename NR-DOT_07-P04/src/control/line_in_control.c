	...
	if(gWiFi.MicMuteState || gWiFi.VisInfState)
	{
		...
		else*/
		{
			...
			if(gWiFi.VisInfState)
			{
				...
				LED_WHITE_MODE_OFF();
				TimeOutSet(&LineInLedTmr, 400);
				if(LedFlag && (LedCnt < 6 || !gWiFi.MicMuteState))
				{
					LED_GREEN_MODE_ON();
				}					
				if((gWiFi.MicMuteState) && (LedCnt >= 6))
				{
					TimeOutSet(&LineInLedTmr, 2500);
					LedCnt = 0;
					LED_RED_MODE_ON();
				}
			}

...
const uint16_t gDecVolArr[MAX_VOLUME + 1] =
{
	0,
	26,		31,		36,		42,		49,		58,		67,		78,
	91,		107,	125,	147,	173,	289,	350,	472,
	549,	701,	880,	1051,	1186,	1368,	1525,	1710,
	1820,	1925,	2050,	2170,	2320,	2460,	2650,	2850//最高音量约700mVrms
};
#endif
...
void CommonMsgProccess(uint16_t Msg)
{
	...
	if(gWiFi.InitState)
	{
		if((Msg == MSG_WIFI_MIC) && (gWiFi.MicMuteState))
		{
			Mcu_SendCmdToWiFi(MCU_TLK_ENA, NULL);
		}
		else if((Msg == MSG_WIFI_MIC) && (!gWiFi.MicMuteState))
		{

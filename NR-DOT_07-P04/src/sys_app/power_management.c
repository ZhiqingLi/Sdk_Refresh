...
//以下定义不同的电压检测事件的触发电压(单位mV)，用户根据自身系统电池的特点来配置
   ...
   #define LDOIN_VOLTAGE_FULL			1450
   #define LDOIN_VOLTAGE_HIGHT    1400
   #define LDOIN_VOLTAGE_HIGH			1300
   #define LDOIN_VOLTAGE_MID			1250
   #define LDOIN_VOLTAGE_LOW			1100
   #define LDOIN_VOLTAGE_OFF			1050	//注意：这里是ADC值，Vol=log/4095*3.41,单位V
#else

...
bool IsBurningSilentState = FALSE;		//Wifi静默升级状态
		...
		if(gSys.CurModuleID != MODULE_ID_WIFI)
		{
			...
			if(!State)
			{
				...
				switch(gSys.CurModuleID)
				{
					...
					case MODULE_ID_SLAVE_BLUETOOTH:
	#endif
...
void WiFiAppSetPlayMode(uint8_t Mode)
{
	...
	else if(Mode == WIFI_PLAY_MODE_MENU_PLAY_SD)
	{
		SetMcuPlayMode = MODULE_ID_PLAYER_WIFI_SD;	
		    ...
		    || (Mode == WIFI_PLAY_MODE_AIRPLAY) || (Mode == WIFI_PLAY_MODE_DLNA) || (Mode == WIFI_PLAY_MODE_SPOTIFY_PLAY))
	{
...
void WiFiLedCb(void* Param)
{
 static uint32_t  LedBlinkCnt;
	...
	LedBlinkCnt++;
	if((WiFiFirmwareUpgradeStateGet() && !IsBurningSilentState) || (WiFiFactoryStateGet() == TRUE))
	{
		LED_WHITE_MODE_OFF();

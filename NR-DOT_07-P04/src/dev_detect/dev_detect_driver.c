...
bool IsLineOutLink(void)
{
#define LINEOUT_JETTER_TIMES		10	//连接检测消抖时间：10次，100ms

	static uint8_t LineOutLinkState = 0;

	//设为输入，带上拉
	GpioClrRegBits(LINEOUT_DETECT_PORT_OE, LINEOUT_DETECT_BIT_MASK);
	GpioClrRegBits(LINEOUT_DETECT_PORT_PU, LINEOUT_DETECT_BIT_MASK);
	GpioClrRegBits(LINEOUT_DETECT_PORT_PD, LINEOUT_DETECT_BIT_MASK);

	GpioSetRegBits(LINEOUT_DETECT_PORT_IE, LINEOUT_DETECT_BIT_MASK);

	if(GpioGetReg(LINEOUT_DETECT_PORT_IN) & LINEOUT_DETECT_BIT_MASK)
	{
		LineOutLinkState = 0;						//断开状态不做消抖处理
	}
	else
	{
		if(LineOutLinkState < LINEOUT_JETTER_TIMES)	//连接状态做消抖处理
		{
			LineOutLinkState++;
		}
	}
	return (LineOutLinkState >= LINEOUT_JETTER_TIMES);
}
#endif
...
#else
#ifndef FUNC_WIFI_TALK_AND_AUDIO_EFFECT_EN
	gSys.MicEnable = TRUE;
#endif
	return TRUE;
#endif

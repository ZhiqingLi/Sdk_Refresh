				       ...
				       MGC_MsdCmdSet* pCmdSet)
{
    ...
    if(!hInPipe)
    {
		//os_printf("!!!!!!!!!!!MGC_CreateBotInstance fail !!!!!!!!!!!!!!!!!!\r\n");
		return NULL;
    }
    ...
    os_printf("MSD/BOT: failed to allocate instance data\r\n");
    return NULL;
}
					    ...
					    const MUSB_HfiDeviceInfo* pInfo)
{
    ...
    //os_printf("pvolume->bAnnounced = %x,bLun=%x,pbot->bLuncount=%x\r\n",pVolume->bAnnounced,bLun,pBot->bLunCount);
				    ...
				    uint8_t bIsReady)
{
    ...
    if(bIsReady)
    {
		...
		else if(!pVolume->bAnnounced)
		{
			...
			//os_printf("$$$$$$$ MUSB_HfiAddDevice fun\r\n");
			...
			//os_printf("+++ bGotMedium = %x +++\r\n",pVolume->bGotMedium);
		    if(pVolume->bGotMedium)
		    {
		    	//os_printf("$$$$$$$ MUSB_HfiMediumInserted fun\r\n");
...
static uint32_t MGC_BotDataComplete(void* pParam, MUSB_Irp* pIrp)
{
	...
	MGC_MsdBotVolume* pVolume = (MGC_MsdBotVolume*)pParam;
	MGC_MsdBotProtocol* pBot = pVolume->pBot;
//os_printf("--mgc bot data complete--\r\n");
...
static uint32_t MGC_BotCswComplete(void* pParam, MUSB_Irp* pIrp)
{
	...
	RetVal = 1;
//os_printf("---mgc bot csw complete---\r\n");
	if(!pBot || (pBot != pBot->pSelf)) 
		return RetVal;

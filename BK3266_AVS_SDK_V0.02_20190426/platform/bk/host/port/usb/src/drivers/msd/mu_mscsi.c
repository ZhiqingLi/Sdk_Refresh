				...
				uint16_t wStatusLength)
{
    ...
    switch(pScsi->bState)
    {
		...
		//os_printf("******SCSI_0x12 cmd complete***\r\n");
...
//		os_printf("before send test unit ready: bMediumchange=%d,Nomedia=%d\r\n",bMediumChange,pScsi->bNoMedium);
		...
		//os_printf("#############bError=%d,bUsbError=%d##################\r\n",bError,bUsbError);
		if(bError || bUsbError)
		{
		    /* retry if needed */
...
//表示U盘的容量为0,异常?????
    ...
    else
    {
		if(bOpcode==MGC_SCSI_READ10)
			os_printf("cmd 28, dwLength=%d\r\n",dwLength);
		else
			os_printf("send scsi cmd : %x\r\n",bOpcode);

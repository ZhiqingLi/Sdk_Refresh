...
//	os_printf("MGC_FdrcReadBusState:devctl=%x\r\n",devctl);
...
//	os_printf("read bus state: bIsHost=%x\r\n",pPort->bIsHost);
...
//	os_printf("power = %x,npower=%x,devctl=%x,ndevctl=%x,pPort->bWantSession=%x\r\n",
...
uint8_t MGC_FdrcServiceDefaultEnd(MGC_Port* pPort, MGC_BsrItem* pItem)
{
	...
	//os_printf("endpoint 0 isr handle:wCsrVal=%x,wcount=%x,pPort->bIsHost=%x\r\n",wCsrVal,wCount,pPort->bIsHost);	
//	os_printf("wcsrval = %x,wcount=%x\r\n",wCsrVal,wCount);
...
//		os_printf("--csr0 value:%x,bResult:%x--\r\n",wCsrVal,bResult);
				     ...
				     MGC_BsrItem* pItem)
{
	...
	//		os_printf("MGC_FdrcServiceTransmitAvail,status=%x\r\n",status);
	    if(status)//出错
	    {
			/* reset error bits */

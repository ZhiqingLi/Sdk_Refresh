...
static uint8_t UdiskConnectFlag = 0;
...
{
    uint8_t bEnd;
	static MGC_EndpointResource resource[5];
	...
	pPort->pPrivateData = (void *)(&resource[0]);
/*
os_printf("bEndcount:%x,sizeof(Endpointresource)=%x\r\n",pPort->bEndCount,sizeof(MGC_EndpointResource));
...
/*	else
	{
		os_printf("MGC_DrcInit no memory3\r\n");
				 ...
				 uint16_t* pwCsr, uint16_t wCount)
{
    ...
    if(pPort->bIsHost)
    {
		if(status || (pEnd->bIsTx && !pEnd->dwTxSize) || (!pEnd->bIsTx && !wCount))
		{
		    ...
		    *pwCsr = 0;
//			os_printf("service default endpoint\r\n");
...
{
	//os_printf("MGC_DrcResetPort!!\r\n");
    MGC_Controller* pController = pPort->pController;

    /* kill timer */
	//os_printf("MGC_NoneCancelTimer\r\n");
...
static void MGC_WaitSessReqTimeout(void* pParam, uint16_t wTimer)
{
    ...
    MGC_Port* pPort = pController->pPort;
    MUSB_OtgClient* pOtgClient = (MUSB_OtgClient*)pPort->pOtgClient;
    //os_printf("MGC_WaitSessReqTimeout\r\n");
    if  (pPort->bIsSuspend || pPort->bIsReset)
    {
	return;
    }
...
void MGC_DrcChangeOtgState(uint8_t toOtg, MGC_Port* pPort)
{
    ...
    {
//    	os_printf("exting host or device\r\n");
...
//	os_printf("toOtg=%x\r\n",toOtg);
	...
	*/
	...
	{
//		os_printf("--MUSB A HOST:start timer--\r\n");
	//MGC_NoneArmTimer
...
//插入U盘后只是把此事件的消息传递给上层函数，何时开始枚举由上层决定
	...
	//			    	os_printf("===QQQQ==\r\n");
		...
		if((BSR_READ_OK_EVENT == RetVal)|| (BSR_ERROR_EVENT == RetVal))
...
static void MGC_OtgStateGetId(MGC_Port* pPort, uint8_t bIsReset)
{
    ...
    */
//os_printf("mgc_otgstatusgetID,pPort->bIsHost=%x,pPort->bconnectorID=%x,bIsreset=%x\r\n",
...
//指向的是MGC_Controllerwrapper结构体中的成员变量 MGC_Port PortImpl
	...
	//os_printf("MUSB_RegisterOtgClient\r\n");
	...
	if(pFunctionClient)
	{
	    ...
	    dwStatus = MGC_FunctionRegisterClient(pImplPort, pFunctionClient);
			//os_printf("MGC_FunctionRegisterClient,dwStatus=%x\r\n",dwStatus);
	}

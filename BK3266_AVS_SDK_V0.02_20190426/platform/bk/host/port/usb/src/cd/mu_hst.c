...
static void MGC_HostSetDeviceAddress(void* pParam, uint16_t wTimer)
{
	...
	//os_printf("create and start request\r\n");
    ...
    tmp=MUSB_StartControlTransfer(pPort->pInterfacePort, &(pEnumData->Irp));
...
//	os_printf("hotst get config====wLength:%x,%x\r\n");
    MUSB_StartControlTransfer(pPort->pInterfacePort, &(pEnumData->Irp));
}
...
static void MGC_HostRetryEnum(void* pParam, uint16_t wTimer)
{
    ...
    MGC_Port* pPort = pController->pPort;
//    MGC_EnumerationData* pEnumData = &(pPort->EnumerationData);
	os_printf("host retry enum\r\n");
    pPort->bWantSession = TRUE;
    pPort->pfProgramBusState(pPort);
}
#endif
uint8_t get_device_flag = 0;
/*
...
//os_printf("mgc_hostenumerator--:%x\r\n",pEnumData->bState);
uint32_t Ret = 1;
    ...
    {
    	os_printf("error :irp.dwstatus=%x ",pEnumData->Irp.dwStatus);
...
//	os_printf("$$$$$$$$$$$$ apconfigdescriptors alloc $$$$$$$$$$$$\r\n");
...
//		os_printf("pEnumdata->dwData = %x,wlengh=%x\r\n",pEnumData->dwData,wLength);
			    ...
			    MUSB_pfHubEnumerationComplete pfHubEnumerationComplete)
{
	...
	memory_usage_show();
//	os_printf("++++++ start emunate malloc size:%d ++++++++++\r\n",sizeof(MGC_Device));
    if(pDevice)
    {
...
void MUSB_RejectDevice(MUSB_BusHandle hBus, MUSB_Device* pDevice)
{
    ...
    {
//     os_printf("++++++free apConfigDescriptors +++++++\r\n");
...
//	 memory_usage_show();
    ...
    {
//    os_printf("++++++free pDescriptorBuffer +++++++\r\n");
...
//	 memory_usage_show();
    ...
    if(pImplDevice)
    {
//    	os_printf("++++++free MGC_Device struct :size:%d +++++++\r\n",sizeof(MGC_Device));
...
void MUSB_DeviceDisconnected(MUSB_Device* pDevice)
{
    ...
    MUSB_DeviceDriver* pDriver = NULL;
//os_printf("++++++musb_deviceDisconnected++++++\r\n");
    if(pDevice)
    {
	    //os_printf("+++++++0000++++\r\n");
		pPort = pDevice->pPort;

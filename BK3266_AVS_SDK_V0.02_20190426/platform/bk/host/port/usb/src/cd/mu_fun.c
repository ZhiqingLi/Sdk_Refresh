				    ...
				    MUSB_FunctionClient* pClient)
{
	...
	static uint32 MGC_Pipe_pointer[2];
	static MGC_Pipe pipe[2];
...
/*
	os_printf("bnumconfiguration=%x,sizeof(configdescriptor)=%x\r\n",bNumConfigurations,sizeof(MUSB_ConfigurationDescriptor*));
...
//    if(pPort->apConfigDescriptors)
    {
		...
		dwStatus = MGC_FunctionVerifyResources(pPort, pClient->pStandardDescriptors,
		    ...
		    MUSB_DT_CONFIG, TRUE);
		//os_printf("---MGC_FunctionRegisterClient 1 dwstatus :%x\r\n",dwStatus);
		...
		//os_printf("---MGC_FunctionRegisterClient 2 dwstatus :%x\r\n",dwStatus);			
		}
    }
 //   else
//    {
...
//    }
 ...
 /*   
    os_printf("maxendpoint=%x,sizeof(*pipe)=%x\r\n",pPort->bMaxFunctionEnds,sizeof(MGC_Pipe*));
	pPort->apPipe = (MGC_Pipe**)MUSB_MemAlloc(
	    pPort->bMaxFunctionEnds * sizeof(MGC_Pipe*));*/
//	if(pPort->apPipe)
	{
/*    os_printf("maxendpoint=%x,sizeof(MGC_Pipe)=%x\r\n",pPort->bMaxFunctionEnds,sizeof(MGC_Pipe));

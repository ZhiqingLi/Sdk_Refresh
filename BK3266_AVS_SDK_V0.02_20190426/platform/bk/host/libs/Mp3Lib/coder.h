...
#define pcm_Alaw_table      STATNAME(pcm_Alaw_table)
#define pcm_Ulaw_table      STATNAME(pcm_Ulaw_table)
...
typedef struct _SideInfo {
	int mainDataBegin;
	int privateBits;
	int scfsi[MAX_NCHAN][MAX_SCFBD];	//[2][4]	/* 4 scalefactor bands per channel */
	
	SideInfoSub	sis[MAX_NGRAN][MAX_NCHAN]; //[2][2]
} SideInfo;
...
typedef struct _DequantInfo {
	int workBuf[MAX_REORDER_SAMPS];		//[((192-126)*3)] /* workbuf for reordering short blocks */
...
typedef struct _IMDCTInfo {
	int outBuf[MAX_NCHAN][BLOCK_SIZE][NBANDS];//[2][18][32]	/* output of IMDCT */	
	int overBuf[MAX_NCHAN][MAX_NSAMP / 2];	//[2][576/2]	/* overlap-add buffer (by symmetry, only need 1/2 size) */
...
typedef struct _SubbandInfo {
    ...
    int vbuf_ext[MAX_NCHAN * 2 * NBANDS];   /* vbuf externed for LAYER 2 decoder */
...
extern CONST short pcm_Ulaw_table[256];
extern CONST short pcm_Alaw_table[256];

 ...
 * Return:      none
 **************************************************************************************/
void MP3_CALL SetBitstreamPointer(BitStreamInfo *bsi, int nBytes, unsigned char *buf)
{
	/* init bitstream */
	bsi->bytePtr = buf;
 ...
 **************************************************************************************/
static void MP3_CALL RefillBitstreamCache(BitStreamInfo *bsi)
{
 ...
 * TODO:        optimize for ARM
 **************************************************************************************/
unsigned int MP3_CALL GetBits(BitStreamInfo *bsi, int nBits)
{
 ...
 **************************************************************************************/
int MP3_CALL CalcBitsUsed(BitStreamInfo *bsi, unsigned char *startBuf, int startOffset)
{
	int bitsUsed;
 ...
 **************************************************************************************/
int MP3_CALL CheckPadBit(MP3DecInfo *mp3DecInfo)
{
	FrameHeader *fh;
 ...
 **************************************************************************************/
int MP3_CALL UnpackFrameHeader(MP3DecInfo *mp3DecInfo, unsigned char *buf)
{
	...
	if (fh->brIdx) 
	{
	    ...
	    /************************/
	    frame_size = mp3DecInfo->bitrate/1000;
	    switch(fh->layer) {
        case 1:
            frame_size = (frame_size * 12000) / mp3DecInfo->samprate;
            frame_size = (frame_size + fh->paddingBit) * 4;
            break;
        case 2:
            frame_size = (frame_size * 144000) / mp3DecInfo->samprate;
            frame_size += fh->paddingBit;
            break;
        default:
        case 3:
            if(fh->ver > 0)  //MPEG 2/2.5
                frame_size = (frame_size * 72000) / mp3DecInfo->samprate;
            else             //MPEG 1
                frame_size = (frame_size * 144000) / mp3DecInfo->samprate;
            frame_size += fh->paddingBit;
            break;
        }
        mp3DecInfo->framesize = frame_size;
	    /***********************/
	}
	else
	{
	    mp3DecInfo->framesize = 0;
	}
 ...
 **************************************************************************************/
int MP3_CALL UnpackSideInfo(MP3DecInfo *mp3DecInfo, unsigned char *buf)
{
	int gr, ch, bd, nBytes;

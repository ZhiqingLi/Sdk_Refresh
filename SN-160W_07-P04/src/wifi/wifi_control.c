...
#define WIFI_POWER_ON_INIT_TIME		200	//WiFi模组上电后的初始化持续时间(单位: 10ms)
#else
#define WIFI_POWER_ON_INIT_TIME		600	//6s(WiFi模组上电后的初始化持续时间)
#endif
...
#endif
			PowerOffCnt = FALSE;
			gSys.NextModuleID = MODULE_ID_RTC;
...
uint32_t WiFiControl(void)
{
	...
	WiFiControlGpioInit();		
	WiFiInitStep = 2;
	WiFiPowerOnInitTimeCnt = 100; //WiFi power on delay time
#endif

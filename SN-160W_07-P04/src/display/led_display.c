	...
	if((gDisplayMode <= DISP_DEV_LCD58) && (gDisplayMode >= DISP_DEV_LED1888))
	{
		...
		switch(sRtcControl->SubState)
		{
			...
			case RTC_SET_ALARM_VOLUME:
				DispChar(' ', POS_COM_THOUSAND);
				DispChar('U', POS_COM_HUNDREDS);
				DispChar((sNvmRtcInfo.AlarmVolume[sRtcControl->AlarmNum]/10), POS_COM_TENS);
				DispChar((sNvmRtcInfo.AlarmVolume[sRtcControl->AlarmNum]%10), POS_COM_UNITS);
				DispIcon(ICON_COLON, LIGHT_OFF);
				break;
#endif
...
//显示WiFi设置MCU的闹钟时间，屏整体慢闪5S
void DispWifiSetAlarmTime(void)
{
	uint32_t Temp;
	...
	Temp = sRtcControl->AlarmTime.Hour*100;
	Temp += sRtcControl->AlarmTime.Min;
	...
	if(gBlink.FalshBlinkFlag)
	{
		if(sRtcControl->AlarmNum == 1)
		{
			DispIcon(ICON_A1MARK, LIGHT_ON);
			DispIcon(ICON_ALARM1, LIGHT_ON);
		}
		else if(sRtcControl->AlarmNum == 2)
		{
			DispIcon(ICON_A2MARK, LIGHT_ON);
			DispIcon(ICON_ALARM2, LIGHT_ON);
		}
	...
	if((gDisplayMode <= DISP_DEV_LCD58) && (gDisplayMode >= DISP_DEV_LED1888))
	{
		...
		if(!IsTimeOut(&DispMsgTmr))
		{
			switch(gDispMsgId)
			{
				...
				case DISP_MSG_WIFI_SET_ALARM:
					DispWifiSetAlarmTime();
					break;
#endif

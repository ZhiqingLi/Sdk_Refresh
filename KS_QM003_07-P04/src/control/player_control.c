...
#define PLAYER_LED_CB_TIME   150       //LED闪烁扫描时间

// 获取随机数   20160724 改在app_config.h中定义
...
{		
	...
	{			
		...
		case MSG_STOP:
#ifdef BT_REDIAL_CALL_IPONE
			{
				if(GetCurBtConnectedFlag())
				{
					BTRedialLastCalledNum();
				}
			}
#else
			AudioPlayerStop();
...
//播放模式下的LED显示处理(此时间间隔PLAYER_LED_CB_TIME 可以用户自己定义)
void PlayerLedCb(void* Param)
{
...
{
	uint16_t Msg  = 0;
	SW_TIMER PlayerLedTimer;
    
#ifndef FUNC_MIXER_SRC_EN
	AudioSampleRateSet(44100);
#endif
    
	InitTimer(&PlayerLedTimer, PLAYER_LED_CB_TIME, PlayerLedCb);
	StartTimer(&PlayerLedTimer);
	if(gSys.CurModuleID == MODULE_ID_BLUETOOTH)
	{
		APP_DBG("enter bt player\n");
		if(gWiFi.WiFiNotifyChangeModeFlag == FALSE)
		{
			if(gSys.ModuleIDBackUp != MODULE_ID_BT_HF)
			{
		...
		{
			BTSetScanEnable();
			APP_DBG("Bluetooth enter Discoverable Service mode!\n");
		}
		
		Mcu_SendBtCmdToWiFi(MCU_BT_STAS, NULL);             //同步播放状态
#endif

		gWiFi.WiFiNotifyChangeModeFlag = FALSE;
    	gSys.ModuleIDBackUp = MODULE_ID_UNKNOWN;		
	}
	else
	{ 
...
//					AudioPlayerPreSong();
	...
	{
		BTSetScanDisable();
		APP_DBG("Bluetooth enter Disable mode!\n");
	}
#endif

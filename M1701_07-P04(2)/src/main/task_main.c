...
extern void SlaveBtGpioInit(void);
...
void GuiTaskEntrance(void)
{
	...
	if((gSys.CurModuleID != MODULE_ID_IDLE) && (gSys.CurModuleID != MODULE_ID_POWEROFF))
	{
...
//					SysClkDivison(1);	//ljg, call directly
		...
		if(((gSys.NextModuleID == MODULE_ID_IDLE) || (gSys.NextModuleID == MODULE_ID_POWEROFF))
			&& (gWiFi.WiFiPowerOffRequestFlag == FALSE))
		{

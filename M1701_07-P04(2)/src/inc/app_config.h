...
//Uart Rx
	...
	#endif
#else
	#if (WIFI_UART_RX_PORT == FUART_RX_GPIO_A13)
	#define WIFI_UART_RXD_PORT_OUT				GPIO_A_OUT
	#define WIFI_UART_RXD_PORT_BIT				GPIOA13	
	#elif (WIFI_UART_RX_PORT == FUART_RX_GPIO_A24)
	#define WIFI_UART_RXD_PORT_OUT				GPIO_A_OUT
	#define WIFI_UART_RXD_PORT_BIT				GPIOA24	
	#elif (WIFI_UART_RX_PORT == FUART_RX_GPIO_B8)
	#define WIFI_UART_RXD_PORT_OUT				GPIO_B_OUT
	#define WIFI_UART_RXD_PORT_BIT				GPIOB8	
	#elif (WIFI_UART_RX_PORT == FUART_RX_GPIO_B29)
	#define WIFI_UART_RXD_PORT_OUT				GPIO_B_OUT
...
#define WIFI_MIC_PORT_BIT				GPIOB2
#endif
...
//****************************************************************************************
	#define FUNC_LINEIN_EN   					// Linein
		...
		#define LINEIN_DETECT_BIT_MASK		GPIOA19
	#endif
...
#define AMP_GPIO_MUTE_EN        //打开该宏，支持GPIO控制功放MUTE
  ...
  #define AMP_MUTE_PORT_BIT				  GPIOA22
...
#define AMP_POWER_PORT_BIT				  GPIOA12
...
#define FUNC_SLAVE_BLUETOOTH_EN
  ...
  #define SLAVE_BTPOWER_EN
    ...
    #define SLAVE_BTDETE_PORT_IE				  (SLAVE_BTDETE_PORT_OUT + 1)
    #define SLAVE_BTDETE_PORT_OE				  (SLAVE_BTDETE_PORT_OUT + 2)
    #define SLAVE_BTDETE_PORT_PU				  (SLAVE_BTDETE_PORT_OUT + 5)
    #define SLAVE_BTDETE_PORT_PD				  (SLAVE_BTDETE_PORT_OUT + 6)
	#endif
    ...
    #define SLAVE_BTMUTE_PORT_IN		      GPIO_B_IN
...
//	#define USE_POWERKEY_SLIDE_SWITCH     //for slide switch case ONLY
  ...
  #define POWERON_DETECT_VOLTAGE        //开机检测电池电压，低于关机电压则直接进入关机流程
	...
	#define	OPTION_CHARGER_DETECT		      //打开该宏定义，支持GPIO检测充电设备插入功能
			...
			//充电检测端口设置
			#define CHARGE_DETECT_PORT_PU			GPIO_C_PU
			#define CHARGE_DETECT_PORT_PD			GPIO_C_PD
			#define CHARGE_DETECT_PORT_IN			GPIO_C_IN
			#define CHARGE_DETECT_PORT_IE			GPIO_C_IE
			#define CHARGE_DETECT_PORT_OE			GPIO_C_OE
			#define CHARGE_DETECT_BIT				  GPIOC1
	#endif
	  ...
	  #define FUNC_POWERKEY_SOFT_POWERON_EN //软开关用普通GPIO检测，该功能需要用硬开机和普通GPIO配合实现
		#define FUNC_GPIO_POWER_ON_EN         //GPIO控制系统上电
		  ...
		  #define POWERON_GPIO_PORT_IE			GPIO_A_IE
			...
			#define POWERON_GPIO_PORT_BIT			GPIOA0
		#endif
	#endif
...
#include "pwm.h"
 ...
 //红灯
  ...
  #define LED_RED_MODE_ON()				do{\
										              PwmDisableChannel(LED_RED_PWM_PORT_MASK);\
...
//绿灯	
	...
	#define LED_GREEN_MODE_ON()			do{\
										              PwmDisableChannel(LED_GREEN_PWM_PORT_MASK);\
...
//蓝灯	
	...
	#define LED_BLUE_MODE_ON()			 do{\
										               PwmDisableChannel(LED_BLUE_PWM_PORT_MASK);\
 ...
 #define  LED_WHITE_MODE_ON()      do{\
							                     ...
							                     PwmEnableChannel(LED_RED_PWM_PORT_MASK, PWM_IO_SEL1, PWM_MODE_OUT);\
	                                 PwmDisableChannel(LED_GREEN_PWM_PORT_MASK);\
							                     PwmConfig(LED_GREEN_PWM_PORT_MASK, 1200, 636);\
							                     PwmEnableChannel(LED_GREEN_PWM_PORT_MASK, PWM_IO_SEL1, PWM_MODE_OUT);\
	                                 PwmDisableChannel(LED_BLUE_PWM_PORT_MASK);\
							                     PwmConfig(LED_BLUE_PWM_PORT_MASK, 1200, 636);\
							                     PwmEnableChannel(LED_BLUE_PWM_PORT_MASK, PWM_IO_SEL1, PWM_MODE_OUT);\
									                 }while(0)
...
#define  LED_YELLOW_MODE_ON()	   do{\
 ...
 #define LED_WIFI_MODE_PORT_OUT		GPIO_A_OUT
 #define LED_WIFI_MODE_BIT_MASK		GPIOA20

 #define LED_WIFI_MODE_PORT_IE		(LED_WIFI_MODE_PORT_OUT + 1)
 #define LED_WIFI_MODE_PORT_OE		(LED_WIFI_MODE_PORT_OUT + 2)	
 #define LED_WIFI_MODE_PORT_PU		(LED_WIFI_MODE_PORT_OUT + 5)	
 #define LED_WIFI_MODE_PORT_PD		(LED_WIFI_MODE_PORT_OUT + 6)	
 
 #ifdef LED_WIFI_MODE_BIT_MASK
  #define LED_WIFI_MODE_ON()				do{\
										GpioSetRegOneBit(LED_WIFI_MODE_PORT_PU, LED_WIFI_MODE_BIT_MASK);\
										GpioClrRegOneBit(LED_WIFI_MODE_PORT_PD, LED_WIFI_MODE_BIT_MASK);\
										GpioClrRegOneBit(LED_WIFI_MODE_PORT_IE, LED_WIFI_MODE_BIT_MASK);\
										GpioSetRegOneBit(LED_WIFI_MODE_PORT_OE, LED_WIFI_MODE_BIT_MASK);\
										GpioClrRegOneBit(LED_WIFI_MODE_PORT_OUT, LED_WIFI_MODE_BIT_MASK);\
									}while(0)

	#define LED_WIFI_MODE_OFF()				do{\
										GpioSetRegOneBit(LED_WIFI_MODE_PORT_PU, LED_WIFI_MODE_BIT_MASK);\
										GpioClrRegOneBit(LED_WIFI_MODE_PORT_PD, LED_WIFI_MODE_BIT_MASK);\
										GpioClrRegOneBit(LED_WIFI_MODE_PORT_IE, LED_WIFI_MODE_BIT_MASK);\
										GpioSetRegOneBit(LED_WIFI_MODE_PORT_OE, LED_WIFI_MODE_BIT_MASK);\
										GpioSetRegOneBit(LED_WIFI_MODE_PORT_OUT, LED_WIFI_MODE_BIT_MASK);\
									}while(0)
 #else
  #define	LED_WIFI_MODE_ON()		
  #define	LED_WIFI_MODE_OFF()		
 #endif	
	
/****************************************************************************************/									
 #define LED_BT_MODE_PORT_OUT		GPIO_A_OUT
 #define LED_BT_MODE_BIT_MASK		GPIOA21

 #define LED_BT_MODE_PORT_IE		(LED_BT_MODE_PORT_OUT + 1)
 #define LED_BT_MODE_PORT_OE		(LED_BT_MODE_PORT_OUT + 2)	
 #define LED_BT_MODE_PORT_PU		(LED_BT_MODE_PORT_OUT + 5)	
 #define LED_BT_MODE_PORT_PD		(LED_BT_MODE_PORT_OUT + 6)	
 
 #ifdef LED_BT_MODE_BIT_MASK
  #define LED_BT_MODE_ON()				do{\
										GpioSetRegOneBit(LED_BT_MODE_PORT_PU, LED_BT_MODE_BIT_MASK);\
										GpioClrRegOneBit(LED_BT_MODE_PORT_PD, LED_BT_MODE_BIT_MASK);\
										GpioClrRegOneBit(LED_BT_MODE_PORT_IE, LED_BT_MODE_BIT_MASK);\
										GpioSetRegOneBit(LED_BT_MODE_PORT_OE, LED_BT_MODE_BIT_MASK);\
										GpioClrRegOneBit(LED_BT_MODE_PORT_OUT, LED_BT_MODE_BIT_MASK);\
									}while(0)

	#define LED_BT_MODE_OFF()				do{\
										GpioSetRegOneBit(LED_BT_MODE_PORT_PU, LED_BT_MODE_BIT_MASK);\
										GpioClrRegOneBit(LED_BT_MODE_PORT_PD, LED_BT_MODE_BIT_MASK);\
										GpioClrRegOneBit(LED_BT_MODE_PORT_IE, LED_BT_MODE_BIT_MASK);\
										GpioSetRegOneBit(LED_BT_MODE_PORT_OE, LED_BT_MODE_BIT_MASK);\
										GpioSetRegOneBit(LED_BT_MODE_PORT_OUT, LED_BT_MODE_BIT_MASK);\
									}while(0)
 #else
  #define	LED_BT_MODE_ON()		
  #define	LED_BT_MODE_OFF()		
 #endif			
#endif									
...
#ifdef FUNC_SLEEP_EN
  #define SLEEP_POWEROFF_TMR   5   //单位：分钟
#endif

#ifdef FUNC_SLEEP_LEDOFF_EN
	#define SLEEP_LED_OFF_TMR    30
#endif

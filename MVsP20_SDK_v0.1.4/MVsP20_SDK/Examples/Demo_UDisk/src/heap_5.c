...
#if 0
void init_zi_area(void)
{
    extern uint32_t Image$$RW_XMEM$$ZI$$Base;
    extern uint32_t Image$$RW_XMEM$$ZI$$Limit;
    uint8_t* zi_start = (uint8_t*)&Image$$RW_XMEM$$ZI$$Base;
    uint8_t* zi_end = (uint8_t*)&Image$$RW_XMEM$$ZI$$Limit;
    memset (zi_start, 0, (zi_end-zi_start));
}
...
void prvInitialiseHeap(void)
{
	...
	xHeapRegions[i].pucStartAddress = (uint8_t*)&Image$$RW_XMEM$$ZI$$Limit;
	xHeapRegions[i].xSizeInBytes = (0x20000000+128*1024) - (uint32_t)&Image$$RW_XMEM$$ZI$$Limit;
...
#endif
#if 1
void prvInitialiseHeap(void)	//ram config,simon
{
	extern char _end;
	HeapRegion_t xHeapRegions[2];
	...
	//printf("xHeapRegions[0].xSizeInBytes = %d\n",xHeapRegions[0].xSizeInBytes);
...
{
//	malloc_size += size;
//	printf("size:%d\t %d\t %d\t\n",size,malloc_size,xPortGetFreeHeapSize());

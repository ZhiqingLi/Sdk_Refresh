...
void BtMfiEvent(uint32_t event, BT_STACK_CALLBACK_PARAMS * param)
{
	switch(event)
	{
		case BT_STACK_EVENT_MFI_CONNECTED:
			{
				DBG("MFi Connected : bt address = %02x:%02x:%02x:%02x:%02x:%02x\n",
		...
		case BT_STACK_EVENT_MFI_DATA_RECEIVED:
			{
				...
				DBG("BT_STACK_EVENT_MFI_DATA_RECEIVED:\n");
				for(i = 0; i < param->paramsLen; i++)
				{
					DBG("0x02x ", param->params.mfiReceivedData[i]);
				}
				DBG("\n");
			}
			break;

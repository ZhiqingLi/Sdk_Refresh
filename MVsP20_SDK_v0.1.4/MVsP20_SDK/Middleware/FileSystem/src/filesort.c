...
static void FileSortByCreateTime(uint16_t file_num_in_disk, uint8_t* buf);//len 固定为5byte
...
void FileSortSearchNewFile(FS_CONTEXT* FsContext)
{
	...
	{
//		memcpy(FileNameTable[FsContext->gFsInfo.FileSum - 1], &FsContext->gCurrentEntry->CrtTimeTenth, 5);//与文件名排序公用相同buffer，变量名待修正
...
//创建时间的5个byte依次为 [0]：创建时间10ms位，[1:2]:创建时间，[3:4]:创建日期
//当buf1<buf2时，返回值<0
//当buf1==buf2时，返回值=0
//当buf1>buf2时，返回值>0
...
static void FileSortByCreateTime(uint16_t file_num_in_disk, uint8_t* buf)//len 固定为5byte
{
	...
	Start = -1;							//设置二分查找范围上边界为数组0位置
	End = CntInFileSortTableTemp;	//设置二分查找范围下边界为当前buf中元素个数

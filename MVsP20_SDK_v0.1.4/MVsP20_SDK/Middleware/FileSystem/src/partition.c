...
extern uint8_t*		ExFatFullNameBuf;
extern uint8_t*		gFolderNoFatChain;
uint8_t		ExFatBuffer[1280];
#endif
...
//	FS_DBG("SectorSizeMul: %d\n", (uint16_t)SectorSizeMul);
	...
	ExFatFullNameBuf = &ExFatBuffer[512];
	gFolderNoFatChain = &ExFatBuffer[1024];
#endif
	while(1)
	{
		LoopFlag = 0;
	...
	//			FS_DBG("Extended partition bootSecNum:%ld\n", (uint32_t)BootSecNum);
			...
			if(pPart->PartitionType == 0x07)
			{
				...
				{
					BootSecNum += Le32ToCpu(pPart->FirstSectorNum);
					gIsExFat = TRUE;//type为0x07不能完全确定是ExFat，有可能是大于32GB的FAT32，之后需要再次判断
					break;
				}
				LogicDriverId++;
			}
	...
	#define ExFatBoot ((EXFAT_BOOT_SECTOR*)((FsContext->FAT_BUFFER)))
		...
		{
			gIsExFat = FALSE;//不是ExFat，可能是大于32GB的FAT32
		}
	}							//------------------------------------------------------------ExFat End
#endif

...
void enter_standby_S3S4(bool standby_enter, bool lowpower)
{
    UINT32 i, temp_count = 0;
    msg_apps_t msg;
...
#endif
            } else if(g_app_info_state.aout_mode == AO_SOURCE_DACI2S) {
#if (CASE_BOARD_TYPE == CASE_BOARD_DEMO)
                //DEMO IC:A0,A1,A2,VRO_S
                enable_pa(3, 0, AO_SOURCE_DACI2S);
#elif (CASE_BOARD_TYPE == CASE_BOARD_DVB_ATS2825)
                //DVB:B7,B8,B9,A6
                enable_pa(4, 0, AO_SOURCE_DACI2S);
#elif (CASE_BOARD_TYPE == CASE_BOARD_DVB_ATS2823)
                //DVB:AOUTL,AOUTR,VRO,VROS
                enable_pa(0, 0, AO_SOURCE_DACI2S);
#else
                //DEMO IC:A0,A1,A2,A3
                enable_pa(1, 0, AO_SOURCE_DACI2S);
#endif
            } else {
                ;//nothing
            }
...
app_result_e sys_power_off(bool standby_enter, bool lowpower)
{
    ...
    if(sys_comval->bat_charge_mode == BAT_CHARGE_MODE_OUTER) {
        enter_standby_S3S4(standby_enter, lowpower);
    } else
#endif
    {
...
#if defined(BOARD_DPI_7C370_FAR)
void __section__(".text.bank10") hold_system_power(bool on)
{

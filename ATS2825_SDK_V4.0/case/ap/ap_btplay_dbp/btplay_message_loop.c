...
void __section__(".bank_2") message_loop_init_dbp(uint8 *cur_connected_flag, uint8 *last_connected_flag)
{
    uint8 loop = 0;
    bool  wait_connect = FALSE;

    //如果刚开始还未连接，强制播报一次等待连接
    if((g_btplay_cur_info.status == BTPLAY_IDLE) && (g_standby_exit_flag == FALSE) \
            && (g_ap_switch_var.call_background_status != CALL_BACK_STATUS_BACK)) {
        wait_connect = TRUE;
        btplay_tts_play_dbp(0, g_bt_stack_cur_info, cur_connected_flag, last_connected_flag);
    }

    for(loop = 0; loop < RMT_DEV_NUM; loop++) {
        last_connected_flag[loop] = ((g_bt_stack_cur_info.rmt_dev[loop].serv_conn & A2DP_CONNECTED) != 0) \
                                    || ((g_bt_stack_cur_info.rmt_dev[loop].serv_conn & HFP_CONNECTED) != 0);
    }

    if(wait_connect) {
        /* 播放 "等待连接" 后如果蓝牙已连接, 则重置该标志以在之后播放 "蓝牙连接成功". */
        last_connected_flag[0] = 0;
    }
...
app_result_e get_message_loop(void)
{
...
#ifdef ENABLE_DOUBLE_PHONE_
    uint8 loop = 0;
    bt_stack_info_t temp_bt_stack_cur_info;
    uint8 cur_connected_flag[RMT_DEV_NUM] = {0};
    uint8 last_connected_flag[RMT_DEV_NUM] = {0};
#else
    uint8 cur_hfp_status;
    uint8 cur_a2dp_status;
#endif
    ...
    message_loop_init();
#ifdef ENABLE_DOUBLE_PHONE_
    message_loop_init_dbp(cur_connected_flag, last_connected_flag);
#endif
    ...
    while(1) {
#ifdef ENABLE_DOUBLE_PHONE_
        libc_memcpy(&temp_bt_stack_cur_info, &g_bt_stack_cur_info, sizeof(bt_stack_info_t));
#endif
        update_sound_mode();
...
#ifndef ENABLE_TRUE_WIRELESS_STEREO_USE
#if 0
...
#ifndef ENABLE_DOUBLE_PHONE_
        {
            ...
            //双手机的tts播报和数码管的显示处理
            for(loop = 0; loop < RMT_DEV_NUM; loop++) {
                cur_connected_flag[loop] = ((temp_bt_stack_cur_info.rmt_dev[loop].serv_conn & A2DP_CONNECTED) != 0) \
                                           || ((temp_bt_stack_cur_info.rmt_dev[loop].serv_conn & HFP_CONNECTED) != 0);
                if(cur_connected_flag[loop] != last_connected_flag[loop]) {
                    btplay_tts_play_dbp(loop, temp_bt_stack_cur_info, cur_connected_flag, last_connected_flag);
                }
                last_connected_flag[loop] = cur_connected_flag[loop];
            }
#endif

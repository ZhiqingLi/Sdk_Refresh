...
app_result_e __section__(".text.dbp")KeyNextLongClickDeal(void);
app_result_e __section__(".text.dbp")KeyPrevLongClickDeal(void);
#endif
const key_event_map_t __section__(".rodata.ke_maplist") btcall_ke_maplist[] = {
    ...
    { { KEY_NEXT_VOLADD, 0, KEY_TYPE_LONG | KEY_TYPE_HOLD, 0}, key_next_deal},
    { { KEY_PREV_VOLSUB, 0, KEY_TYPE_LONG | KEY_TYPE_HOLD, 0}, key_prev_deal},
#else
...
#endif
#ifdef ENABLE_DOUBLE_PHONE_
    { { KEY_NEXT, 0, KEY_TYPE_LONG_UP, 0}, KeyNextLongClickDeal},
    { { KEY_PREV, 0, KEY_TYPE_LONG_UP, 0}, KeyPrevLongClickDeal},
...
#endif
#if defined(BOARD_JL_36410_FAR)
	{ { KEY_MODE, 0, KEY_TYPE_SHORT_UP, 0}, key_phone_deal},
	{ { KEY_EQ, 0, KEY_TYPE_SHORT_UP, 0}, key_phone_deal},
	{ { KEY_VADD, 0, KEY_TYPE_SHORT_UP, 0}, key_phone_deal},
	{ { KEY_VSUB, 0, KEY_TYPE_SHORT_UP, 0}, key_phone_deal},
    { { KEY_CLK, 0, KEY_TYPE_SHORT_UP, 0}, key_phone_deal},
	{ { KEY_CLK, 0, KEY_TYPE_LONG, KEY_DEAL_FILTER}, key_phone_long_deal},
#endif
...
app_result_e __section__(".text.dbp")KeyPrevLongClickDeal(void)
{
    ...
    if(g_app_info_state_all.two_phone_mode == 0) {
        return key_prev_deal();
    }

    /** 支持一拖二才执行
    */
    if(RMT_DEV_NUM > 1) {
        libc_print("\npre2 call double incoming deal", 0, 0);
        i = (uint8)FindCallStateOfTwoPhone(HFP_STATUS_INCOMING);
        if(i < RMT_DEV_NUM) {
            com_btmanager_hfp_call_req(BTCALL_HFP_HANGUP_SPECIAL_CALL);
        }
        return RESULT_NULL;
    }
}
...
app_result_e __section__(".text.dbp")KeyNextLongClickDeal(void)
{
    ...
    if(g_app_info_state_all.two_phone_mode == 0) {
        return key_next_deal();
    }

    /** 支持一拖二才执行
    */
    if(RMT_DEV_NUM > 1) {
        libc_print("\nnext2 call double incoming deal", 0, 0);
        i = (uint8)FindCallStateOfTwoPhone(HFP_STATUS_INCOMING);
        if(i < RMT_DEV_NUM) {
            com_btmanager_hfp_call_req(BTCALL_HFP_HANGUP_OLD_CALL);
        }
        return RESULT_NULL;
    }

...
const key_event_map_t __section__(".rodata.ke_maplist") usound_ke_maplist[] = {
    ...
    /*! 短按按next切换下一曲事件 */
    { { KEY_NEXT_VOLADD, 0,  KEY_TYPE_LONG | KEY_TYPE_HOLD, KEY_DEAL_NULL}, _key_next_deal},
    /*! 短按按prev切换上一曲事件 */
    { { KEY_PREV_VOLSUB, 0,  KEY_TYPE_LONG | KEY_TYPE_HOLD, KEY_DEAL_NULL}, _key_prev_deal},
#else
    /*! 短按按next切换下一曲事件 */
...
#if defined(BOARD_LINKPLAY_MUVO)
    /*! 音量加 */
    { { KEY_NEXT_VOLADD, 0, KEY_TYPE_SHORT_UP, KEY_DEAL_FILTER_UP}, _key_vol_add_deal},
#else
    /*! 音量加 */
...
#if defined(BOARD_LINKPLAY_MUVO)
    /*! 音量减 */
    { { KEY_PREV_VOLSUB, 0, KEY_TYPE_SHORT_UP, KEY_DEAL_FILTER_UP}, _key_vol_sub_deal},
#else
    /*! 音量减 */

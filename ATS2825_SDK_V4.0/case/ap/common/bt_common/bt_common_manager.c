...
static void btmanager_global_var_init(bool standby_exit)
{
    ...
    if(standby_exit == TRUE) {
        ...
        g_btmanager_gl_var.smart_rec_status = SMART_REC_IDLE;
...
// 配置管理，OTA时只需要SPP、BLE
    ...
    if(g_app_info_state_all.two_phone_mode == 1) {
        cfg->enable_spp = 0;//com_get_config_default(BTMANAGER_ENABLE_SPP);
        cfg->enable_ble = 0;//com_get_config_default(BTMANAGER_ENABLE_BLE);
    }
#endif
...
#ifdef ENABLE_TRUE_WIRELESS_STEREO
uint8 __section__(".bank_2") auto_connect_get_tws_dev_list(bt_paired_info_t *cur_list)
{
    ...
    if((cur_list->dev_num > 0) && (g_bt_auto_connect_ctrl.enable_auto_connect == 1)) {
        ...
        if(g_bt_auto_connect_ctrl.tws_prior_role == TWS_SLAVE) {
            ...
            g_bt_auto_connect_ctrl.dev_info[0].conn_flag = 0;
        }
...
void __section__(".bank_2") auto_connect_get_phone_dev_list(bt_paired_info_t *cur_list, uint8 left_dev_num,
        uint8 auto_ptr)
{
    //bt_paired_info_t *cur_list;
    uint8 i, j;
...
#ifdef ENABLE_TRUE_WIRELESS_STEREO
    if(g_app_info_state_all.two_phone_mode == 0) {
        auto_ptr = auto_connect_get_tws_dev_list(cur_list);
        if(support_dev_num_tmp > 1) {
            support_dev_num_tmp --;
        }
    }
#endif
    auto_connect_get_phone_dev_list(cur_list, support_dev_num_tmp, auto_ptr);
...
#ifdef ENABLE_DOUBLE_PHONE_
uint8 __section__(".bank_3") get_sync_rmt_device_list(void)
{
    uint8 dev_num;
    bt_paired_info_t *cur_list;
    msg_apps_t msg;

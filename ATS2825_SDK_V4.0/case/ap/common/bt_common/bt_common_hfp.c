...
static const enum HFP_APP_EventCodeEnum s_hfp_ev_codes[] = {
    ...
    HFP_EV_CANCEL_CALL_REQ,    /* 3 BTCALL_HFP_HANGUP_SPECIL_CALL*/
    HFP_EV_CANCEL_CALL_REQ,    /* 4 BTCALL_HFP_HANGUP_OLD_CALL*/
#endif
};

#ifdef ENABLE_DOUBLE_PHONE_
uint8 FindCallStateOfTwoPhone(uint8 Type)
{
    uint8 i;
    if(g_btmanager_gl_var.support_dev_num > 1) {
        for(i = 0; i < g_btmanager_gl_var.support_dev_num; i++) {
            if(g_bt_stack_cur_info.rmt_dev[i].hfp_status == HFP_STATUS_CALLING) {
                if(g_bt_stack_cur_info.rmt_dev[i ^ 1].hfp_status == HFP_STATUS_INCOMING) {
                    if(Type == HFP_STATUS_INCOMING) {
                        return i ^ 1;
                    } else if(Type == HFP_STATUS_CALLING) {
                        return i;
                    }
                }
            }
    ...
    return g_btmanager_gl_var.support_dev_num;
}
#endif
...
#ifdef ENABLE_DOUBLE_PHONE_
    if(BTCALL_HFP_HANGUP_SPECIAL_CALL == req) {
        i = FindCallStateOfTwoPhone(HFP_STATUS_INCOMING);
        ...
        return 0;
    } else if(BTCALL_HFP_HANGUP_OLD_CALL == req) {
        i = FindCallStateOfTwoPhone(HFP_STATUS_CALLING);

        if(i == g_btmanager_gl_var.support_dev_num) {
            return 0;
        }

        com_set_active_dev(MSG_BTSTACK_SET_HFP_ACTIVE_DEV_SYNC, i ^ 1);
        //消息类型(即消息名称)
        msg.content.data[0] = s_hfp_ev_codes[req];
        ...
        sys_os_time_dly(100);//
        {
            msg.content.data[0] = (uint8)HFP_EV_ANSWER_CALL_REQ;

            //发送同步消息
            send_sync_msg_btmanager(g_bt_stack_cur_info.rmt_dev[i ^ 1].addr.bytes, &msg, NULL, 0);
        }

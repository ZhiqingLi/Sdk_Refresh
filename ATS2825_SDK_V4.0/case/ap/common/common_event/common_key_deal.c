...
app_result_e RfcommBackLink(void);
app_result_e RfcommDisconnect(void);
#endif
...
app_result_e com_key_deal_enter_double_phone(void);
#endif

#ifdef SMART_BOX_BUF_MODE
app_result_e com_key_record_buf_start(void);
//app_result_e com_key_record_buf_end(void);
#endif
#ifdef SMART_BOX_OPUS_MODE
int lp_key_record_triger(void);
void lp_set_vol_dev_to_phone(void);
//app_result_e com_key_record_stop(void);
#endif //SMART_BOX_OPUS_MODE
#if defined(SMART_BOX_OPUS_MODE) || defined(SMART_BOX_BUF_MODE)
int lp_key_record_stop(void);
#endif

#if defined(SUPPORT_MIC_MUTE_FUNCTION)
app_result_e com_key_dal_mic_mute(void);
#endif
...
const key_event_map_t __section__(".rodata.ke_maplist") com_ke_maplist[] = {
    ...
    { { KEY_VADD, 0, KEY_TYPE_SHORT_UP, KEY_DEAL_NULL }, com_key_deal_volume_add },
    { { KEY_VADD, 0, KEY_TYPE_LONG | KEY_TYPE_HOLD, KEY_DEAL_FILTER_UP }, com_key_deal_volume_add },
#else
    /*! 音量加 */
    { { KEY_VADD, 0, KEY_TYPE_DOWN | KEY_TYPE_LONG | KEY_TYPE_HOLD, KEY_DEAL_FILTER_UP }, com_key_deal_volume_add },
#endif

#if defined(BOARD_LINKPLAY_MUVO)
    { { KEY_NEXT_VOLADD, 0, KEY_TYPE_SHORT_UP, KEY_DEAL_NULL }, com_key_deal_volume_add },
#else
...
//#endif //OTA_VERSION
#if defined(BOARD_JL_36410_FAR)
    { { KEY_VSUB, 0, KEY_TYPE_SHORT_UP, KEY_DEAL_NULL }, com_key_deal_volume_sub },
    { { KEY_VSUB, 0, KEY_TYPE_LONG | KEY_TYPE_HOLD, KEY_DEAL_FILTER_UP }, com_key_deal_volume_sub },
#else
    /*! 音量减 */
    { { KEY_VSUB, 0, KEY_TYPE_DOWN | KEY_TYPE_LONG | KEY_TYPE_HOLD, KEY_DEAL_FILTER_UP }, com_key_deal_volume_sub },
#endif
#if defined(BOARD_LINKPLAY_MUVO)
    { { KEY_PREV_VOLSUB, 0, KEY_TYPE_SHORT_UP, KEY_DEAL_NULL }, com_key_deal_volume_sub },
#else
...
//#endif
...
#ifdef SMART_BOX_OPUS_MODE
    { { KEY_EQ, 0, KEY_TYPE_SHORT_UP, 0 }, lp_key_record_triger },
    /*! hid disconenct */
    //{ { KEY_EQ, 0, KEY_TYPE_DBL_CLICK, 0 }, lp_key_record_stop },

#if defined(SUPPORT_MIC_MUTE_FUNCTION)
#if defined(BOARD_JL_36410_FAR)
    { { KEY_CLK, 0, KEY_TYPE_LONG, KEY_DEAL_FILTER}, com_key_dal_mic_mute},
#else
    { { KEY_EQ, 0, KEY_TYPE_LONG, KEY_DEAL_FILTER}, com_key_dal_mic_mute},
#endif
#endif
#endif //#ifdef SMART_BOX_OPUS_MODE
#ifdef SMART_BOX_BUF_MODE
    { { KEY_EQ, 0, KEY_TYPE_DBL_CLICK, 0 }, lp_key_record_stop },
    { { KEY_EQ, 0, KEY_TYPE_SHORT_UP, 0 }, com_key_record_buf_start },
#endif //SMART_BOX_BUF_MODE 
    ...
    { { KEY_MODE, 0, KEY_TYPE_LONG, KEY_DEAL_FILTER }, com_key_deal_shutoff },
#else
...
#endif
#ifdef ENABLE_DOUBLE_PHONE_
    { { KEY_MODE, 0, KEY_TYPE_DBL_CLICK, 0 }, com_key_deal_enter_double_phone },
#endif //#ifdef ENABLE_DOUBLE_PHONE_

#ifdef __BQB_TEST_PROFILE_
    {{ KEY_NEXT, 0, KEY_TYPE_DBL_CLICK },   RfcommBackLink},
    {{ KEY_EQ,   0, KEY_TYPE_DBL_CLICK },   RfcommDisconnect},
#endif
...
app_result_e com_key_dal_mic_mute(void)
{
    libc_print("mic: ", sys_comval->mic_mute, 2);
    if(sys_comval->mic_mute) {
        sys_comval->mic_mute = 0;
    } else {
        sys_comval->mic_mute = 1;
        lp_record_stop_force();
    }

    com_smart_mic_mute(sys_comval->mic_mute, TRUE);
    sys_vm_write(sys_comval, VM_AP_SETTING, sizeof(comval_t));
}
#endif


#ifdef ENABLE_TRUE_WIRELESS_STEREO
void __section__(".text.BANK2") com_app_deal_play_prev_next(void)
{
...
//  if (g_app_info_state_all.voice_recongnize_mode != 0)
//  {
//      return RESULT_NULL;
//  }
...
#if defined(BOARD_LINKPLAY_DEMO) || defined(BOARD_LINKPLAY_MUVO) || defined(BOARD_DPI_7C370_FAR) || defined(BOARD_JL_36410_FAR)
    //lp_set_vol_dev_to_phone();
#endif
...
app_result_e com_key_deal_enter_double_phone(void)
{
    ...
    //unstall current bt stack
    //com_btmanager_exit(FALSE,TRUE);

    if(g_app_info_state_all.two_phone_mode == 0) {
        //now tws mode
        g_app_info_state_all.two_phone_mode = 1;
        //ret_val = RESULT_ENTER_BT_DBP;
    } else {
        //now two phone mode
        g_app_info_state_all.two_phone_mode = 0;
        //ret_val = RESULT_BLUETEETH_SOUND;
        com_btmanager_exit(FALSE, TRUE);
    }
    //install new bt stack

...
void __section__(".bank") com_battery_charge_full_check(uint32 real_battery_grade)
{
    uint16 msg;

    g_sys_counter.charge_full_cnt++;
    if(g_sys_counter.charge_full_cnt > 4) { //2秒后，检测电压是否满
        if(real_battery_grade < (BATTERY_GRADE_MAX + 1)) { //BATTERY_GRADE_MAX = 4.20V
            g_sys_counter.charge_full_cnt = 0;
...
void com_battery_charge_deal(void)
{
    ...
    tmp_charge_state = key_battery_get_status(&real_battery_grade, &volume_limit, &app_id);

    if(real_battery_grade < BATTERY_GRADE_MAX) {
        tmp_battery_value = real_battery_grade;
    } else {
        tmp_battery_value = BATTERY_GRADE_MAX;
    }
    //PRINT_INFO_INT("bat charge:", (tmp_charge_state << 8) | tmp_battery_value);
    //PRINT_INFO_INT("bat reg:", ((act_readb(BATADC_DATA)) & 0x7F));
    volume_limit *= 10;
...
//    }
    ...
    if((g_app_info_state.charge_state == BAT_NORMAL) && (g_app_info_state.bat_value <= 1)) {
        ...
        com_battery_charge_full_check(real_battery_grade);
    } else {
        ; //nothing for QAC
    }
}

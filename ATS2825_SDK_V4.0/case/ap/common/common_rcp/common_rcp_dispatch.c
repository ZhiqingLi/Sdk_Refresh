...
#endif
    pBackBuf = sys_malloc(p_rcp->rcp_len);
    if(pBackBuf != NULL) {
        com_btmanager_sppble_copy_rcp(pBackBuf, p_rcp->rcp_len);
        com_rcp_dispatch_invalid(pBackBuf, p_rcp->rcp_len, p_rcp->rcp_flag);
        sys_free(pBackBuf);
    }
...
dispatch_again:
    ...
    while(cur_rcp.rcp_len > 0) {
        ...
        if(g_rcp_long_buffer_flag == TRUE) {
            ...
            //com_rcp_dispatch_invalid(g_rcp_long_buffer + g_rcp_long_index, cur_len, p_rcp->rcp_flag);
...
void com_rcp_dispatch_invalid(uint8 *rcp_data, uint16 rcp_len, uint8 rcp_serv)
{
    ...
    if((rcp_serv < RCP_SERVICE_MAX) && (g_app_ext_serv_cb[rcp_serv] != NULL)) {

...
{
    sys_share_query_read(APP_ID_BTSTACK, g_p_bt_stack_cur_info);
...
app_result_e get_message_loop(void)
{
    ...
    while(1) {
...
#endif
        {
            ...
            old_dev_id = g_p_bt_stack_cur_info->a2dp_active_id;
        ...
        if((btplay_gl_var.codec_flag == TRUE) &&
                ((g_p_bt_stack_cur_info->controller_status == BT_CONTROLLER_STATUS_IDLE)
                 || (g_p_bt_stack_cur_info->rmt_dev[g_p_bt_stack_cur_info->a2dp_active_id].a2dp_status == A2DP_STATUS_NONE)
                 || (old_dev_id != g_p_bt_stack_cur_info->a2dp_active_id)
                 || (g_p_tws_info->pri_full_exit_flag == 1))) {
            libc_print("old_dev_id: ", old_dev_id, 2);
            old_dev_id = g_p_bt_stack_cur_info->a2dp_active_id;
            btplay_engine_pipe_control(PIPE_OP_FILTER);
            a2dp_decode_quit();

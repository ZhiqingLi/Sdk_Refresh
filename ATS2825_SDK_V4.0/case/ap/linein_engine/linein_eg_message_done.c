...
app_result_e linein_eg_message_done_bank(private_msg_t *cur_msg)
{
    ...
    switch(cur_msg->msg.type) {
        ...
        case MSG_LINEIN_EG_GET_CAP_SIZE_SYNC: {
            *(uint32 *)(cur_msg->msg.content.addr) = 0;
        }
        break;
        default:
            //aa channel cannot smart recognize
            if(g_linein_channel_sel == LINEIN_CH_A) {
                break;
            }
            if(cur_msg->msg.type == MSG_PLAYEREG_RECOGNIZE_PAUSE_MUSIC) {
                smart_enter_stop();
            }
            if(cur_msg->msg.type == MSG_PLAYEREG_RECOGNIZE_RESUME_MUSIC) {
                smart_exit_play();
            }
            g_linein_eg_result = smart_process_message_deal(cur_msg, g_mmm_pp_handle, mmm_pp_cmd, NULL);
            break;
    }

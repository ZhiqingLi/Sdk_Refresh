...
SupportDBP=`grep -a "SupportDBP" $int_file | awk -F '=' '{print $2}'`

SupportI2SFormat=`grep -a "SupportI2SFormat" $int_file | awk -F '=' '{print $2}'`
...
if [ $SupportI2SFormat == 0 ]; then
	cp -f ../../psp_rel/bin/AFI_48K.bin ../../psp_rel/bin/AFI.bin
fi

if [ $SupportI2SFormat == 1 ]; then
	cp -f ../../psp_rel/bin/AFI_blank_48K.bin ../../psp_rel/bin/AFI.bin
fi

if [ $SupportI2SFormat == 2 ]; then
	cp -f ../../psp_rel/bin/AFI_16K.bin ../../psp_rel/bin/AFI.bin
fi

if [ $SupportI2SFormat == 3 ]; then
	cp -f ../../psp_rel/bin/AFI_NEAR.bin ../../psp_rel/bin/AFI.bin
fi

if [ $SupportI2SFormat == 3 ]; then
	cp -f dsplib/hf080808_mic.dsp dsplib/hf080808.dsp
else
	cp -f dsplib/hf080808_I2S.dsp dsplib/hf080808.dsp
fi

if [ "$config_cfg_name" == "jl_36410_far.cfg" ]; then
	cp -f media/tts_rc_jl_36410.lib media/tts_rc.lib
else
	cp -f media/tts_rc_demo.lib media/tts_rc.lib
fi

./GenerateBoardFile.sh $int_file
...
then   	
	...
	sed '0,/^\/\/\#define SUPPORT_OTA/s//\#define SUPPORT_OTA/' ../inc/case_independent.h> tmpfile
...
then   
	...
	sed '0,/^\/\/\#define ENABLE_DOUBLE_PHONE_.*/s//\#define ENABLE_DOUBLE_PHONE_/' ../inc/case_independent.h > tmpfile
	mv tmpfile ../inc/case_independent.h

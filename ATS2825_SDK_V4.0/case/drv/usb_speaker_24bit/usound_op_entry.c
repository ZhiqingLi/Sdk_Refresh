...
int usound_inner_get_status(usound_status_t *pstatus)
{
    ...
    //pstatus->start_play = g_start_play;//play flag
    ...
    pstatus->uspeaker24_energy = 0x500;//g_usb_data_count;
...
int usound_inner_get_info(uint32 info_index, uint32 *pinfor, uint32 *info_len)
{
    ...
    if(g_play_data_count >= (int32)read_len) {
        g_no_data_count = 0;
        ...
        g_play_data_count -= (int32)read_len;
...
int usound_inner_set_cmd(uint32 cmd, uint32 cmd_param)
{
    ...
    switch(cmd) {
        ...
        case SET_USPEAKER_24BIT:
            if(USPEAKER24_PLAY == cmd_param) {
                //dac_start();
                libc_print("DAC start cmd", 0, 0);
            } else if(USPEAKER24_PAUSE == cmd_param) {
                dac_stop();
                libc_print("DAC stop cmd", 0, 0);
            } else if(USPEAKER24_SAMPLE_RATE == cmd_param) {
                g_set_sample_flag = 0;
                set_dac_sample_rate(g_play_sample / 1000);
            } else if(USPEAKER24_TTS_START == cmd_param) {
                libc_print("tts start", 0, 0);
                g_ud_stop_by_tts = 1;
            } else if(USPEAKER24_TTS_STOP == cmd_param) {
                libc_print("tts stop", 0, 0);
                g_ud_stop_by_tts = 0;
            } else {
                ;//nothing
            }

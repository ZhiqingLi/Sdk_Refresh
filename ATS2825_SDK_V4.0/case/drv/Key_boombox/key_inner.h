...
extern void dae_param_print0(uint8 audiopp_type);
...
extern int32 key_inner_dae_filter_ctl(uint32 on_off, void *info_ptr, void *null3);
extern int32 key_inner_i2c_write(char address, char *buf, int size);
extern int32 key_inner_i2c_read(char address, char *buf, int size);
extern int32 key_inner_i2c_read_write(uint8 address, uint8 *buf, uint32 size);
extern int32 key_inner_spi_write(uint8 *buf, uint16 size);
extern int32 key_inner_spi_read(uint8 *buf, uint16 size);
extern int32 key_inner_spi_read_write(uint8 *wbuf, uint8 *rbuf, uint32 size);
#if defined(ENABLE_ALEXA_LED_NOTIFY)
extern int32 key_inner_alexa_led_show(uint32 led_cmd, void *arg0, void *arg1);
#endif

...
extern void SIRQ_int_process(void);

void SIRQ_Init(void)
{
#if (EXTERN_INTERRUPT_PIN_A22 == 1)
    act_writel(act_readl(GPIOAINEN) & (~(1 << 22)), GPIOAINEN);
    act_writel(act_readl(GPIOAOUTEN) & (~(1 << 22)), GPIOAOUTEN);
    act_writel(act_readl(MFP_CTL0) & (~(MFP_CTL0_GPIOA22_MASK)) | (5 << MFP_CTL0_GPIOA22_SHIFT), MFP_CTL0);
#elif (EXTERN_INTERRUPT_PIN_B11 == 1)
    act_writel(act_readl(GPIOBINEN) & (~(1 << 11)), GPIOBINEN);
    act_writel(act_readl(GPIOBOUTEN) & (~(1 << 11)), GPIOBOUTEN);
    act_writel(act_readl(MFP_CTL2) & (~(MFP_CTL2_GPIO_B11_MASK)) | (3 << MFP_CTL2_GPIO_B11_SHIFT), MFP_CTL2);
#endif

#if (EXTERN_INTERRUPT_PIN_A1 == 1)
    act_writel(act_readl(GPIOAINEN) & (~(1 << 1)), GPIOAINEN);
    act_writel(act_readl(GPIOAOUTEN) & (~(1 << 1)), GPIOAOUTEN);
    act_writel(act_readl(MFP_CTL1) & (~(MFP_CTL1_GPIOA1_MASK)) | (5 << MFP_CTL1_GPIOA1_SHIFT), MFP_CTL1);
#elif (EXTERN_INTERRUPT_PIN_A7 == 1)
    act_writel(act_readl(GPIOAINEN) & (~(1 << 7)), GPIOAINEN);
    act_writel(act_readl(GPIOAOUTEN) & (~(1 << 7)), GPIOAOUTEN);
    act_writel(act_readl(MFP_CTL1) & (~(MFP_CTL1_GPIOA7_MASK)) | (3 << MFP_CTL1_GPIOA7_SHIFT), MFP_CTL1);
#endif

#if (EXTERN_INTERRUPT_RISING_EDGE_TRIG == 1)

#if (EXTERN_INTERRUPT_PIN_B11 == 1 || EXTERN_INTERRUPT_PIN_A22 == 1)    //SIRQ0
    act_writel(act_readl(INTC_EXTCTL) & (~(1 << INTC_EXTCTL_EXTYPE0)), INTC_EXTCTL);
    //SIRQ0 设置下拉
    act_writel(act_readl(PADPUPD) | (1 << PADPUPD_SIRQ0PD) & (~(1 << PADPUPD_SIRQ0PU)), PADPUPD);
#endif
#if (EXTERN_INTERRUPT_PIN_A1 == 1 || EXTERN_INTERRUPT_PIN_A7 == 1)      //SIRQ1
    act_writel(act_readl(INTC_EXTCTL) & (~(1 << INTC_EXTCTL_EXTYPE1)), INTC_EXTCTL);
    //SIRQ1 设置下拉
    act_writel(act_readl(PADPUPD) | (1 << PADPUPD_SIRQ1PD) & (~(1 << PADPUPD_SIRQ1PU)), PADPUPD);
#endif
    ...
    act_writel(act_readl(INTC_EXTCTL) | (1 << INTC_EXTCTL_EXTYPE1), INTC_EXTCTL);
    //SIRQ1 设置上拉
    act_writel(act_readl(PADPUPD) | (1 << PADPUPD_SIRQ1PU) & (~(1 << PADPUPD_SIRQ1PD)), PADPUPD);
#endif

#endif

#if (EXTERN_INTERRUPT_PIN_B11 == 1 || EXTERN_INTERRUPT_PIN_A22 == 1)    //SIRQ0
    //disable SIRQ0 interrupt pending mask
    act_writel(act_readl(INTC_MSK) & (~(1 << INTC_MSK_SIRQ0_IM)), INTC_MSK);

    // set SIRQ0 interrupt source
    act_writel(act_readl(INTC_CFG0) & (~(1 << INTC_CFG0_SIRQ0_CFG0)), INTC_CFG0);
    act_writel(act_readl(INTC_CFG1) & (~(1 << INTC_CFG1_SIRQ0_CFG1)), INTC_CFG1);
    act_writel(act_readl(INTC_CFG2) & (~(1 << INTC_CFG2_SIRQ0_CFG2)), INTC_CFG2);

    // clear interrupt pending
    act_writel(act_readl(INTC_EXTIP) | (1 << INTC_EXTIP_E0PD), INTC_EXTIP);

    sys_request_irq(IRQ_SIRQ0, SIRQ_int_process);
#endif

#if (EXTERN_INTERRUPT_PIN_A1 == 1 || EXTERN_INTERRUPT_PIN_A7 == 1)      //SIRQ1
    //disable SIRQ1 interrupt pending mask
    act_writel(act_readl(INTC_MSK) & (~(1 << INTC_MSK_SIRQ1_IM)), INTC_MSK);

    // set SIRQ1 interrupt source
    act_writel(act_readl(INTC_CFG0) & (~(1 << INTC_CFG0_SIRQ1_CFG0)), INTC_CFG0);
    act_writel(act_readl(INTC_CFG1) & (~(1 << INTC_CFG1_SIRQ1_CFG1)), INTC_CFG1);
    act_writel(act_readl(INTC_CFG2) & (~(1 << INTC_CFG2_SIRQ1_CFG2)), INTC_CFG2);

    // clear interrupt pending
    act_writel(act_readl(INTC_EXTIP) | (1 << INTC_EXTIP_E1PD), INTC_EXTIP);

    sys_request_irq(IRQ_SIRQ1, SIRQ_int_process);
#endif
}

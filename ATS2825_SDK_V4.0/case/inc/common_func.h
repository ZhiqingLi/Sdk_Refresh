...
typedef struct {
    ...
    uint8 ap_id; //关机的应用
    uint8 engine_state;
    uint8 for_dbp_flag;//whether enter s3 is dbp
    uint16 reserve[1];
} g_config_var_t;
...
extern void i2s_mfp_cfg(void) __FAR__;

#if defined(SUPPORT_MIC_MUTE_FUNCTION)
extern void com_smart_mic_mute(uint8 flag, bool tone) __FAR__;
#endif

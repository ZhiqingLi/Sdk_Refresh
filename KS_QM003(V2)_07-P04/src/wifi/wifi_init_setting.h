...
#define MCU_CAP_PRJ_PROJECT_NAME	"KS_QM003"

//**MCU+SID+name&:设置设备默认SSID名称
#define MCU_SSID_NAME	"KS"
...
#define MCU_CAP_001_BIT23	0	//WiFi模块是否有接USB麦克风，默认为没有接0，否则为1，要使能这一位设定，必须要同时设定MCU+PTV+nnn：nnn>=002 才生效；只有A31/A28支持这个功能；并且固件需要是标准固件。
...
#define MCU_CAP_001_BIT26	0	//是否Mono输出，默认为0，立体声输出，否则设置为1。如果只有一个喇叭，请设置该位。
...
#endif
#define MCU_CAP_002_BIT17	1	//是否关闭multiroom多音箱功能，默认0不关闭
#define MCU_CAP_002_BIT18	0	//MCU是否支持同时播放和录音，不支持为1，默认0支持
#define MCU_CAP_002_BIT19	0	//是否有外部音源输入（AUX/BT...) ，如果有，还需要回复MCU+CAP+PLM
...
#define MCU_CAP_PLM_BIT2	0	//有无蓝牙，默认0（如果App不需要显示当前处于蓝牙状态，例如切换到蓝牙状态WiFi已经关闭，则此处不需要回复1）
...
#define MCU_CAP_EXT_BIT1	0	//位1 有无灯
#define MCU_CAP_EXT_BIT2	0	//位2 有无冷关灯
#define MCU_CAP_EXT_BIT3	0	//位3.有无暖光灯
#define MCU_CAP_EXT_BIT4	0	//位4.有无RGB灯
#define MCU_CAP_EXT_BIT5	0	//位5.有无律动功能
#define MCU_CAP_EXT_BIT6	0	//位6.有无自动开灯功能
#define MCU_CAP_EXT_BIT7	0	//位7 有无自动关灯功能
#define MCU_CAP_EXT_BIT8	0	//位8.有无灯控同步模式支持
...
#define MCU_PRESET_VAL		5	//设置预置按键数量，最多为10个，为了向前兼容，0表示默认6个预置（不回复默认为0），-1表示没有预置功能。注意n前面有个+，这个指令在正常开机后发送也有效。

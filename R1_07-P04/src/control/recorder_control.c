...
bool RecorderControl(void)
{
	...
	{
#ifdef FUNC_SLAVE_UART_EN
	  Slave_SendCmdToBt(COM_REC_ERR);         //没有录音设备
#endif
...
#ifdef FUNC_SLAVE_UART_EN
	  Slave_SendCmdToBt(COM_REC_OK);         //录音建立成功
#endif
...
//					Msg = MSG_COMMON_CLOSE;
		...
		{
			Mp3OutBufWriteFile(Mp3GetDataLen()); // dump all remained data
			RecoderExit(RECORD_FORMAT);
		}
	}
	else
	{
#ifdef FUNC_SLAVE_UART_EN
	  Slave_SendCmdToBt(COM_REC_ERR);         //录音建立失败，可能是磁盘拔出或者空间不足
#endif
...
{
#define ENCODER_ANA_GAIN_VAL  4096    //录音音量增益
	uint16_t i;
	uint16_t Cnt = 0;
				...
				if((SampleCnt - Cnt) > Temp)
				{
					if(RECORD_CHANNEL == 1)		// 单声道
					{
						for(i = 0; i < Temp; i++)
						{
							...
							MixBuf[TempIndex-1] = (MixBuf[TempIndex-1] * ENCODER_ANA_GAIN_VAL*8) >> 15;
...
//录音播放失败
static void FailLeaveRecPlayBack(void)
{
#ifdef FUNC_SLAVE_UART_EN
	Slave_SendCmdToBt(COM_PA_ERR);         //录音播放失败
#endif
	LeaveRecPlayBack();
}
...
#ifdef FUNC_SLAVE_UART_EN
	Slave_SendCmdToBt(COM_PA_OK);         //录音播放成功
	AudioAnaSetChannel(AUDIO_CH_NONE);
#endif

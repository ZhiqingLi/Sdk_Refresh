...
#endif
uint32_t gCurDecoderType;
bool IsDecoderWithHardDecoder;
...
//	extern const uint16_t gDecVolArr[];
	...
	uint8_t OutputChannelSelect_Bak = gSys.OutputChannelSelect;
#endif
	...
	if(WiFiSlaveSounBoxCntGet())
	{
		if(OutputChannelSelect_Bak == CLASSD_OUTPUT_CHANNEL)
		{
			...
			else if(gSys.CurModuleID == MODULE_ID_SLAVE_BLUETOOTH)
	#else
...
//	DecoderPlayTime = 0;
	...
	else if((DecoderType == MP3_DECODER)
		|| (DecoderType == WMA_DECODER)
		|| (DecoderType == SBC_DECODER))
	{
		IsDecoderWithHardDecoder = TRUE;
	}
	else
	{
		IsDecoderWithHardDecoder = FALSE;
	}
	...
	{
		if(DRC_ERROR_OK != InitDrc(&gSys.AudioDRCEffectHandle, audio_decoder->song_info->num_channels, audio_decoder->song_info->sampling_rate))
		{
    ...
    {
        if(VB_ERROR_OK != vb_init(&gSys.AudioVbEffectHandle, audio_decoder->song_info->num_channels, 
                                            audio_decoder->song_info->sampling_rate, AUDIO_EFFECT_PARAM_VB_CUTOFF_FREQ))
        {
...
static bool SongPlayDo(void)
{
	...
	while(Retry--)
	{
		...
		if(DecoderPlayState == DECODER_PLAY_START_DECODE)			//启动解码一帧
		{
			...
			{
#if defined(FUNC_AUDIO_DRC_EFFECT_EN) || defined(FUNC_AUDIO_VB_EFFECT_EN) || (EQ_STYLE_MODE == 2) || (EQ_STYLE_MODE == 3)
				if(0
#ifdef FUNC_AUDIO_VB_EFFECT_EN
				|| (gSys.AudioVbEffectFlag && (gCurDecoderType != FLAC_DECODER))
#endif
#if (EQ_STYLE_MODE == 2) || (EQ_STYLE_MODE == 3)
				|| (gSys.EqStatus == 1)
#endif
				...
				{
					if(IsDecoderWithHardDecoder == TRUE)
					{
	...
	//		APP_DBG("XR: %d ms, %d\n", PastTimeGet(&DecoderTimer), audio_decoder->song_info->pcm_data_length);
			...
			//音量预增益
			DigitalGainSet((int32_t *)audio_decoder->song_info->pcm0_addr, 
												audio_decoder->song_info->pcm_data_length,
												audio_decoder->song_info->num_channels, 
												DIGITAL_PER_GAIN_VAL);
#endif
#ifdef FUNC_OUTPUT_CHANNEL_AUTO_CHANGE
#ifdef FUNC_WIFI_MULTIROOM_PLAY_EN
			if((gSys.OutputChannelSelect != CLASSD_OUTPUT_CHANNEL)
...
#endif
			if(IsDecoderWithHardDecoder == FALSE)
			{
#ifdef FUNC_AUDIO_VB_EFFECT_EN
				if(gSys.AudioVbEffectFlag)
				{
					AudioVbPro((int16_t *)audio_decoder->song_info->pcm0_addr, audio_decoder->song_info->num_channels, audio_decoder->song_info->sampling_rate, audio_decoder->song_info->pcm_data_length);
				}
#endif
#if (EQ_STYLE_MODE == 2) || (EQ_STYLE_MODE == 3)
				if(gSys.EqStatus == 1)
				{
					AudioSwEQPro(audio_decoder->song_info->pcm0_addr, audio_decoder->song_info->num_channels, audio_decoder->song_info->pcm_data_length);
				}
#endif
#ifdef FUNC_AUDIO_DRC_EFFECT_EN
#ifdef FUNC_SOUND_REMIND
...
#endif
					AudioDRCPro((int16_t *)audio_decoder->song_info->pcm0_addr, audio_decoder->song_info->num_channels, audio_decoder->song_info->sampling_rate, audio_decoder->song_info->pcm_data_length);
#endif
#ifdef AUDIO_EFFECT_SWITCH_FADEOUT_FADEIN
				if(IsFadeOutEn)
				{
					DigitalFadeOut(audio_decoder->song_info->pcm0_addr, audio_decoder->song_info->pcm_data_length, audio_decoder->song_info->num_channels);
				}
				else if(IsFadeInEn)
				{
					DigitalFadeIn(audio_decoder->song_info->pcm0_addr, audio_decoder->song_info->pcm_data_length, audio_decoder->song_info->num_channels);
				}
#endif			
			}
			...
			{
		    SilenceAmpMuteProc((void*)audio_decoder->song_info->pcm0_addr, audio_decoder->song_info->pcm_data_length);
			}
#endif	
...
uint8_t SbcDecoderProcess(void)
{	
	while(1)
	{
        audio_decoder_clear_xr_done();
...
#if defined(FUNC_AUDIO_DRC_EFFECT_EN) || defined(FUNC_AUDIO_VB_EFFECT_EN) || defined(FUNC_AUDIO_3D_EFFECT_EN) || (EQ_STYLE_MODE == 2) || (EQ_STYLE_MODE == 3)
...
#endif
        //20160413  这里解决连续播放提示音蓝牙解码在这里死循环
        while(!audio_decoder_check_xr_done() && IsBtAudioPlay())        
        {
            OSQueueMsgRecv(NULL, NULL, 1);
        }

		MsgClear(MSG_DECODER_XR_DONE);
        audio_decoder_clear_xr_done();
        ...
        while(!MixerIsDone(MIXER_SOURCE_DEC))
        {
            OSQueueMsgRecv(NULL, NULL, 1);
        }
	}
...
void SbcDecoderStart(void)
{
	...
	if(gSys.AudioDRCEffectFlag)
	{
		if(DRC_ERROR_OK != InitDrc(&gSys.AudioDRCEffectHandle, (CurSongInfo->num_channels == 1) ? MIXER_FORMAT_MONO : MIXER_FORMAT_STERO, CurSongInfo->sampling_rate))
        {
    ...
    if(gSys.AudioVbEffectFlag)
    {
        if(VB_ERROR_OK != vb_init(&gSys.AudioVbEffectHandle, (CurSongInfo->num_channels == 1) ? MIXER_FORMAT_MONO : MIXER_FORMAT_STERO, 
                                            CurSongInfo->sampling_rate, AUDIO_EFFECT_PARAM_VB_CUTOFF_FREQ))
        {
            gVbEffectInited = FALSE;
        } 
        else

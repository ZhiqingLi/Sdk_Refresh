...
#ifdef FUNC_WIFI_EN
	|| (gSys.CurModuleID == MODULE_ID_WIFI) || (gSys.CurModuleID == MODULE_ID_PLAYER_WIFI_SD) || (gSys.CurModuleID == MODULE_ID_PLAYER_WIFI_USB)
#ifdef FUNC_WIFI_POWER_KEEP_ON
	|| gWiFi.OtherModuleWiFiAudioEn
#endif
#endif
	)
	//这里因为IISIN信号幅值与解码音量一致，所以IISIN使用解码音量表
	{
...
void CommonMsgProccess(uint16_t Msg)
{
	...
	if(gWiFi.InitState)
	{
		...
		else if((Msg == MSG_WIFI_TALK) && (!gWiFi.MicMuteState || gWiFi.TestModeState))
		{
			if((gSys.CurModuleID == MODULE_ID_WIFI) && (gWiFi.MicState != 1))
			{
				Mcu_SendCmdToWiFi(MCU_TALK_ON, NULL);
			}
			else if(gSys.CurModuleID != MODULE_ID_WIFI)
			{

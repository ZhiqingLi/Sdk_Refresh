...
void SysVarInit(void)
{
	...
	gSys.AudioDRCEffectFlag = 1;
#endif	
	
#ifdef FUNC_AUDIO_3D_EFFECT_EN
	gSys.Audio3dEffectFlag = 1;
#endif
    
#ifdef FUNC_AUDIO_VB_EFFECT_EN
    gSys.AudioVbEffectFlag = 1;
#endif	
...
#ifdef FUNC_WIFI_EN
	gSys.McuSyncWiFiVolume = BP_GET_ELEMENT(pBpSysInfo->McuSyncWiFiVolume);
#ifdef FUNC_WIFI_ALI_PROJECT_EN 
	if(gSys.McuSyncWiFiVolume > 70)
	{
		gSys.McuSyncWiFiVolume = 70;
	}
	else if(gSys.McuSyncWiFiVolume < 35)
	{
		gSys.McuSyncWiFiVolume = 35;
	}
#else
	if(gSys.McuSyncWiFiVolume < 20)
	{
		gSys.McuSyncWiFiVolume = 20;
	}
#endif
	gSys.Volume = (MAX_VOLUME * gSys.McuSyncWiFiVolume) / 100;
#endif

#ifdef POWERON_DEFAULT_VOLUME_EN
	gSys.Volume = DEFAULT_VOLUME;
#ifdef FUNC_WIFI_EN
...
#endif
#ifdef FUNC_WIFI_EN
	pBpSysInfo->McuSyncWiFiVolume = gSys.McuSyncWiFiVolume;
#endif
...
void MsgSend(uint16_t Msg)
{
	...
	if((Msg == MSG_VOL_UP || Msg == MSG_VOL_DW) && IsSoundRemindPlaying())
	{
		return;
	}

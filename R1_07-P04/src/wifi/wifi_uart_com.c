...
void WiFi_SendCmdToMcu(uint8_t WiFiCmd, uint8_t* CmdData)
{
	...
	{		
		...
		case AXX_MCU_RDY:
			...
			Mcu_SendCmdToWiFi(MCU_MMC_GET, NULL);             //查询TF卡是否在线。
#endif
			break;
		...
		case AXX_FACTORY:
			...
			gSys.McuSyncWiFiVolume = (gSys.Volume * 100)/MAX_VOLUME;
			SetSysVol();
		...
		case AXX_BURNING:
			...
			if((gSys.CurModuleID != MODULE_ID_WIFI) && !IsBurningSilentState)
			{
				gSys.NextModuleID = MODULE_ID_WIFI;
				MsgSend(MSG_COMMON_CLOSE);
			}
		...
		case AXX_SLI_OTA:
			IsBurningSilentState = TRUE;
			APP_DBG("Silent Burning start!\n");
			break;

		case AXX_SLI_DON:
			IsBurningSilentState = FALSE;
			APP_DBG("Silent Burning end!\n");
			break;
...
void WiFi_CmdProcess(void)
{
	...
	else if(memcmp(gWiFiCmd, "+SLI+OTA", 8) == 0)
	{
		WiFi_SendCmdToMcu(AXX_SLI_OTA, NULL);		
	}	
	else if(memcmp(gWiFiCmd, "+SLI+DON", 8) == 0)
	{
		WiFi_SendCmdToMcu(AXX_SLI_DON, NULL);		
	}	

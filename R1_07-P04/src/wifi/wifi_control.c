...
{
	static bool MmcIsFirst = FALSE;
	
	if(gWiFi.CardInsertState != State)
	{
		if(State == WIFI_STATUS_SD_PLUGIN)
		{
			if(MmcIsFirst && (gSys.CurModuleID != MODULE_ID_PLAYER_WIFI_SD))
			{
...
{	
	if(Vol >= MAX_VOLUME)
	{
		gSys.McuSyncWiFiVolume = 100;						
	}
	else
	{
		gSys.McuSyncWiFiVolume = (Vol * 100)/MAX_VOLUME;
	}
	Mcu_SendCmdToWiFi(MCU_CUR_VOL, &gSys.McuSyncWiFiVolume);
}
...
void WiFiSetMcuVolume(uint8_t Vol)
{
	if(Vol > 100)
	{
		gSys.McuSyncWiFiVolume = Vol%100;
	}
	else
	{
		gSys.McuSyncWiFiVolume = Vol;
	}
	gSys.Volume = (MAX_VOLUME * gSys.McuSyncWiFiVolume) / 100;
	APP_DBG("WiFiSetMcuVolume: %d!\n", gSys.Volume);
	SetSysVol();
}
...
{
	Mcu_SendCmdToWiFi(MCU_CUR_VOL, &gSys.McuSyncWiFiVolume);
	APP_DBG("WiFi get set mcu Volume: %d!\n", gSys.Volume);
}
...
void WiFiAppSetPlayMode(uint8_t Mode)
{
	...
	|| (Mode == WIFI_PLAY_MODE_SPOTIFY_PLAY) || (WIFI_PLAY_MODE_ALIAPP_PLAY))
	{
...
void WiFiLedCb(void* Param)
{
  static uint32_t  LedBlinkCnt;
	...
	if((gWiFi.VisInfState == TRUE) || (gWiFi.MicState != FALSE) || gWiFi.MicMuteState)
	{
		...
		else
		{
			...
			{
				TimeOutSet(&WiFiLedBlinkTimer, 150);
				if(LedBlinkCnt%2 == TRUE)
				{
					LED_YELLOW_MODE_ON();
				}					
				if((gWiFi.MicMuteState) && (LedBlinkCnt >= 15))
				{

...
#define MVA_BT_OBEX_UPDATE_FUNC_SUPPORT /*online MVA supporting function, double bank-method*/

#ifdef MVA_BT_OBEX_UPDATE_FUNC_SUPPORT

#define FLASH_MVA_UPDATE_START_OFFSET 0x120000  /*MUST be based on the real condition!!!*/
...
#define DUAL_BAND_NVRAM_MAJIC_ADDR  176
void DualBankUpdateReboot(uint32_t FlashUpdateBankAddr)
{
    uint8_t T[4];
    
    T[0] = ( FlashUpdateBankAddr        & 0xFF);
    T[1] = ((FlashUpdateBankAddr >>  8) & 0xFF);
    T[2] = ((FlashUpdateBankAddr >> 16) & 0xFF);
    T[3] = ((FlashUpdateBankAddr >> 24) & 0xFF);
    
    NvmWrite(DUAL_BAND_NVRAM_MAJIC_ADDR, /*&FlashUpdateBankAddr*/T, 4); //write addr for update usage!!!
...
uint8_t MvaCrcCheck(void)
{
    ...
    {
        DBG(("NO MVA file is found.\n"));
        return 0; 
    }
    
    for(i = 0 ; i < MvaFileTotalSize - 4 ; i ++)
    {
        SpiFlashRead(Addr, Tmp, 1);
        Crc16 = CRC16(Tmp,1,Crc16);
        Addr ++;
    }
    
    SpiFlashRead(Addr,   Tmp,   1);
    SpiFlashRead(Addr+1, Tmp+1, 1);
    SpiFlashRead(Addr+2, Tmp+2, 1);
    SpiFlashRead(Addr+3, Tmp+3, 1);
...
void ObexUpdateProc(const ObexCallbackInfo *Info)
{    
    switch(Info->type)
    {
        case BT_OBEX_NAME:        
...
//            DBG(("Info->type = %d",Info->type));
            
            if(ObexStautsFlag == OBEX_STATUS_MVA_FOUND)
            {
                if(MvaFileTotalSize == 0)//first time
                {
                    SpiFlashErase(FLASH_MVA_UPDATE_START_OFFSET, 4096);
                }
                
                if((MvaFileTotalSize & 0xFFFFF000) != ((MvaFileTotalSize+Info->seglen) & 0xFFFFF000) )
                {
                    //erase new flash 4KB
                    SpiFlashErase(FLASH_MVA_UPDATE_START_OFFSET + ((MvaFileTotalSize+Info->seglen) & 0xFFFFF000), 4096);                    
                }
                
                SpiFlashWrite(FLASH_MVA_UPDATE_START_OFFSET + MvaFileTotalSize, Info->segdata, Info->seglen);
            }
            
            MvaFileTotalSize += Info->seglen;
            APP_DBG("Mva file total = %d bytes received (cur block size = %d).\n", MvaFileTotalSize, Info->seglen);
		...
		case BT_OBEX_DISCONNECTED:
            ...
            if(ObexStautsFlag == OBEX_STATUS_MVA_FOUND)//end of mva data transfering, then I should do the CRC and report the result if error found.
            {
                ...
                if(MvaCrcCheck())
                {
                    ...
                    DualBankUpdateReboot(FLASH_MVA_UPDATE_START_OFFSET);
                }
            }
            ObexStautsFlag = OBEX_STATUS_NONE;
#endif        
			APP_DBG("````````BT_OBEX_DISCONNECTED\n");
			break;

...
const uint16_t gDecVolArr[MAX_VOLUME + 1] =
{
	...
	332,	391,	460,	541,	586,	628,	780,	935,
	1088,	1190,	1340,	1420,	1550,	1720,	1850,	2000//最高音量约420mVrms
};
#else
const uint16_t gDecVolArr[MAX_VOLUME + 1] =
{
	0,
	26,		31,		36,		42,		49,		58,		67,		78,
	91,		107,	125,	147,	173,	189,	200,	272,
	349,	401,	580,	701,	786,	868,	925,	1010,
	1180,	1325,	1450,	1570,	1720,	1860,	2050,	2250//最高音量约420mVrms
};
#endif
...
const uint16_t gAnaVolArr[MAX_VOLUME + 1] =
{
	...
	91,		107,	125,	147,	173,	204,	240,	282,
	332,	391,	460,	541,	636,	748,	880,	1035,
	1218,	1433,	1786,	2284,	2834,	3446, 3900,	4150//最高音量约420mVrms
};
...
//CS3110音量表
const uint16_t CS3110VolArr[MAX_VOLUME + 1] =
{
	0,
	0x8080,		0x8383,		0x8686,		0x8989,		0x8C8C,		0x8F8F,		0x9393,		0x9595,
	0x9898,		0x9B9B,	  0x9E9E,	  0xA1A1,	  0xA4A4,	  0xA7A7,	  0xA9A9,   0xACAC,
	0xAEAE,	  0xB1B1,	  0xB3B3,  	0xB5B5,	  0xB7B7,	  0xB9B9, 	0xBBBB,	  0xBEBE,
	0xC1C1,   0xC4C4,	  0xC7C7,	  0xC9C9,	  0xCBCB,	  0xCECE,   0xD1D1,	  0xD5D5//最高音量约420mVrms
};	
#endif
...
#ifdef FUNC_DIGITAL_VOLUME_ADJ_EN
void CS3110SpiWriteOneChar(uint8_t Vol)
{
	uint8_t count = 16;
	uint16_t VolTemp;
	
	VolTemp = CS3110VolArr[Vol];
	APP_DBG("Set Cs3110 vol = %x\n", VolTemp);
	CS3110_SPI_CLK_PORT_LOW();
	CS3110_SPI_CS_PORT_LOW();
	WaitUs(10);
	
	while(count--)
	{
		if((VolTemp & 0x8000) == 0x8000)
		{
			CS3110_SPI_DOUT_PORT_HIGH();
		}
		else
		{
			CS3110_SPI_DOUT_PORT_LOW();
		}
		WaitUs(10);
		CS3110_SPI_CLK_PORT_HIGH();
		WaitUs(20);
		CS3110_SPI_CLK_PORT_LOW();
		WaitUs(10);
		VolTemp <<= 1;
...
void CommonMsgProccess(uint16_t Msg)
{
...
#endif
			{
				McuSyncWiFiVolume(gSys.CS3310Volume);
			}
#endif
#else
			if(gSys.Volume < MAX_VOLUME)
	...
	if(gWiFi.InitState)
	{
		if(Msg == MSG_WIFI_MIC)
		{
			if ((gSys.CurModuleID == MODULE_ID_WIFI) || gWiFi.TestModeState)
			{
		...
		else if(Msg == MSG_WIFI_TALK)
		{
			...
			{
				if((gWiFi.MicState != 1) && (!gWiFi.MicMuteState || gWiFi.TestModeState))
				{

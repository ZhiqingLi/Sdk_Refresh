...
#if MUSIC_SDCARD_EN
    //sd_other_param_set(0);   //BIT(0) = 1 每次读都会sd_stop.   //BIT(1) = 1 每次写都会sd_stop.
    if (SD_IS_SOFT_DETECT()) {
        dev_delay_times(DEV_SDCARD, 3);
    }else if((xcfg_cb.sddet_iosel == IO_MUX_SDCLK) || (xcfg_cb.sddet_iosel == IO_MUX_SDCMD)) {
...
void bsp_sys_init(void)
{
...
#if REC_TO_SPIFALSH
    spiflash_rec_fs_mount();
    //func_cb.mp3_res_play = mp3_res_play;  //播放最后一首录音
	...
	//TMR4CNT = 0;
	//TMR4PR  = 1000000 / 2 - 1;          //500ms, select xosc26_div 1M clk
	//TMR4CON |= BIT(2) | BIT(0);         //Timer works in Counter Mode
    //sys_irq_init(IRQ_TMR245_VECTOR, 1, timer4_isr);
}

...
//    printf("%s: %04x\n", __func__, adc_ch);
    ...
    saradc_setup_time_set(BIT(ADCCH_VBAT),3);  //如通过IO检测电池电压, 当然上下拉分压电阻比较大,如500K, 需要加大采样的建立时间(采样延时)

#if USER_ADKEY
    #if USER_ADKEY_MUX_LED
    saradc_baud_set(0x20);
    ...
    saradc_kick_start(adc_cb.channel);
#if VBAT_DETECT_EN
...
bool get_adc_val(void)
{
    ...
    if (xcfg_cb.user_adkey_mux_sdclk_en || VBAT_DETECT_MUX_SDCLK) {
...
#if VBAT_DETECT_EN && (!VBAT_DETECT_MUX_SDCLK)
    adc_cb.vbat2 = (u8)(adc_cb.sfr[VBAT2_ADCCH] >> 2);
#endif // VBAT_DETECT_EN
#if CHARGE_EN
...
{
    if ((xcfg_cb.user_adkey_mux_sdclk_en || VBAT_DETECT_MUX_SDCLK) && (adc_cb.sdclk_convert)) {
...
void power_on_check(void)
{
    ...
    if (PWRON_FRIST_BAT_EN) {
        if ((!IS_PWRKEY_PRESS()) && (!CHARGE_DC_IN())) {
            return;                                                     //没有按下POWRKEY直接返回开机。
        }
    }
    #endif
    if (!IS_PWRKEY_PRESS()) {
    ...
    while (1) {
        ...
        if ((key_val & K_PWR_MASK) == K_PWR) {
            ...
            if (!sys_cb.poweron_flag) {
                if (tick_check_expire(ticks, PWRON_PRESS_TIME)) { //长按开机时间配置   // (1500+500*xcfg_cb.pwron_press_time)
                    sys_cb.poweron_flag = 1;
                }
            }
        } else {

...
void lock_sleep(void);
void unlock_sleep(void);
...
static void sfunc_sleep(void)
{
    ...
    if ((CLKCON0 & 0x70) != 0x20) {
        //close adpll
        PLL1CON &= ~BIT(13);                //PLL1 SDM Dis
        PLL1CON &= ~BIT(6);                 //PLL1 Analog Dis
        PLL1CON &= ~BIT(12);                //PLL1 LDO DIS
    }
...
#endif
#if SYS_KARAOK_EN
    bsp_karaok_init(AUDIO_PATH_KARAOK, func_cb.last);
#endif
...
{
    //加速掉电。
    CLKGAT1 |= BIT(0);                 //fmamfdt clk enable
    CLKGAT0 |= (BIT(15) | BIT(26));    //FMRX CLK enable
    CLKGAT1 |= (BIT(13) | BIT(14));    //fmlo clk enable
    PWRCON0 = PWRCON0 & ~(0x7 << 4);   // vddio 3.3-->2.5
    FMAMFDTCON |= BIT(16);             // fm det amp1 enable
    FMAMFDTCON |= BIT(17);             // fm det amp2 enable
    delay_5ms(1);
    fmrx_analog_init();                // fm analog enable
...
#if CHARGE_EN
    if (xcfg_cb.charge_en) {
        rtccon3 |= BIT(5);              //VUSB wake up enable(充电唤醒)
    }
#endif

#if !ALARM_WAKEUP_EN
    rtccon3 &= ~BIT(6);                 //RTC alarm wakeup disable
#endif
...
#define CHARGE_FULL_
#define CHARGE_FULL_BLED_ON()          //{GPIOFFEN &= ~BIT(5); GPIOFDE |= BIT(5); GPIOFDIR &= ~BIT(5); GPIOFCLR = BIT(5);}
#define CHARGE_FULL_BLED_OFF()         //{GPIOFDE &= ~BIT(5); GPIOFDIR |= BIT(5); }
...
u8 charge_finish_into_lowpower_check(bool stop_charge)   //程序进入RC低功耗, 如果检测到DC_OUT, 则关机.
{
    ...
    while(1) {
        ...
        if (charge_full_cnt < 60*1000/200) {   //充满电后1分钟内亮绿灯
            CHARGE_FULL_BLED_ON();
        }else {
            CHARGE_FULL_BLED_OFF();
        }
        if (dc_out_cnt >= 2) {
#if  CHARGE_OUT_POWER_ON
            //检测到DC拔出, 则开机.
            if (charge_full_cnt <= CHARGE_FULL_POWER_ON_TIME) {  //充满电后前 CHARGE_FULL_POWER_ON_TIME 时间内拿起可以开机.
                //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
                DEG_N(5);

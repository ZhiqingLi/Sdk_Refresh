...
rec_enc_t rec_enc_cb;// AT(.recbuf.cb);
u8 rec_obuf[REC_OBUF_SIZE] AT(.rec.obuf);
u8 rec_sco_obuf[REC_OBUF_SIZE] AT(.rec.sco);
...
u8 wav_header_buf[512] AT(.rec.wave.header);
...
const char dir_spf_sfn[8]  = {0x53, 0x50, 0x46, 0xC2, 0xBC, 0xD2, 0xF4, '\0'};

const char dir_fm_sfn_exfat[5]  = {0x46, 0x4D, 0x55, 0xF3, '\0'};
const char dir_aux_sfn_exfat[6] = {0x41, 0x55, 0x58, 0x55, 0xF3, '\0'};
const char dir_mic_sfn_exfat[6] = {0x4D, 0x49, 0x43, 0x55, 0xF3, '\0'};
const char dir_bt_sfn_exfat[5]  = {0x42, 0x54, 0x55, 0xF3, '\0'};
const char dir_spf_sfn_exfat[6] = {0x53, 0x50, 0x46, 0x55, 0xF3, '\0'};
...
const char dir_spf_lfn[] = {
    0x05, 0x00,         //unicode characters counter
    0x53, 0x00, 0x50, 0x00, 0x46, 0x00, 0x55, 0x5F, 0xF3, 0x97,     //"SPF录音" unicode
};
...
const char dir_path_spf[8]  = "RECORDS";
#endif // REC_DIR_LFN
...
const char file_name_spf[]  = "SPF0001.WAV";
...
const char file_name_spf[]  = "SPF0001.mp3";
...
bool sfunc_rec_mkdir(void)
{
    ...
    if (func_cb.sta == FUNC_FMRX) {
        ...
        lfn = dir_bt_lfn;
    } else if (func_cb.sta == FUNC_SPDIF) {
        //”SPF录音“ 目录
        if (fs_type == FS_EXFAT) {
            memcpy(&fname_buf[len], dir_spf_sfn_exfat, sizeof(dir_spf_sfn_exfat));
            len += sizeof(dir_spf_sfn_exfat);
        } else {
            memcpy(&fname_buf[len], dir_spf_sfn, sizeof(dir_spf_sfn));
            len += sizeof(dir_spf_sfn);
        }
        lfn = dir_spf_lfn;
    } else {
        //”MIC录音“ 目录
...
bool sfunc_rec_create_file(void)
{
    ...
    if (func_cb.sta == FUNC_FMRX) {
        ...
        strcpy((char *)&fname_buf[pos], file_name_spf);
        pos += SPF_SFN_NUM_OFS;
    } else {
...
{
#if FUNC_REC_EN
    u8 *rbuf = &mp3_bitpool[512];           //需要支持DMA的RAM
...
{
    u8 *wbuf = mp3_bitpool;             //复用mp3_bitpool前512字节
...
{
    u8 *rbuf = &mp3_bitpool[0];               //复用buf,需要支持DMA RAM
...
{
    if ((sys_cb.cur_dev == DEV_SDCARD) && dev_is_online(DEV_UDISK)) {
        sys_cb.cur_dev = DEV_UDISK;
        return true;
    } else if ((sys_cb.cur_dev == DEV_UDISK) && dev_is_online(DEV_SDCARD)) {
...
bool sfunc_record_start(void)
{
    ...
    do {
        ...
        //16000Hz采样率，单声道数据源压缩为msbc格式
        if ((rec->src->spr == SPR_16000) && ((rec->src->nchannel & 0x03) == 1)) {
            if (!msbc_encode_init()) {
                rec->src = 0;
                return false;
            }
        ...
        bsp_karaok_rec_init();                              //要在初始化MP3之前reset karaok
...
#if SYS_KARAOK_EN && KARAOK_REC_EN
    bsp_karaok_rec_exit();
    sys_cb.music_vol = sys_cb.rec_msc_vol;  //还原音乐音量
    bsp_karaok_set_music_volume();
#endif
    led_idle();

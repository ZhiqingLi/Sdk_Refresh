...
typedef struct _BP_RTC_INFO_
{
	...
	uint8_t			RingType[MAX_ALARM_NUM]; 		// 铃声类型 INTER_RING_TYPE - 内置铃声、USB_RING_TYPE - U盘铃声、SD_RING_TYPE - SD卡铃声
	uint8_t			AlarmMode[MAX_ALARM_NUM]; 		// 闹钟模式
	uint8_t			AlarmState[MAX_ALARM_NUM]; 		// 闹钟状态
	RTC_DATE_TIME	AlarmTime[MAX_ALARM_NUM];		// 闹钟时间信息
} BP_RTC_INFO;
#endif

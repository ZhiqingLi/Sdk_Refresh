...
//OTG 使用软件循环fifo 大小为2k，如果想节省该mem可以复用到其他mem，
//比如I2S，前提是客户明确不会使用I2SIN功能
MCU_CIRCULAR_CONTEXT OTGCircularBuf = {0};
int16_t OTG_DeviceRxBuf[1024];//2k Byte buf
#define OTG_AUDIO_RX_BUF_SIZE   sizeof(OTG_DeviceRxBuf)
//#define OTG_DeviceRxBuf I2SIN_FIFO_ADDR 
//#define OTG_AUDIO_RX_BUF_SIZE 2048
...
void OnDeviceAudioRcvIsoPacket(void)
{
	...
	if(UsbAudioPlayFlag)
	{	
		if(PacketLen >= MCUCircular_GetSpaceLen(&OTGCircularBuf))
		{
			PacketLen = MCUCircular_GetSpaceLen(&OTGCircularBuf) - 4;
		}
		MCUCircular_PutData(&OTGCircularBuf, TempBuf, PacketLen);
	}
}
...
bool UsbAudioControl(void)
{
	...
	MCUCircular_Config(&OTGCircularBuf, (void*)OTG_DeviceRxBuf, OTG_AUDIO_RX_BUF_SIZE);
	TimeOutSet(&UsbAudioFreqAdjustTimer, 1000);    
...
//	NVIC_EnableIRQ(DECODER_IRQn);
	...
	{
		if(DRC_ERROR_OK != InitDrc(&gSys.AudioDRCEffectHandle, MIXER_FORMAT_STERO, USB_DAC_FREQ))
        {
	...
	{
		if(THREE_D_ERROR_OK != init_3d(&gSys.Audio3dEffectHandle, MIXER_FORMAT_STERO, USB_DAC_FREQ))
        {
    ...
    {
        if(VB_ERROR_OK != vb_init(&gSys.AudioVbEffectHandle, MIXER_FORMAT_STERO, USB_DAC_FREQ, AUDIO_EFFECT_PARAM_VB_CUTOFF_FREQ))
        {
...
/**
 * @brief  HID发送数据，由PC发起,Mvchip发送到PC
 * @param  pBuffer 发送数据Buffer指针
 * @param  允许发送数据长度 小于等于64.
 * @return 
 */
...
/**
 * @brief  HID接收数据，由PC发起的,PC发送到MVChip
 * @param  pBuffer 接收数据Buffer指针
 * @return 
 */

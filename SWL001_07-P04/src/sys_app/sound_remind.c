...
//   ID                                 中文ID                			英文ID
	...
	{SOUND_UPDATE_FAIL,					STR_TO_UINT("GXSB"),	STR_TO_UINT("UPFR")},	//更新失败
...
#endif
	//wifi语音中禁止播放提示音。
	if(!gSys.SoundRemindOn || (WiFiFirmwareUpgradeStateGet() == TRUE)
	|| ((gWiFi.MicState != WIFI_AVS_STATUS_DIS) && (gWiFi.MicState != WIFI_AVS_STATUS_IDLE)))
	{
		return;
	}
#ifdef FUNC_RECORD_EN

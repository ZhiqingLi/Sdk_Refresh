...
void CommonMsgProccess(uint16_t Msg)
{
	...
	{	
		...
		case MSG_LIGHT_PWM_SET:
			PwmLightDisplayControlSetting();
			break;

		case MSG_LIGHT_SET_UP:
		case MSG_LIGHT_SET_DW:
			PwmLightBrightnessSetting(Msg);
			break;
#endif
...
void SpecTrumProc(int16_t* Buffer, uint32_t Len)
{
	...
	for(i = 0; i < Len; i += 8)
	{
		...
		//TempSampleData = abs(TempSampleData);
		//APP_DBG("SampleData = %d;\n", TempSampleData);
		if(TempSampleData < 1000)
		{
		    AudioSpecTrumStep = 0;	
		}
		else if(TempSampleData < 2000)
		{
			AudioSpecTrumStep = 1;	
		}
		else if(TempSampleData < 3500)
		{
			AudioSpecTrumStep = 2;
		}
		else if(TempSampleData < 4900)
		{
			AudioSpecTrumStep = 3;
		}
		else if(TempSampleData < 6000)
		{
			AudioSpecTrumStep = 4;
		}
		else if(TempSampleData < 6900)
		{
			AudioSpecTrumStep = 5;
		}
		else if(TempSampleData < 8800)
		{
			AudioSpecTrumStep = 6;	
		}
		else if(TempSampleData < 10054)
		{
			AudioSpecTrumStep = 7;
		}
		else if(TempSampleData < 11400)
		{
			AudioSpecTrumStep = 8;
		}
		else if(TempSampleData < 12836)
		{
			AudioSpecTrumStep = 9;
		}
		else
		{
			AudioSpecTrumStep = 10;
		}
 	}		
}
#endif

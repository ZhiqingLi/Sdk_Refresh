...
//	}
	...
	if(!ret)
	{
		...
		memset((void*)&bp_info.RtcInfo, 0, sizeof(NVM_RTC_INFO));
#endif
...
#if (defined(FUNC_RTC_EN) && defined(FUNC_RTC_ALARM_SAVE2FLASH))
		case BP_RTC_INFO_TYPE:
			pInfo = (void *)&(bp_info.RtcInfo);
			break;
#endif
...
bool BP_SaveInfo(BP_SAVE_TYPE SaveType)
{
	...
	APP_DBG("BP_SaveInfo : gSys.Volume = %d\n", bp_info.SysInfo.Volume);
...
static bool SaveBakBPInfo(uint32_t BakNvmAddr, uint32_t NvmBakId, uint8_t SubBakId)	//SaveBakNVM
{
	...
	if(SubBakId == 0)
	{
		...
		if(buf[0] !=0xFF ||buf[1] !=0xFF ||buf[2] !=0xFF ||buf[3] !=0xFF)
		{
			...
			SpiFlashEraseInTcm(BakNvmAddr, NVM_BLOCK_SIZE);
#else
...
static bool LoadBPInfoFromFlash(void)	
{
	...
	if(LoadNvmBakId == 0) // 从来没有保存过，直接返回
	{
		...
		{
#ifdef FLASH_ERASE_IN_TCM
			SpiFlashEraseInTcm(NVM_BAK_ADDR0, NVM_BAK_CNT * NVM_BLOCK_SIZE);
#else
...
#define DWNLDR_USRDAT_ADDR      (0x90)
	...
	NVM_BAK_ADDR0 += BtDataAddr;
#ifdef FLASH_ERASE_IN_TCM
	SpiFlashEraseInTcm(NVM_BAK_ADDR0, 16 * 1024);
#else

...
static const uint8_t Mcu_Brand[] = {MCU_PTV_NUM};   //Device SSID  name
...
void WiFi_SendCmdToMcu(uint8_t WiFiCmd, uint8_t* CmdData);
...
void Mcu_RcvWiFiDataProcess(void)
{
	...
	{
		WiFi_SendCmdToMcu(AXX_UID_UUID, NULL);
	}
...
{	
	...
	if(WiFiPowerOnInitStateGet() == FALSE)
	{
		return;
	}
	...
	switch(McuCmd)
	{
		...
		case MCU_PTV_XXX:
		  ...
		  Temp = 8;
		  memcpy((void*)&CmdBuf[Temp], Mcu_Brand, sizeof(Mcu_Brand));
		  Len = 11;
			break;
...
{
	if((WiFiFirmwareUpgradeStateGet() != 0) || gSys.CurModuleID != MODULE_ID_WIFI)    //升级中禁止播放提示音
	{
		return;
	}
	switch(SoundID)
	{
		case SOUND_WIFI_DONE:
		  SoundRemind(SOUND_WIFI_DONE);          //系统启动完成
			break;
		
		case SOUND_REST_FACTROY:
		  SoundRemind(SOUND_REST_FACTROY);       //恢复出厂设置
			break;
		
		case SOUND_WIFI_CONNECT:
		  SoundRemind(SOUND_WIFI_CONNECT);       //网络连接成功
			break;
		
		case SOUND_WIFI_CONNECTING:
			if(gWiFi.WPSState == WIFI_STATUS_WPS_SCANNING)
			{
		    SoundRemind(SOUND_WIFI_CONNECTING);    //网络连接中
			}
			break;
		
		case SOUND_WIFI_FAIL:
		  SoundRemind(SOUND_WIFI_FAIL);          //网络连接失败
			break;
		
		case SOUND_WIFI_WPS:
			SoundRemind(SOUND_WIFI_WPS);          //网络配置模式启动
		  break;
			
		case SOUND_SYNC_PLAYER:
		  SoundRemind(SOUND_SYNC_PLAYER);
		  break;
		
		case SOUND_ALONE_PLAYER:
		  SoundRemind(SOUND_ALONE_PLAYER);
		  break;
			
    case SOUND_LEFT_PLAYER:
		  SoundRemind(SOUND_LEFT_PLAYER);
		  break;

    case SOUND_RIGHT_PLAYER:
		  SoundRemind(SOUND_RIGHT_PLAYER);
		  break;

    case SOUND_STEREO_PLAYER:
		  SoundRemind(SOUND_STEREO_PLAYER);
		  break;
		
		case SOUND_WIFI_START:
			SoundRemind(SOUND_WIFI_START);
		  break;
		
		case SOUND_WIFI_MODE:
...
void WiFi_SendCmdToMcu(uint8_t WiFiCmd, uint8_t* CmdData)
{
	...
	{		
		...
		case AXX_BOT_DON:
			memset(&gWiFi, 0, sizeof(gWiFi));                     //升级重启后，初始化所有参数，为了显示LED灯
			WiFiWorkStateSet(WIFI_STATUS_INIT_END);
		  WiFiFirmwareUpgradeStateSet(FALSE);
		  WiFi_SoundRemind(SOUND_WIFI_DONE);
			break;
		...
		case AXX_FACTORY:
		  ...
		  WiFi_SoundRemind(SOUND_REST_FACTROY);
			break;
		...
		case AXX_STA_XXX:
			...
			else if(Temp == 0x02)
			{
				if(gWiFi.WPSState == WIFI_STATUS_WPS_SCANNING)
				{
					WiFi_SoundRemind(SOUND_WIFI_CONNECTING);  //提示网络连接中
      ...
      else
			{
				...
				if(gWiFi.WPSState == WIFI_STATUS_WPS_SCANNING)
			  {
			    ...
			    WiFi_SoundRemind(SOUND_WIFI_FAIL);        //提示网络连接失败		
          WiFiWpsStateSet(WIFI_STATUS_WPS_SCAN_STOP);					
			  }
			}			
			break;
		...
		case AXX_WWW_XXX:
			...
			if(Temp == 0x01)
			{
				...
				{
					WiFi_SoundRemind(SOUND_WIFI_CONNECT);     //提示网络连接成功
					WiFiStationStateSet(WIFI_STATUS_STATION_CONNECTED);
				}
      ...
      if(CmdData[2] == '2')
		  {
			  WiFi_SoundRemind(SOUND_RIGHT_PLAYER);
		  }
		  else if(CmdData[2] == '1')
		  {
			  WiFi_SoundRemind(SOUND_LEFT_PLAYER);
		  }
		  else if(CmdData[2] == '0')
		  {
			  WiFi_SoundRemind(SOUND_STEREO_PLAYER);
		  }
			break;
		...
		case AXX_SLV_NOT:
			if(WiFiMutliRoomStateGet() > 0)
			{
				//从机异步播放
				WiFi_SoundRemind(SOUND_ALONE_PLAYER);
			}
		...
		case AXX_SLV_YES:
			...
			{
				//从机同步播放
				WiFi_SoundRemind(SOUND_SYNC_PLAYER);
			}
		...
		case AXX_SLV_XXX:	
			Temp = (CmdData[0] - 0x30) * 100 + (CmdData[1] - 0x30) * 10 + (CmdData[2] - 0x30);
		  if((WiFiSlaveSounBoxCntGet() > 0) && (Temp == 0))     //主机时提示同步、异步播放
		  {
			  WiFi_SoundRemind(SOUND_ALONE_PLAYER);
		  }
		  else if((WiFiSlaveSounBoxCntGet() == 0) && (Temp > 0))
		  {
			  WiFi_SoundRemind(SOUND_SYNC_PLAYER);
		  }

...
#endif
#define WIFI_LED_CB_TIME		100	 //设置WiFiLedCb()函数调用时间间隔(单位:ms)
...
bool WiFiPowerOffRequestFlag = FALSE;
...
uint8_t BatPowerPercentBak = 0;
extern bool ChargeStartFlag;
extern bool ChargeStopFlag;
extern bool BatLowerPowerFlag;
extern uint8_t BatPowerPercent;
#endif
#endif

#ifdef	FUNC_WIFI_UART_UPGRADE
#define UPGRADE_CODE_BANK_ADDR	0x200000	//升级代码在Spi Flash中存放的起始地址
...
void WiFiRequestMcuPowerOff(void)
{
	...
	{		
		...
		else if(PowerOffCnt != 2)
		{
			APP_DBG("McuRequestWiFiPowerOff.......\n");
			PowerOffCnt = 1;
			WiFiPowerOffRequestFlag = TRUE;
		  TimeOutSet(&WiFiPowerOffTimer, 8000);	
		  Mcu_SendCmdToWiFi(MCU_POW_OFF, NULL);
		}
		else
		{
			APP_DBG("McuWillPowerOff.......\n");
			WiFiPowerOffRequestFlag = FALSE;
...
void WiFiSoundRemindStateSet(uint16_t State)
{
	...
	if(gWiFi.WiFiSoundRemindState == 0)
	{
		//提示音使用固定音量
		MixerConfigVolume(MIXER_SOURCE_DEC, gDecVolArr[WIFI_SOUND_REMIND_VOL], gDecVolArr[WIFI_SOUND_REMIND_VOL]);
	}
	else
	{	
		TempVol = gSys.Volume;
...
{
	return !gWiFi.WiFiSoundRemindState;
}
...
void WiFiNoticeMcuNextAlarmTime(uint8_t* SecondData)
{
	...
	{	
		...
		{
			case 5:
				TimeSecond |= (SecondData[4] - 0x30) * 10000;
				
			case 4:
				TimeSecond |= (SecondData[3] - 0x30) * 1000;

			case 3:
				TimeSecond |= (SecondData[2] - 0x30) * 100;

			case 2:
				TimeSecond |= (SecondData[1] - 0x30) * 10;

			case 1:
				TimeSecond |= SecondData[0] - 0x30;
				break;
		}
		...
		RtcGetAlarmTime(sRtcControl->AlarmNum, &sRtcControl->AlarmMode, &sRtcControl->AlarmData, &AlarmTime);
		...
		//计算闹钟时间
		AlarmTime.Hour = (TimeSecond / 3600) + sRtcControl->DataTime.Hour;
		if(AlarmTime.Hour >= 24)
		{
			AlarmTime.Hour -= 24;
		}
		AlarmTime.Min = ((TimeSecond - (AlarmTime.Hour * 3600)) / 60) + sRtcControl->DataTime.Min;
		if(AlarmTime.Min >= 60)
		{
			AlarmTime.Hour += 1;
			AlarmTime.Min -= 60;
...
{
#define WIFI_STATE_CHECK_TIME	10000	//WiFi状态查询时间间隔(10s)
	...
	|| (gWiFi.WWWState == WIFI_STATUS_WWW_ENABLE) || (WiFiDataRcvStartFlag == TRUE)))
	{
		return;
	}
	
	if(WiFiPowerOffRequestFlag && (IsTimeOut(&WiFiPowerOffTimer)))
	{
...
#ifdef FUNC_POWER_MONITOR_EN
#ifdef FUNC_WIFI_BATTERY_POWER_CHECK_EN
			if(ChargeStartFlag == TRUE)
			{
				ChargeStartFlag = FALSE;
				Mcu_SendCmdToWiFi(MCU_BAT_ON, NULL);
				WaitMs(10);
			}
			else if(ChargeStopFlag == TRUE)
			{
				ChargeStopFlag = FALSE;
				Mcu_SendCmdToWiFi(MCU_BAT_OFF, NULL);
				WaitMs(10);
			}
			if(BatLowerPowerFlag == TRUE)
			{
//				Mcu_SendCmdToWiFi(MCU_BAT_LOW, NULL);
//				WaitMs(10);
			}

			if(BatPowerPercentBak != BatPowerPercent)
			{
				BatPowerPercentBak = BatPowerPercent;
				Mcu_SendCmdToWiFi(MCU_BAT_VAL, &BatPowerPercent);
				WaitMs(10);
			}
#endif
#endif
...
void WiFiLedCb(void* Param)
{
 static uint32_t  LedBlinkCnt;
	...
	LedBlinkCnt++;
	if((WiFiFirmwareUpgradeStateGet() == TRUE) || (WiFiFactoryStateGet() == TRUE) || (gWiFi.TestModeState == TRUE))
	{
		LED_ALL_MODE_OFF();
	...
	if(gWiFi.PlayState == TRUE)
	{
		TimeOutSet(&WiFiLedBlinkTimer, 800);
		if(LedBlinkCnt >= 2)
		{
		  LED_WHITE_MODE_ON();
		  LedBlinkCnt = 0;
	...
	{
		if((WiFiWwwStateGet() == WIFI_STATUS_WWW_ENABLE) || (WiFiRa0StateGet() == WIFI_STATUS_AP_CONNECTED))
		{
			LED_BLUE_MODE_ON();
		}
		else 
		{	
			TimeOutSet(&WiFiLedBlinkTimer, 300);
			if(LedBlinkCnt >= 2)
			{
				LED_BLUE_MODE_ON();
		    LedBlinkCnt = 0;
...
uint32_t WiFiControl(void)
{
	...
	WiFi_SoundRemind(SOUND_WIFI_MODE);
...
#ifdef FUNC_WIFI_POWER_KEEP_ON
	WiFiMicOn();
	if((FirstPowerOnFlag == FALSE) && (WiFiPowerOnInitEndFlag == TRUE))
	{
#ifdef FUNC_WIFI_TALK_AND_AUDIO_EFFECT_EN
  ...
  WiFiMicOn();	
	...
	while(Msg != MSG_COMMON_CLOSE)
	{
		...
		else if(WiFiPowerOnInitEndFlag && (WiFiPowerOnInitTimeCnt < 1))
		{
			if((gWiFi.StationState == WIFI_STATUS_STATION_DISCONNECTED) && !FirstPowerOnFlag)
			{
			  WiFi_SoundRemind(SOUND_WIFI_FAIL);        //提示网络回连接失败
		    WiFiStationStateSet(WIFI_STATUS_STATION_DISCONNECTED);
				FirstPowerOnFlag = TRUE;
			}
		}
#endif
		...
		switch(Msg)
		{	
			case MSG_WIFI_POWEROFF:  //WiFi模式只有收到这个消息才会退出，
			...
			case MSG_WIFI_WPS:
				if(gSys.CurModuleID == MODULE_ID_WIFI)
				{
				  WiFiWorkStateSet(WIFI_STATUS_STATION_DISCONNECTED);
					if(gWiFi.WPSState != WIFI_STATUS_WPS_SCANNING || gWiFi.WWWState == WIFI_STATUS_WWW_ENABLE)
					{
						//Mcu_SendCmdToWiFi(MCU_STA_DEL, NULL);                //清除当前记录的路由信息
						Mcu_SendCmdToWiFi(MCU_STA_OFF, NULL);
					  Mcu_SendCmdToWiFi(MCU_WIF_WPS, NULL);
					}
					else
					{
						//重复按WPS键，则播放提示音
						WiFi_SoundRemind(SOUND_WIFI_WPS);  //提示进入网络配置模式
					}
				}
				break;

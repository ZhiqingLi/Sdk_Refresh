...
const uint16_t gDecVolArr[MAX_VOLUME + 1] =
{
	...
	332,	391,	460,	541,	586,	628,	680,	735,
	788,	890,	940,	1080,	1250,	1300,	1400,	1510//最高音量约800mVrms
};

//IIS_IN音量表
const uint16_t gI2sinVolArr[MAX_VOLUME + 1] =
{
	0,
	26,		31,		36,		42,		49,		58,		67,		78,
	91,		107,	125,	147,	173,	189,	200,	222,
	249,	301,	380,	401,	486,	568,	625,	710,
	780,	895,	930,	1070,	1220,	1360,	1500,	1600//最高音量约800mVrms
};
...
const uint16_t gAnaVolArr[MAX_VOLUME + 1] =
{
	...
	91,		107,	125,	147,	173,	204,	240,	282,
	332,	391,	460,	541,	636,	748,	880,	1035,
	1218,	1433,	1786,	2284,	2834,	3446, 3900,	4300//最高音量约800mVrms
};
...
void SetSysVol(void)
{
	...
	{
		MixerConfigVolume(MIXER_SOURCE_ANA_MONO, gI2sinVolArr[gSys.Volume], gI2sinVolArr[gSys.Volume]);
	  MixerConfigVolume(MIXER_SOURCE_ANA_STERO, gI2sinVolArr[gSys.Volume], gI2sinVolArr[gSys.Volume]);
	}
	else
	{
...
#if defined(FUNC_RADIO_EN) || defined(FUNC_SLAVE_BLUETOOTH_EN)
	//20170413,需要C11,C12做模拟输入时，才初始化为模拟输入，防止做标准GPIO时异常
...
void SilenceAmpMuteProc(int16_t* Buffer, uint32_t Len)
{
	...
	for(i = 0; i < Len; i += Temp)
	{
		...
		if(
		   ...
		   (gSys.CurModuleID == MODULE_ID_SLAVE_BLUETOOTH) && (abs(TempSampleData) < ANA_SILENCE_MUTE_VALUE)
		#else
		   0
		#endif
		   || (gSys.CurModuleID == MODULE_ID_LINEIN) && (abs(TempSampleData) < ANA_SILENCE_MUTE_VALUE)
			 || (gSys.CurModuleID != MODULE_ID_LINEIN) && (abs(TempSampleData) < DEC_SILENCE_MUTE_VALUE))           //计算音频数据的绝对值
		{
  ...
  DataLenCnt += Len;
	...
	if(IsTimeOut(&MuteSilenceDetTmr))
	{
	  ...
	  else if(((MuteCnt*100)/DataLenCnt) < SILENCE_MUTE_DET_CNT)
	  {
			SysMuteFlag = FALSE;
    }
		MuteCnt = 0;
...
int32_t InitDrc(DRCContext *ct, int32_t num_channels, int32_t sample_rate)
{
	...
	DRC_CROSSOVER_FILTER_TYPE filter_type = AUDIO_EFFECT_PARAM_DRC_CROSSOVER_FTYPE;
	...
	return init_drc(ct, num_channels, sample_rate, fc, filter_type, q, threshold, ratio, attack_tc, release_tc);
}
...
void AudioDRCPro(int16_t *PcmDataAddr, int32_t CurNumChannel, int32_t CurSampleRate, int32_t PcmDataLen)
{
	...
	if(gSys.AudioDRCEffectFlag)
	{
		...
		{			
            ...
            DigitalGainSet(PcmDataAddr, PcmDataAddr, CurNumChannel, pregain);//此处代码为了向P1兼容，DRC前有一个增益控制
#endif
			apply_drc(&gSys.AudioDRCEffectHandle, PcmDataAddr, PcmDataAddr, PcmDataLen, AUDIO_EFFECT_PARAM_DRC_PREGAIN1, AUDIO_EFFECT_PARAM_DRC_PREGAIN2);		
		}
	}
}
#endif
...
void AudioVbPro(int16_t *PcmDataAddr, int32_t CurNumChannel, uint32_t CurSamplerate, int32_t PcmDataLen)
{
    ...
    if(gSys.AudioVbEffectFlag)
    {
        ...
        apply_vb(&gSys.AudioVbEffectHandle,PcmAddr,  PcmAddr, PcmDataLen, AUDIO_EFFECT_PARAM_VB_INTENSITY);
    }    
}
#endif

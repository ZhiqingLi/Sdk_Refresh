...
int16_t AduioBuf_MIC_Bypass[MIC_BUF_LEN];		//MIC通路原始数据缓存buf
#endif
#endif
...
void AudioProcessTaskEntrance(void)
{
	...
	echo_init(&ct, 1, DacAdcSampleRateGet(), AUDIO_EFFECT_PARAM_ECHO_CUTOFF_FREQ, ECHO_DELAY_SAMPLE, echo_sbuf);
#endif
	...
	while(1)
	{
		...
		if(gSys.MicEnable)	//如果当前有MIC，则MIC占用一路ADC，LINEIN、GUITAR、FM占用另一路ADC
		{
			...
			&& (gSys.CurModuleID != MODULE_ID_PLAYER_WIFI_SD) && (gSys.CurModuleID != MODULE_ID_PLAYER_WIFI_USB))
			{
				...
				echo_apply(&ct, (void*)AduioBuf, (void*)AduioBuf, SampleCnt, EchoDepthStep * DEPTH_ONE_STEP, EchoDelayStep * DELAY_ONE_STEP * DacAdcSampleRateGet() / 44100, AUDIO_EFFECT_PARAM_ECHO_DIRECT_SND_EN); 
...
#if defined(FUNC_AUDIO_DRC_EFFECT_EN) || defined(FUNC_AUDIO_3D_EFFECT_EN) || defined(FUNC_AUDIO_3D_EFFECT_EN)||(EQ_STYLE_MODE == 2) || (EQ_STYLE_MODE == 3)
                if(((gSys.CurModuleID == MODULE_ID_LINEIN) 
					|| (gSys.CurModuleID == MODULE_ID_RADIO) 
					|| (gSys.CurModuleID == MODULE_ID_RECORDER)

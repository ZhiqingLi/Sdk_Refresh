...
extern bool SlaveBluetoothControl(void);
#endif
...
extern int32_t gErrNum; //驱动层关于快速响应的一个错误标志
...
void GuiTaskEntrance(void)
{
...
#ifndef FUNC_GPIO_SOFT_POWERKEY_EN
...
#ifdef FUNC_DU361_EN
#ifdef FUNC_USE_AP_DU_UART_PROTOCOL_EN
  ApduUartProtocolInit();
#endif
	DU361_init();
#endif	
#ifdef FUNC_BT_EN
  ...
  STA33X_init();
	...
	while(1)
	{
		...
		gErrNum = -1;    						            //进入模式前，对全局错误号清零，该标志目前仅用于快速响应
...
//					SysClkDivison(1);	//ljg, call directly
		...
		if((gSys.NextModuleID == MODULE_ID_POWEROFF) || (gSys.NextModuleID == MODULE_ID_IDLE))
		{
			SoundRemind(SOUND_PWR_OFF);        //播放关机提示音
		}
#ifdef FUNC_GPIO_SOFT_POWERKEY_EN
		else if((gSys.CurModuleID == MODULE_ID_IDLE) && (gSys.NextModuleID != MODULE_ID_IDLE))
		{
			SoundRemind(SOUND_PWR_ON);
		}
#endif
		...
		if((gSys.CurModuleID != MODULE_ID_POWEROFF) 
		  ...
		  && (gSys.CurModuleID != MODULE_ID_END)

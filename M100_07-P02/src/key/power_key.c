...
static const uint16_t PowerKeyEvent[2] = {MSG_NONE/*按键开始*/, MSG_PLAY_PAUSE/*MSG_PWR_KEY_SP短按松开*/};	
...
#elif defined(USE_POWERKEY_SLIDE_SWITCH)

	#warning	"GPIO PUSH BUTTON POWER KEY ENABLE!"
...
// Initialize POWER_KEY scan operation.
void GpioPowerKeyScanInit(void)
{
	DBG("GpioPowerKeyScanInit*******\n");
	GpioSetRegOneBit(POWERKEY_DETECT_PORT_IE, POWERKEY_DETECT_BIT);
	GpioClrRegOneBit(POWERKEY_DETECT_PORT_OE, POWERKEY_DETECT_BIT);
	GpioClrRegOneBit(POWERKEY_DETECT_PORT_PU, POWERKEY_DETECT_BIT);
	GpioClrRegOneBit(POWERKEY_DETECT_PORT_PD, POWERKEY_DETECT_BIT);
	GpioClrRegOneBit(POWERKEY_DETECT_PORT_DS, POWERKEY_DETECT_BIT);
	PowerKeyState = POWER_KEY_STATE_IDLE;
...
#ifdef FUNC_GPIO_POWER_KEY_EN
// POWER_KEY与普通的按键不同，连接按钮开关（软开关）时的主要作用还是系统开关机，当然，也允许复用短按功能。
// 短按产生时，推送短按消息。超过短按区间，此处不做任何处理，由系统硬件检测关机。
uint16_t GpioPowerKeyScan(void)							
{
	if(PowerKeyInitFlag)  
	//防止开机时长按POWER键产生短按消息，这里系统启动后，只有检测到按键松开后，才会开始检测短按消息
	{
	...
	switch(PowerKeyState)
	{
		...
		case POWER_KEY_STATE_CP:
			...
			{
				PowerKeyState = POWER_KEY_STATE_IDLE;
				return MSG_NONE;//PowerKeyEvent[1];			
			}
			else
			{
				//do no thing

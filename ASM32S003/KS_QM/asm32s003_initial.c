...
//EXI0_INT= EXI0,EXI1_INT= EXI1, EXI2_INT=EXI2~EXI3, EXI3_INT=EXI4~EXI8, EXI4_INT=EXI9~EXI13    
    ...
    GPIOA0_EXI_Init(EXI5);                                   	 //PA0.5 as input
    EXTI_trigger_CMD(ENABLE,EXI_PIN5,_EXIFT);                  	//ENABLE falling edge
	EXTI_trigger_CMD(ENABLE,EXI_PIN5,_EXIRT);                  	//ENABLE rising edge
    EXTI_interrupt_CMD(ENABLE,EXI_PIN5);                	   	//enable EXI
    GPIO_EXTI_interrupt(GPIOA0,0b00000000010000);				  //enable GPIOA05 as EXI
...
void GTC0_CONFIG(void)
{
    ...
    GTC_Configure(GTC0 , GTC_FIN_PCK , 1 , 9 , Counter_Size_16BIT  , 100 , 1);   //TCCLK=sysclock/2^1/10，20m-->1us
...
void GTC1_CONFIG(void)
{
    ...
    GTC_Configure(GTC1 , GTC_FIN_PCK , 1 , 9 , Counter_Size_16BIT  , 100 , 50);   //TCCLK=sysclock/2^1/10，20m-->1us
...
void GTC2_CONFIG(void)
{
    ...
    GTC_Configure(GTC2 , GTC_FIN_PCK , 0 , 9 , Counter_Size_16BIT  , 1000 , 300);   //TCCLK=sysclock/2^0/10，TCCLK=0.5US
...
void GTC3_CONFIG(void)
{
    ...
    GTC_Configure(GTC3 , GTC_FIN_PCK , 1 , 9 , Counter_Size_32BIT  , 1000 , 500);   //TCCLK=sysclock/2^1/10，20m-->1us
...
{
    I2C_DeInit();                                           //clear all I2C Register
    I2C_Slave_Init(I2C_G2,FAST_MODE,0x4A,0x20,0xAE);        //slave address=0xac(8bit)，standard mode，speed=sysclock/0x138,0x78=setup time
    I2C_Int_Enable();                                       //use interrupt mode 
}
/*************************************************************/
//CORET Functions
...
void CORET_CONFIG(void)
{
	...
	CK802->CORET_RVR=2499999;								//CORETCLK=sysclock/8=20M/8=0.4us  e.g:1s=(CORET_RVR+1)*(8/20M);CORET_RVR=2500000-1=2499999
	CORET_reload();																
...
//------SYSTEM CLK AND PCLK FUNTION---------------------------/
	SYSCON_General_CMD(ENABLE,ENDIS_ISOSC);                                     //enable or disable ISOSC，ISOSC=internal sub crystal
	SYSCON_General_CMD(ENABLE,ENDIS_IMOSC);										//enable or disable IMOSC，IMOSC=internal main crystal
	SYSCON_General_CMD(DISABLE,ENDIS_EMOSC);									//enable or disable EMOSC，EMOSC=ext crystal

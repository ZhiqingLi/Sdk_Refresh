...
//	PDS(按键开始)				SPR(短按松开)			CPS(长按开始)			CPH(长按保持)			  	CPR(长按松开)
    ...
    {MSG_NONE, 				MSG_PLAY_PAUSE,		MSG_WIFI_WPS,		MSG_WIFI_WPS,			MSG_NONE},
...
*****************************************************************************/
B_T GetGpioKeyState(void)
{
    uint8_t ScanIndex;

    for (ScanIndex = 0; ScanIndex < GPIO_KEY_NUM_CNT; ScanIndex++)
    {
        if(GPIO_KEY_STATE_IDLE != GpioKeyState[ScanIndex])
        {
            return TRUE;
        }
    }

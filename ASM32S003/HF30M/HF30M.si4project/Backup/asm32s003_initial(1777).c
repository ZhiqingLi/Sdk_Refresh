...
void GTC0_CONFIG(void)
{
    ...
    GTC_Configure(GTC0 , GTC_FIN_PCK , 1 , 9 , Counter_Size_16BIT  , 100 , 1);   //TCCLK=sysclock/2^1/10ï¼?0m-->1us
...
void GTC1_CONFIG(void)
{
    ...
    GTC_Configure(GTC1 , GTC_FIN_PCK , 1 , 9 , Counter_Size_16BIT  , 100 , 50);   //TCCLK=sysclock/2^1/10ï¼?0m-->1us
...
void GTC2_CONFIG(void)
{
    ...
    GTC_Configure(GTC2 , GTC_FIN_PCK , 0 , 9 , Counter_Size_16BIT  , 1000 , 300);   //TCCLK=sysclock/2^0/10ï¼TCCLK=0.5US
...
void GTC3_CONFIG(void)
{
    ...
    GTC_Configure(GTC3 , GTC_FIN_PCK , 1 , 9 , Counter_Size_32BIT  , 1000 , 500);   //TCCLK=sysclock/2^1/10ï¼?0m-->1us
...
void I2C_MASTER_CONFIG(void)
{
    ...
    I2C_Master_Init(I2C_G1,FAST_MODE,0x040,0x20);           //use I2C group 1ï¼fast modeï¼speed=sysclock/0x040,0x20=setup time
...
{
    I2C_DeInit();                                           //clear all I2C Register
    I2C_Slave_Init(I2C_G2,FAST_MODE,0x4A,0x20,0xAE);        //slave address=0xac(8bit)ï¼standard modeï¼speed=sysclock/0x138,0x78=setup time
    I2C_Int_Enable();                                       //use interrupt mode 
}
/*************************************************************/
//CORET Functions
...
//------SYSTEM CLK AND PCLK FUNTION---------------------------/
	SYSCON_General_CMD(ENABLE,ENDIS_ISOSC);                                     //enable or disable ISOSCï¼ISOSC=internal sub crystal
	SYSCON_General_CMD(ENABLE,ENDIS_IMOSC);										//enable or disable IMOSCï¼IMOSC=internal main crystal
	SYSCON_General_CMD(DISABLE,ENDIS_EMOSC);									//enable or disable EMOSCï¼EMOSC=ext crystal

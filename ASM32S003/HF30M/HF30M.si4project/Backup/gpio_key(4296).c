...
uint16_t GpioKeyScan(void)							
{
	...
	uint8_t GpioKeyIndex = 0xff,GpioScanIndex;
    ...
    for (GpioScanIndex = 0; GpioScanIndex < GPIO_KEY_NUM_CNT; GpioScanIndex++)
    {
    	switch(GpioKeyState[GpioScanIndex])
    	{
            uint8_t GpioKeyMask = ~(1<<GpioScanIndex);
    		...
    		case GPIO_KEY_STATE_CP:
    			...
    			{
    				GpioKeyState[GpioScanIndex] = GPIO_KEY_STATE_IDLE;
    				return GpioKeyEvent[GpioScanIndex][4];//return MSG_NONE;				
    			}
    			else 
    			{
    				return GpioKeyEvent[GpioScanIndex][3];
    			}
    			break;

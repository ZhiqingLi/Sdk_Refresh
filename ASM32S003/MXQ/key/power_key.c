...
uint16_t PowerKeyScan(void)							
{
	...
	switch(PowerKeyState)
	{
		...
		case POWER_KEY_STATE_CP:
			...
			{
				/* 检测到关机按键,系统进入关机，不会返回*/
				PowerKeyState = POWER_KEY_STATE_CP_UP;
                APP_DBG("Detect power off key,System will enter power down!!!!\n");
                return MSG_POWER;
			}
			break;

		case POWER_KEY_STATE_CP_UP:
			//此处仅保证一次按键不会响应多次短按
			if(GPIO_Read_Status(GPIO_ON_OFF_PORT_OUT,GPIO_ON_OFF_PORT_BIT))
			{
				PowerKeyState = POWER_KEY_STATE_IDLE;
...
void SystemPowerOnDetect(void)
{
	...
	GPIO_PullHigh_Init(GPIO_UPGRADE_DETECT_PORT_OUT,GPIO_UPGRADE_DETECT_PORT_BIT);
	GPIO_Init(GPIO_UPGRADE_DETECT_PORT_OUT,GPIO_UPGRADE_DETECT_PORT_BIT,Intput); 

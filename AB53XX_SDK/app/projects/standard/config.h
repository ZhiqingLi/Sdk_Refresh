...
/*****************************************************************************
standard工程默认用到的IO口(配置工具使用Boombox.setting)如下: 开发时注意IO是否有冲突.

PA0~PA6: 7脚屏显示使用,默认显示为 GUI_LEDSEG_7P7S
PA7:     打印及升级IO (打印可以在uart0_mapping_sel中关闭或重新映射到PB2/PB3)
PB0~PB2: SD卡接口(SD0MAP_G2), SD检测默认使用单独IO检测(PE5)
PB3~PB4: USB口
PE0:     MUTE引脚  (可在Boombox.setting中修改)
PE5:     默认SD检测IO口 (可在Boombox.setting中修改SD检测方式)
PE6:     硬件红外
PE7:     ADKEY(USER_ADKEY), 默认做了10个按键(最多支持12个) (可在Boombox.setting中修改)
PF0,PF1: AUX (默认无AUX检测) (可在Boombox.setting中修改)
PF2:     MICL (可在Boombox.setting中修改)
PF3,PF4: 两个LED灯 (可在Boombox.setting中修改)
PWRKEY:  支持5个按键 (可在Boombox.setting中修改)
 *****************************************************************************/
...
#define BT_TWS_M_RECON_EN             	1	//TWS主机回连模式选择，0（只回连手机）， 1（回连手机和从机）
...
#define BT_HFP_EC_NR_EN                 0   //是否打开手机端回音和降噪算法
...
#define BT_SCO_FAR_DELAY_EN             0   //通话是否增加延时，来解决某些第三方测试盒的通话测试问题。增加延时的时候不能使用通话录音
#define BT_SCO_MIC_POST_GAIN            xcfg_cb.mic_post_gain
...
#define FMTX_POWER                      1   //发射功率选择，1：大， 0：小
#define FMTX_DAC_EN                     1   //是否打开DAC
#define FMTX_FIX_BT                     0   //是否打开蓝牙优化选项（选小发射功率配合）

/*****************************************************************************
* Module    : AUX功能配置
******************************************************************************/
#define AUX_CHANNEL_CFG                 (CH_AUXL_PF0 | CH_AUXR_PF1) //选择LINEIN通路
#define MIC_CHANNEL_CFG                 CH_MICR0                    //选择MIC的通路
#define AUX_2_SDADC_EN                  1                           //AUX是否进SDADC, 否则直通DAC。进SDADC可以调AUX EQ, 及AUX录音等功能
#define LINEIN_DETECT_EN                1                           //是否打开LINEIN检测
#define SDCMD_MUX_DETECT_LINEIN         1                           //是否复用SDCMD检测LINEIN插入
...
#define AUX_DNR_EN                      0                       //AUX是否需要动态降噪
#define DAC_AUCAR_EN                    0                       //是否四声道车机前后左右音量调节及音效设置
#define DAC_DRC_EN                      0                       //是否使能DRC功能
#define DAC_VBS_EN                      0                       //是否使能虚拟低音功能
...
#define SPF_REC_EN                      0   //是否打开SPDIF录音功能
...
#define KARAOK_REC_NO_ECHO_EN           0   //录音是否不录混响
#define REC_ONE_FOLDER_EN               0   //是否各模式录音放到同一目录下
#define REC_DIR_LFN                     1   //是否打开长文件名目录
#define REC_AUTO_PLAY                   1   //录音结束是否回放当前录音内容
...
#define SPIFALSH_MUSIC_PLAY_REC        1           // 1 FUNC_EXSPIFLASH_MUSIC 模式下播放录音示例： 支持录音上下曲，及删除当前播放的录音  //0 播放镜像音乐示例
...
#define DUAL_MIC_CHANNEL_EN             0   //双MIC使能.  //SYS_KARAOK_EN为1时有效
...
#define USER_KEY_DOUBLE                 {KU_HSF, KU_PREV_VOL_DOWN, KU_NEXT_VOL_UP}     //支持双击的按键
...
#define SD1_MAPPING                     SD0MAP_G1   //选择SD1 mapping
...
#define VBAT_DETECT_MUX_SDCLK           0           //电池电量检测复用SDCLK,  检测电池的1/2分压. 电池接100K上下拉实现1/2分压.
...
#define AUCAR_EQ_LOUD_BASS_TREBLE_EN    0           //车机EQ/LOUD/BASS/TRM同时有效. //注意EQ资源文件不能为空(0KB)  //一般配合车机相关宏使用 SYS_ADJ_DIGVOL_EN  /  DAC_AUCAR_EN

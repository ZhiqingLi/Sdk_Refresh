...
void kara_rec_stop(void);
void kara_rec_start(void);

AT(.text.func.music)
void msc_rec_init(void)
{
    rec_src.spr = SPR_44100;
    rec_src.nchannel = 1;
    rec_src.source_start = kara_rec_start;
    rec_src.source_stop  = kara_rec_stop;
}
#endif
...
void func_music_file_new(void)
{
    if (f_msc.file_change) {
        ...
        if (fs_open_num(f_msc.file_num)) {
            ...
            f_msc.fname[99] |= BIT(1);                              //已获取长文件名
    #if MUSIC_LRC_EN
            bsp_lrc_init();
    #endif // MUSIC_LRC_EN
            if (!(f_msc.fname[99] & BIT(7))) {
                //UTF-16转换成GBK编码
                utf16_convert_to_gbk(f_msc.fname, f_msc.fname[98]);
                f_msc.fname[99] |= BIT(7);
            }
#endif // GUI_LCD_EN
...
#if BT_APP_EN
void func_app_msc_process(void)
{
    if (app_cb.msc_list_sta == 1) {     //曲目名字传输开始
        static u32 list_tick = 0;
        if (tick_check_expire(list_tick, 100)) {    //慢慢发，1S 10条
            bsp_app_msc_get_name_for_num(app_cb.msc_list_cnt++);
            if (app_cb.msc_list_cnt > app_cb.msc_list_end) {

	...
	{	
		...
		else 
		{
			...
			else
			{
				...
				else if(memcmp(gWiFiCmd, "+IOT1+ON", 8) == 0)
				{
					CmdLen = 8;	
				}
				else if(memcmp(gWiFiCmd, "+IOT1+OFF", 9) == 0)
				{
					CmdLen = 9;	
				}
				else if(memcmp(gWiFiCmd, "+IOT", 4) == 0)
				{
					CmdLen = 20;	
				}
...
//MCU发送给WiFi模组的命令数据
/*****************************************************************************
 函 数 名  : Mcu_SendCmdToWiFi
 功能描述  : MCU传送命令到WiFi
 输入参数  : uint16_t McuCmd   
             uint8_t* DataBuf  
 输出参数  : 无
 返 回 值  : 
 调用函数  : 
...
{	
	...
	switch(McuCmd)
	{
		...
		case MCU_IOTN__ON:
			memcpy((void*)&CmdBuf[Temp], (void*)"MCU+IOT", 7);
			CmdBuf[7] = (gWiFi.IotSlaveDeviceID + 0x30);
			Temp = 8;
			memcpy((void*)&CmdBuf[Temp], (void*)"+ON", 3);
			CmdBuf[11] = '&';
			Len = 12;
			break;
		...
		case MCU_IOTN_OFF:
			...
			CmdBuf[7] = (gWiFi.IotSlaveDeviceID + 0x30);
			Temp = 8;
			memcpy((void*)&CmdBuf[Temp], (void*)"+OFF", 4);
			CmdBuf[12] = '&';
			Len = 13;
			break;
		...
		case MCU_IOTN_LIGHT:
			...
			CmdBuf[7] = (gWiFi.IotSlaveDeviceID + 0x30);
			Temp = 8;
			memcpy((void*)&CmdBuf[Temp], (void*)"Percentage+", 11);
			CmdBuf[19] = (DataBuf[0]/100) + 0x30;		
			CmdBuf[20] = ((DataBuf[0]%100)/10) + 0x30;
			CmdBuf[21] = (DataBuf[0]%10) + 0x30;
			CmdBuf[22] = '&';
			Len = 23;
			break;
		...
		case MCU_IOTN_COLOR:
			...
			CmdBuf[7] = (gWiFi.IotSlaveDeviceID + 0x30);
			Temp = 8;
			memcpy((void*)&CmdBuf[Temp], (void*)"Color+", 6);
            if((DataBuf[0]/16) < 10)							//数字0-9
            {
                CmdBuf[14] = (DataBuf[0]/16) + 0x30;
            }
            else													//字符A-F
            {
                CmdBuf[14] = ((DataBuf[0]/16)-10)+0x41;
            }
            
            if((DataBuf[0]%16) < 10)							//数字0-9
            {
                CmdBuf[15] = (DataBuf[0]%16) + 0x30;
            }
            else													//字符A-F
            {
                CmdBuf[15] = ((DataBuf[0]%16)-10)+0x41;
            }
            
			if((DataBuf[1]/16) < 10)							//数字0-9
            {
                CmdBuf[16] = (DataBuf[1]/16) + 0x30;
            }
            else													//字符A-F
            {
                CmdBuf[16] = ((DataBuf[1]/16)-10)+0x41;
            }
            
            if((DataBuf[1]%16) < 10)							//数字0-9
            {
                CmdBuf[17] = (DataBuf[1]%16) + 0x30;
            }
            else													//字符A-F
            {
                CmdBuf[17] = ((DataBuf[1]%16)-10)+0x41;
            }

            if((DataBuf[2]/16) < 10)							//数字0-9
            {
                CmdBuf[18] = (DataBuf[2]/16) + 0x30;
            }
            else													//字符A-F
            {
                CmdBuf[18] = ((DataBuf[2]/16)-10)+0x41;
            }
            
            if((DataBuf[2]%16) < 10)							//数字0-9
            {
                CmdBuf[19] = (DataBuf[2]%16) + 0x30;
            }
            else													//字符A-F
            {
                CmdBuf[19] = ((DataBuf[2]%16)-10)+0x41;
            }
			CmdBuf[20] = '&';
			Len = 21;
			break;

		case MCU_PAS_IOTN__ON:
			memcpy((void*)&CmdBuf[Temp], (void*)"MCU+PAS+IOT", 11);
			CmdBuf[11] = (gWiFi.IotSlaveDeviceID + 0x30);
			Temp = 12;
			memcpy((void*)&CmdBuf[Temp], (void*)"+ON", 3);
			CmdBuf[15] = '&';
			Len = 16;
		...
		case MCU_PAS_IOTN_LIGHT:
			memcpy((void*)&CmdBuf[Temp], (void*)"MCU+PAS+IOT", 11);
			CmdBuf[11] = (gWiFi.IotSlaveDeviceID + 0x30);
			Temp = 12;
			memcpy((void*)&CmdBuf[Temp], (void*)"Percentage+", 11);
			CmdBuf[23] = (DataBuf[0]/100) + 0x30;		
			CmdBuf[24] = ((DataBuf[0]%100)/10) + 0x30;
			CmdBuf[25] = (DataBuf[0]%10) + 0x30;
			CmdBuf[26] = '&';
			Len = 27;
			break;

		case MCU_PAS_IOTN_COLOR:
			memcpy((void*)&CmdBuf[Temp], (void*)"MCU+PAS+IOT", 11);
			CmdBuf[11] = (gWiFi.IotSlaveDeviceID + 0x30);
			Temp = 12;
			memcpy((void*)&CmdBuf[Temp], (void*)"Color+", 6);
            if((DataBuf[0]/16) < 10)							//数字0-9
            {
                CmdBuf[18] = (DataBuf[0]/16) + 0x30;
            }
            else													//字符A-F
            {
                CmdBuf[18] = ((DataBuf[0]/16)-10)+0x41;
            }
            
            if((DataBuf[0]%16) < 10)							//数字0-9
            {
                CmdBuf[19] = (DataBuf[0]%16) + 0x30;
            }
            else													//字符A-F
            {
                CmdBuf[19] = ((DataBuf[0]%16)-10)+0x41;
            }
            
			if((DataBuf[1]/16) < 10)							//数字0-9
            {
                CmdBuf[20] = (DataBuf[1]/16) + 0x30;
            }
            else													//字符A-F
            {
                CmdBuf[20] = ((DataBuf[1]/16)-10)+0x41;
            }
            
            if((DataBuf[1]%16) < 10)							//数字0-9
            {
                CmdBuf[21] = (DataBuf[1]%16) + 0x30;
            }
            else													//字符A-F
            {
                CmdBuf[21] = ((DataBuf[1]%16)-10)+0x41;
            }

            if((DataBuf[2]/16) < 10)							//数字0-9
            {
                CmdBuf[22] = (DataBuf[2]/16) + 0x30;
            }
            else													//字符A-F
            {
                CmdBuf[22] = ((DataBuf[2]/16)-10)+0x41;
            }
            
            if((DataBuf[2]%16) < 10)							//数字0-9
            {
                CmdBuf[23] = (DataBuf[2]%16) + 0x30;
            }
            else													//字符A-F
            {
                CmdBuf[23] = ((DataBuf[2]%16)-10)+0x41;
            }
			CmdBuf[24] = '&';
			Len = 25;
			break;

		case MCU_IOTN_LBC_NNN:
			memcpy((void*)&CmdBuf[Temp], (void*)"MCU+LBC+003&", 12);
			Len = 12;
			break;
#endif
...
void WiFi_SendCmdToMcu(uint16_t WiFiCmd, uint8_t* CmdData)
{
	...
	{		
		...
		case AXX_AXA_NNN:
			...
			if(Temp == 0x01)
			{
				WiFiLoginStateSet(WIFI_STATUS_LOGIN_FAILED);
			}
			else
			{
				WiFiLoginStateSet(WIFI_STATUS_LOGIN_SUCCESSFUL);
			}
			break;
...
void WiFi_CmdProcess(void)
{
	...
	//凯叔命令end
	//以下平台登录成功命令
	else if(memcmp(gWiFiCmd, "+AXA+", 5) == 0)
	{
		WiFi_SendCmdToMcu(AXX_AXA_NNN, &gWiFiCmd[5]);
	}
#ifdef FUNC_WIFI_ALEXA_PROJECT_EN
#ifdef FUNC_LIGHT_PWM_LED_EN
	else if((memcmp(gWiFiCmd, "+IOT", 4) == 0)
	&& (gWiFiCmd[4] == (gWiFi.IotSlaveDeviceID+0x30)))
	{
		WiFi_CtrlPwmLight(&gWiFiCmd[5],FALSE);
	}
#endif
#endif
#ifdef FUNC_WIFI_UART_UPGRADE

...
static uint8_t bt_hw_power_cmd_state = BT_HW_POWER_CMD_POWER_ON;
extern void BtMtkLDOEn(bool EnFlag);
extern void BtMtkRestEn(bool EnFlag);
extern void BtLDOEn(bool Flag);
...
extern uint8_t GetPairedListMac(const uint8_t *addr1, const uint8_t *addr2);
 ...
 ***************************************/
uint8_t RemoveBtDevice(const uint8_t* addr)
{
	uint8_t ret;

	ret = GetPairedListMac((void*)addr, (void*)GetCurConnectedRemDevAdd());
	if(ret)
	{
		BTDisConnect();
	}
	
	ret = ClearPairedList((void*)addr);
	if(ret == 0)
	{
		DBG("success\n");
	}

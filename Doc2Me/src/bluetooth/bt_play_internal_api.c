...
#ifdef FUNC_BT_EN
bool start_buffer_sbc_data;
bool a2dp_player_switch_bluetooth;
...
//			SetBtModeStatus(BT_MODE_ACTIVE_STATUS);
			...
			{
				start_buffer_sbc_data = TRUE;
				SetPlayState(PLAYER_STATE_PLAYING);
			}
			else if(!gWiFi.PlayState && (gWiFi.WiFiAppPlayMode != WIFI_PLAY_MODE_AVS))
			{
				start_buffer_sbc_data = TRUE;
				a2dp_player_switch_bluetooth = TRUE;
				APP_DBG("BT A2DP PLAYER START SWITCH TO BLUETOOTH\n");
...
//			SetBtModeStatus(BT_MODE_SNIFF_STATUS);
		...
		case BT_DS_CALL_SCO_STREAM_PAUSE:
			...
			if(GetHfpState() == BT_HFP_STATE_ATCTIVE_CALL || GetHfpState() == BT_HFP_STATE_INCOMING_CALL || GetHfpState() == BT_HFP_STATE_OUTGOING_CALL 
				|| GetHfpState() == BT_HFP_STATE_3WAY_ATCTIVE_CALL || GetHfpState() == BT_HFP_STATE_3WAY_INCOMING_CALL || GetHfpState() == BT_HFP_STATE_3WAY_OUTGOING_CALL 
				)
			{
				break;
			}

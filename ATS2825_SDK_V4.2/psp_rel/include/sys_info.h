...
/*-------------------------------------------------------------------------------------------------
OTA 版本固件分布
MBREC   BREC   PARAM | LFI              |LFI_backup         | PARAM_backup BREC_backup VRAM0  VRAM1
(64KB)  (64KB) (64KB)| (LFI_MAX_SIZE)   |(LFI_MAX_SIZE)     | (64KB)       (64KB)      (64KB) (64KB)
----------------------------------------------------------------------------------------------------*/
...
//OTA:LFI大小分2类:
//4Mnor  :LFI的最大大小为0XE00，(flash_sector - s3bt_size - reserved_size - brec_sector*2 - mbrec_size - param_size*2 - vram_size)/2
...
#define RESERVED_USED_SIZE          ((1 + 2 + 2 + 2) * SPI_NOR_SECTOR_PER_BLOCK)

//实际是0xe40但要block对齐到0xe00
#define NOR_4M_LFI_MAX_SIZE         (0xe00)

#define NOR_2M_LFI_MAX_SIZE         (0x600)
...
typedef enum {
    ...
    CONFIG_ID_029_POWER_ONOFF_MODE = 4,
} config_id_for_app_e;

...
#define ATTR_TITLE_MAX_LEN      64
#define ATTR_ARTIST_MAX_LEN      32
...
#define BT_IDLE_TO_SNIFF_TIME  5000
#endif
...
typedef enum {
    ...
    SMART_MODE_SPEED,
    SMART_MODE_BUF,
    SMART_MODE_SLAVE, //发送给副箱清除pipe length
    SMART_MODE_MAX
} smart_recognize_mode_e;
...
#endif
#define NOTIFY_CMD_BLE_STATE                1
#define NOTIFY_CMD_VOICE_DATA               2
typedef uint8(*data_encrypt_t)(uint8 notify_cmd, void *buf, uint16 len);
...
//4 is reserved for other use
#define TWS_MASTER_SMART_MODE  5
#define TWS_MASTER_EXIT_SMART_MODE 6
...
typedef struct {
    uint8 title[ATTR_TITLE_MAX_LEN];
    uint8 artist[ATTR_ARTIST_MAX_LEN];
    uint16 title_len;
    uint16 artist_len;
} ElementAttributesStru;
#endif

...
void set_in_int_ep(void)
{
    ...
    act_writel(0x00000d00, IN2STADDR);
        ...
        case GET_SAMPLERATE:
            if((g_setup_data.bRequest == GET_CUR) && (g_setup_data.wValue == 0x0100)) {
                if(g_play_sample == 44100) {
                    control_read(All_Descriptors_of_Configuration.Audio_Class_Format_Setting1.tSamFreq1, \
                                 0x03, (uint8) g_setup_data.wLength);
                } else if(g_play_sample == 48000) {
                    control_read(All_Descriptors_of_Configuration.Audio_Class_Format_Setting1.tSamFreq2, \
                                 0x03, (uint8) g_setup_data.wLength);
                } else if(g_play_sample == 96000) {
                    control_read(All_Descriptors_of_Configuration.Audio_Class_Format_Setting1.tSamFreq3, \
                                 0x03, (uint8) g_setup_data.wLength);
                } else {
                    ;//for qac
                }

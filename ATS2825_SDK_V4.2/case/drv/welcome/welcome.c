...
void welcome_delay_ms(uint16 dly_ms)
{
    uint8 i, j;
    while(dly_ms != 0) {
        for(i = 0; i < 8; i++) {
...
void disp_starting_board_config(uint8 charge_mode, welcome_argument_t *welcome_arg)
{
...
#if defined(EXTERN_BATTER_DETECT_PIN)
    ...
    if(charge_mode == BAT_CHARGE_MODE_OUTER) {
        //enable charge and battery check
        act_writel(act_readl(EXTERN_CHARGE_DETECT_GPIOUPEN) | EXTERN_CHARGE_DETECT_PIN, EXTERN_CHARGE_DETECT_GPIOUPEN);
        act_writel(act_readl(EXTERN_CHARGE_DETECT_GPIOIN) | EXTERN_CHARGE_DETECT_PIN, EXTERN_CHARGE_DETECT_GPIOIN);
    }
#endif
#endif
...
#if defined(TK_DRIVER_FTC334F) && defined(BOARD_JL_36410_FAR)
bool check_ftc334_key(welcome_argument_t *welcome_arg)
{
    uint32 val = 0;

    val = act_readl(FTC_GPIO_SCAN_PIN_OUT_EN);// & (~(1 << 16 | 1 << 11));
    act_writel(val | (1 << 21), FTC_GPIO_SCAN_PIN_OUT_EN);

    val = act_readl(GPIOAINEN);// & (~(1 << 21));
    act_writel(val | (1 << 16 | 1 << 11), GPIOAINEN);

    //val = act_readl(GPIOAPDEN);// & (~(1 << 21));
    //act_writel(val | (1 << 16 | 1 << 11), GPIOAPDEN);

    //val = act_readl(FTC_GPIO_SCAN_PIN_PU_EN);// & (~(1 << 21));
    //act_writel(val | (1 << 21), FTC_GPIO_SCAN_PIN_PU_EN);

    val = act_readl(GPIOADAT);// & (~(1 << 21));
    act_writel(val | (1 << 21), GPIOADAT);
    welcome_delay_ms(1000);

    //welcome_arg->print("1:", (val >> 11));
    val = act_readl(GPIOADAT);
    if((val & (1 << 11 | 1 << 16)) != (1 << 11 | 1 << 16))
        return TRUE;
...
void disp_starting_adfu_check(void(*adfu_launch)(void), welcome_argument_t *welcome_arg)
{
    ...
    uint8 poweroff_mode = (uint8)cfg_value[CONFIG_ID_029_POWER_ONOFF_MODE];
    ...
    if(is_usb_cable_in() == TRUE) {
        ...
        if (poweroff_mode && ((act_readl(ONOFF_KEY)&(1<<ONOFF_KEY_ONOFF_PRESS_0)) != 0)) //PLAY键
        {
            adfu_flag = TRUE;
        }
...
void disp_starting_low_power(uint8 charge_mode)
{
    ...
    if(usb_cable_adapter == FALSE) {
        ...
        if(charge_mode == BAT_CHARGE_MODE_OUTER) {
            ...
            //if((act_readb(LRADC1_DATA) & 0x7f) < TEMP_ADC_1_51_V) {
            //    enter_s4 = TRUE;
            //}
...
{
#if((CASE_BOARD_TYPE == CASE_BOARD_EVB) || (CASE_BOARD_TYPE == CASE_BOARD_DVB_ATS2825)) \
    && !defined(BOARD_DPI_7C370_FAR) && !defined(BOARD_JL_36410_FAR)
...
#if defined(SPEAKER_A_PA_EN_GPIOOUT)
    //mute enable, close pa
...
//#if defined(BOARD_LINKPLAY_FAR_DEMO)
    //power on the dsp
//    act_writel(act_readl(GPIOAOUTEN) | (1 << 5), GPIOAOUTEN);
//    act_writel((act_readl(GPIOADAT) & (~(1 << 5))), GPIOADAT);
//    welcome_delay_ms(10);
//    act_writel((act_readl(GPIOADAT) | (1 << 5)), GPIOADAT);
//#endif

...
uint8 g_dae_filter_flag = 0;

extern void key_inner_alexa_led_deal(void);

#if (SUPPORT_ADC_KEY == 1)

#if defined(BOARD_LINKPLAY_MUVO)
#define ADC_KEY_NUM  (4)
#elif defined(BOARD_DPI_7C370_FAR)
...
uint8 key_power_io_handle(void)
{
    if((act_readl(POWER_GPIO_DATA) & POWER_GPIO_PIN))
        return NO_KEY;

    return 0x05;        //map to power key
}
#endif
...
#if defined(BOARD_DPI_7C370_FAR)
    if(key_val == NO_KEY)
        key_val = key_power_io_handle();
#endif

#if defined(TK_DRIVER_FTC334F)
#if defined(BOARD_JL_36410_FAR)
    if((act_readl(ONOFF_KEY) & (1 << ONOFF_KEY_ONOFF_PRESS_0)) != 0) {
        key_val = KEY_PHY_PLAY;    //KEY_POWER
    }
#endif

    if(key_val == NO_KEY)
        key_val = read_ftc334_data();
#endif

#if defined(ENABLE_ALEXA_LED_NOTIFY)
    key_inner_alexa_led_deal();
#endif

...
const uint8 __section__(".rodata_1") battery_grade_vel_temp[BATTERY_GRADE_MAX + 1] = {
    0,               //低电    返回 0 -- 低电关机
    TEMP_ADC_1_52_V, //空格电  返回 1 -- 低电提示，1分钟提示1次(3.5V)
    TEMP_ADC_1_54_V, //一格电  返回 2
    TEMP_ADC_1_57_V, //一格电  返回 3
    TEMP_ADC_1_60_V, //两格电  返回 4
    TEMP_ADC_1_62_V, //两格电  返回 5
    TEMP_ADC_1_64_V, //三格电  返回 6
    TEMP_ADC_1_66_V, //三格电  返回 7
    TEMP_ADC_1_68_V, //四格电  返回 8
    TEMP_ADC_1_69_V, //四格电  返回 9

    /*
    TEMP_ADC_1_48_V, //空格电  返回 1 -- 低电提示，1分钟提示1次(3.3V)
    TEMP_ADC_1_50_V, //一格电  返回 2       3.4V
    TEMP_ADC_1_52_V, //一格电  返回 3       3.5V
    TEMP_ADC_1_55_V, //两格电  返回 4       3.7V
    TEMP_ADC_1_58_V, //两格电  返回 5       3.8V
    TEMP_ADC_1_62_V, //三格电  返回 6       3.9V
    TEMP_ADC_1_63_V, //三格电  返回 7       3.9V
    TEMP_ADC_1_67_V, //四格电  返回 8       4.0V
    TEMP_ADC_1_69_V  //四格电  返回 9       4.1V
    */
};
#else
const uint8 __section__(".rodata_1") battery_grade_vel_temp[BATTERY_GRADE_MAX + 1] = {
...
void ker_battery_charge_deal(void)
{
    ...
    if(g_key_bat_charge_mode == BAT_CHARGE_MODE_OUTER) {
        /*外挂充电检测*/
        if(IS_EXTERN_CAHREGE_IN()) {
#if defined(EXTERN_CHARGE_FULL_GPIODAT)
...
void __section__(".bank0") key_bat_charge_init(chg_current_e CurrentMA, chg_voltage_e VoltageMV)
{
    ...
    if(g_key_bat_charge_mode == BAT_CHARGE_MODE_OUTER) {
...
#if defined(EXTERN_CHARGE_FULL_GPIOIN)
        ...
        libc_print("charge init", 0, 0);

#if defined(BOARD_DPI_7C370_FAR)
        //use GPIOA21 map to tempadc
        act_writel((act_readl(PMUADC_CTL) | (1 << PMUADC_CTL_TEMPADC_EN)), PMUADC_CTL);
        act_writel((act_readl(AD_Select) & (~AD_Select_GPIOA21_MASK)), AD_Select);
        act_writel((act_readl(AD_Select) | (0x01 << AD_Select_GPIOA21_SHIFT)), AD_Select);
#elif defined(BOARD_JL_36410_FAR)
        //use GPIOB0 map to lradc1
...
#if defined(BOARD_JL_36410_FAR)
            bat_adc = (act_readb(LRADC1_DATA)) & 0x7F;
            //bat_adc = 0X74;
#else

 ...
 * \note
 *******************************************************************************/
void cnvrt_ascii_to_unicode(uint8 *pstraddr, uint8 strlgth)
{
    uint8 *data_p1;
    uint8 *data_p2;
 ...
 * \note
 *******************************************************************************/
uint32 conver_string_to_hex(const char *src, int lenth)
{
    int total = 0;
    int temp_value;
...
void descriptor_init(void)
{
    ...
    if(0 != g_usb_vid[2]) {
        Device_Descriptor.idVendor = (unsigned short) conver_string_to_hex((char *) &g_usb_vid[2], 4);
    }

    if(0 != g_usb_pid[2]) {
        Device_Descriptor.idProduct = (unsigned short) conver_string_to_hex((char *) &g_usb_pid[2], 4);
    }
    ...
    cnvrt_ascii_to_unicode((uint8 *) &manufacure_string.wManufacturerName, string_len);
    ...
    cnvrt_ascii_to_unicode((uint8 *) &device_name_string.wProductName, string_len);

    sys_get_fw_info((uint8 *) &ret_type, FW_INFO_USB_SN_TYPE_ADDR, 1);
    //for config no care,1,random(fw generate),2 fix(fw config)
    if((2 == ret_type) || (1 == ret_type)) {
        ...
        if(string_len != 0) {
            ...
            cnvrt_ascii_to_unicode((uint8 *) &SerialNumber.wManufacturerName, string_len);
        } else {
            //if no char in fw wo don't need report to pc

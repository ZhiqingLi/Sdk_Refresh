...
void pb_ostream_from_buffer(pb_byte_t *buf, size_t bufsize, pb_ostream_t *stream)
{
...
static PB_ENCODE_SECTION bool checkreturn encode_field(pb_ostream_t *stream,
        const pb_field_t *field, const void *pData)
{
...
bool checkreturn pb_encode_negative_varint(pb_ostream_t *stream, pb_uint64_t value)
{
    ...
    while(value) {
        ...
        if(compensation) {
            ...
            value ^= (pb_uint64_t)((0xFFu >> (8 - bits)) <<
                                   25); /* set the number of bits needed on the lowest of the most significant 7 bits */
            compensation -= bits;
        }
        i++;
    }
...
bool PB_ENCODE_SECTION checkreturn pb_encode_tag(pb_ostream_t *stream, pb_wire_type_t wiretype, uint32_t field_number)
{
    pb_uint64_t tag = ((pb_uint64_t)field_number << 3) | wiretype;
    return pb_encode_varint(stream, tag);
}

bool PB_ENCODE_SECTION checkreturn pb_encode_tag_for_field(pb_ostream_t *stream, const pb_field_t *field)
{
...
bool PB_ENCODE_SECTION checkreturn pb_encode_string(pb_ostream_t *stream, const pb_byte_t *buffer, size_t size)
{
    if(!pb_encode_varint(stream, (pb_uint64_t)size))
        return false;
...
bool PB_ENCODE_SECTION checkreturn pb_encode_submessage(pb_ostream_t *stream, const pb_field_t fields[],

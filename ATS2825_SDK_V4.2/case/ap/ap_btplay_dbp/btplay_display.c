...
#endif
    {
        if((g_btplay_cur_info.status != BTPLAY_IDLE) || (g_bt_stack_cur_info.num_connected == 2)) {
            return;
        }
    }
        ...
        case VIEW_UPDATE_STATUS: {
            ...
            if(g_comval.support_led_display == 1) {
...
#endif
                {
                    ...
                    btplay_ui_display();
#else
                    if(g_bt_stack_cur_info.num_connected == 2) {
                        btplay_ui_display_dbp();
                    } else {
                        btplay_ui_display();
                    }
#endif

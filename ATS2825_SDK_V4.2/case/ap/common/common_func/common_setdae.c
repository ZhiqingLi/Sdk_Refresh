...
bool __set_dae_config(dae_config_t *p_dae_cfg)
{
    ...
    if((g_app_info_state.stub_tools_type == STUB_PC_TOOL_ASET_EQ_MODE)
            ...
            && (g_app_info_vector[APP_TYPE_GUI].app_id != APP_ID_BTCALL)) {
        //同步当前ASET工具的PEQ设置
...
//      PRINT_INFO_INT("index_aux",index);
        ...
        sys_comval->dae_cfg.p_mdrc_band_standard_mode[0].makeup = \
                vol_dae_para_temp_standard_mode.vol_dae_para_sm.make_up_low;

        sys_comval->dae_cfg.p_mdrc_band_standard_mode[1].makeup = \
                vol_dae_para_temp_standard_mode.vol_dae_para_sm.make_up_middle;

        sys_comval->dae_cfg.p_mdrc_band_standard_mode[2].makeup = \
                vol_dae_para_temp_standard_mode.vol_dae_para_sm.make_up_high;
    } else {
        //设置压缩器(非AUX)模式下参数

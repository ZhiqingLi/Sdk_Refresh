...
void sys_counter_handle(void)
{
    ...
    if((g_bt_stack_cur_info.conn_status != CONN_STATUS_PAIRED) && (g_bt_stack_cur_info.conn_status != CONN_STATUS_LINKED)) {
        msg_apps_t msg;
        if(g_bt_stack_cur_info.ble_con_flag == 1) {
            msg.type = MSG_BTSTACK_BLE_DISCONNECT_SYNC;
            msg.content.data[0] = 0;
            send_sync_msg_btmanager(NULL, &msg, NULL, 0);
        }

        if(g_bt_stack_cur_info.spp_con_flag == 1) {

...
const discrete_led_config_t discrete_led_cfg[LED_ID_MAX] = {
    ...
    { LED_AUX_PLAY_PORT, LED_AUX_PLAY_PIN },
#endif
#if defined(SUPPORT_MIC_MUTE_FUNCTION) && !defined(BOARD_JL_36410_FAR)
    { LED_MIC_MUTE_PORT, LED_MIC_MUTE_PIN},
#endif
...
void discrete_led_init(void)
{
    ...
    for(n = 0; n < LED_ID_MAX; n++) {
        ...
        g_discrete_led_info[n].reserve = LED_RESERVE_ENABLE_CTL;
    ...
    com_smart_mic_mute(sys_comval->mic_mute, FALSE);
#endif
}
...
void discrete_led_set(led_id_e id, led_mode_e mode, led_duty_t *duty_param)
{
    ...
    if(mode == LED_ALWAYS_OFF) {
        if(g_discrete_led_info[id].on_off == TRUE) {
            ...
            if((g_discrete_led_info[id].reserve & LED_RESERVE_ENABLE_MASK) == LED_RESERVE_ENABLE_CTL) {
...
void __section__(".text.bank24") discrete_led_disable(led_port_e id, uint8 state)
{
	if(state == 0)	//off
		key_discrete_led_off(g_discrete_led_info[id].led_port, g_discrete_led_info[id].led_pin);
	else //on
    ...
    g_discrete_led_info[id].reserve &= (~LED_RESERVE_ENABLE_MASK);
    g_discrete_led_info[id].reserve |= LED_RESERVE_DISABLE_CTL;
}

void __section__(".text.bank24") discrete_led_enable(led_port_e id)
{
    g_discrete_led_info[id].reserve &= (~LED_RESERVE_ENABLE_MASK);
    g_discrete_led_info[id].reserve |= LED_RESERVE_ENABLE_CTL;
    if(g_discrete_led_info[id].on_off == TRUE) {
        key_discrete_led_on(g_discrete_led_info[id].led_port, g_discrete_led_info[id].led_pin);
    }
}
...
void __section__(".text.led_twinkle") discrete_led_twinkle(void)
{
    ...
    if((g_discrete_led_info[i].reserve & LED_RESERVE_ENABLE_MASK) == LED_RESERVE_DISABLE_CTL) {
        return;
    }

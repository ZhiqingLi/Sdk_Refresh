...
int com_btmanager_avrcp_op_async(btplay_avrcp_opid_e opid, uint8 force)
{
    msg_apps_t msg;
...
//    }
    ...
    msg.content.data[0] = s_btstack_avrcp_ops[opid].op_type;
    msg.content.data[1] = s_btstack_avrcp_ops[opid].op_id;
    msg.content.data[2] = force;

    //发送异步消息
    send_async_msg_btmanager(g_bt_stack_cur_info.rmt_dev[g_bt_stack_cur_info.a2dp_active_id].addr.bytes, &msg);

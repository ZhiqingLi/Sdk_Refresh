...
void SPEAKER_CONTROL_ENABLE(void)
{
    if(g_app_info_state.aout_mode == AO_SOURCE_DAC
            || g_app_info_state.aout_mode == AO_SOURCE_DACI2S
            || g_app_info_state.aout_mode == AO_SOURCE_DACSPDIF) {
        if(get_headphone_state() == HEADPHONE_STATE_NULL) {
#if defined(SPEAKER_A_PA_EN_PIN)
            act_writel(act_readl(SPEAKER_A_PA_EN_GPIOOUT) | SPEAKER_A_PA_EN_PIN, SPEAKER_A_PA_EN_GPIOOUT);
#endif

#if defined(SPEAKER_A_PA_BST_EN_PIN)
            act_writel(act_readl(SPEAKER_A_PA_BST_EN_GPIOOUT) | SPEAKER_A_PA_BST_EN_PIN, SPEAKER_A_PA_BST_EN_GPIOOUT);
#endif

#if defined(SPEAKER_A_PA_D_AB_PIN)
            act_writel(act_readl(SPEAKER_A_PA_D_AB_GPIOOUT) | SPEAKER_A_PA_D_AB_PIN, SPEAKER_A_PA_D_AB_GPIOOUT);
#endif

#if defined(SPEAKER_A_PA_CTRL_PIN)
            act_writel(act_readl(SPEAKER_A_PA_CTRL_GPIOOUT) | SPEAKER_A_PA_CTRL_PIN, SPEAKER_A_PA_CTRL_GPIOOUT);
#endif

#if defined(SPEAKER_A_PA_BST_EN_PIN) && (SPEAKER_A_PA_BST_EN_LEVEL == ENABLE_LEVEL_HIGH)
            act_writel((act_readl(SPEAKER_A_PA_BST_EN_GPIODATA) | SPEAKER_A_PA_BST_EN_PIN),
                       SPEAKER_A_PA_BST_EN_GPIODATA);
#endif

#if defined(SPEAKER_A_PA_BST_EN_PIN) && (SPEAKER_A_PA_BST_EN_LEVEL == ENABLE_LEVEL_LOW)
            act_writel((act_readl(SPEAKER_A_PA_BST_EN_GPIODATA) & (~SPEAKER_A_PA_BST_EN_PIN)),
                       SPEAKER_A_PA_BST_EN_GPIODATA);
#endif

#if defined(SPEAKER_A_PA_D_AB_PIN) && (SPEAKER_A_PA_D_AB_LEVEL == ENABLE_LEVEL_HIGH)
            act_writel((act_readl(SPEAKER_A_PA_D_AB_GPIODATA) | SPEAKER_A_PA_D_AB_PIN), SPEAKER_A_PA_D_AB_GPIODATA);
#endif

#if defined(SPEAKER_A_PA_D_AB_PIN) && (SPEAKER_A_PA_D_AB_LEVEL == ENABLE_LEVEL_LOW)
            act_writel((act_readl(SPEAKER_A_PA_D_AB_GPIODATA) & (~SPEAKER_A_PA_D_AB_PIN)), SPEAKER_A_PA_D_AB_GPIODATA);
#endif

#if defined(SPEAKER_A_PA_CTRL_PIN) && (SPEAKER_A_PA_CTRL_LEVEL == ENABLE_LEVEL_HIGH)
            act_writel((act_readl(SPEAKER_A_PA_CTRL_GPIODATA) | SPEAKER_A_PA_CTRL_PIN), SPEAKER_A_PA_CTRL_GPIODATA);
#endif

#if defined(SPEAKER_A_PA_CTRL_PIN) && (SPEAKER_A_PA_CTRL_LEVEL == ENABLE_LEVEL_LOW)
            act_writel((act_readl(SPEAKER_A_PA_CTRL_GPIODATA) & (~SPEAKER_A_PA_CTRL_PIN)), SPEAKER_A_PA_CTRL_GPIODATA);
#endif
        }
...
void SPEAKER_CONTROL_DISABLE(void)
{
    if(g_app_info_state.aout_mode == AO_SOURCE_DAC
            ...
            || g_app_info_state.aout_mode == AO_SOURCE_DACSPDIF) {
        if(get_headphone_state() == HEADPHONE_STATE_NULL) {
#if defined(SPEAKER_A_PA_EN_PIN) && (SPEAKER_A_PA_EN_LEVEL == ENABLE_LEVEL_HIGH)
            act_writel((act_readl(SPEAKER_A_PA_EN_GPIODATA) & (~SPEAKER_A_PA_EN_PIN)), SPEAKER_A_PA_EN_GPIODATA);
#endif

#if defined(SPEAKER_A_PA_EN_PIN) && (SPEAKER_A_PA_EN_LEVEL == ENABLE_LEVEL_LOW)
            act_writel((act_readl(SPEAKER_A_PA_EN_GPIODATA) | SPEAKER_A_PA_EN_PIN), SPEAKER_A_PA_EN_GPIODATA);
#endif

...
extern int8 g_smart_timer;
extern int8 g_smart_rec_buf_timer;
...
void __section__(".bank_var") _read_var(void)
{
    ...
    if((g_config_var.magic != VRAM_MAGIC(VM_AP_CONFIG)) || (g_config_var.ap_id == APP_FUNC_INVALID)
        ...
        g_config_var.for_dbp_flag = 0;
        g_config_var.ap_id = 0xFF;  //cann't use g_ap_switch_var.app_default_func_id, because it is inited after here
        g_config_var.engine_state = 0;  //init the engine state, fix this value have been set to random
        g_config_var.reserve[0] = 0x5689;
    ...
    if(((uint32) argc & 0xff) == SWITCH_ON) {
        ...
        if((act_readl(RTC_BAK0) & (1 << MY_RTC_S3BT)) != 0) {
            ...
            if(g_config_standby_exit == TRUE) {
                ...
                g_app_info_state_all.two_phone_mode = g_config_var.for_dbp_flag ;
                libc_print("startmode", g_config_var.for_dbp_flag, 2);
#endif
            }
        } else {
...
Reset_start:
        ...
        if((((uint32) argc & 0xff) == SWITCH_OFF) || (((uint32) argc & 0xff) == SWITCH_LOWPOWER)) {
            discrete_led_disable(LED_ID_BT_LINK, 1);
        }
#endif
        //解除静音模式
        com_set_mute(FALSE);
...
#ifdef ENABLE_DOUBLE_PHONE_
                g_config_var.for_dbp_flag = g_app_info_state_all.two_phone_mode;
                sys_vm_write(&g_config_var, VM_AP_CONFIG, sizeof(g_config_var_t));
                libc_print("entermode", g_config_var.for_dbp_flag, 2);
#endif
                result = sys_power_off(TRUE, FALSE);
                goto config_exit;
            }
        } else {
            g_config_var.esd_flag = FALSE;          //清除ESD标记
...
//#if defined(BOARD_JL_36410_FAR)
//                if(IS_EXTERN_CAHREGE_IN() && IS_EXTERN_BATTERY_IN())
//                    result = config_36410_charging_mode_loop();
//                result = sys_power_off(FALSE);
//#else
                result = sys_power_off(FALSE, isLowPower);
//#endif
            }
        }

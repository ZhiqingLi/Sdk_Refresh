...
typedef struct { //可用10.5k
    uint32 stub_buf[1024 / 4];          // 1k
    uint8 ppv_data[2048];               //缓存远端数据
    uint32 share_buf[(7 * 1024) / 4];   //7k buffer
    uint32 rev_buf[512 / 4];            //接受PC上参数
} sv_asqt_ram_16k_t;
...
typedef struct {
    Fifo_ctl_t ppv_fifo;        //缓存从PC获取的远端数据
    sv_aec_info_t AEC_info;
    sv_configs_t AsqtCfg;
    uint32 m4k_addr[2];         //dsp导出数据给m4k的地址
    uint32 asqt_volume;         //取值范围为0 ~ 31，默认值为15
} asqt_parm_16k_t;
...
extern sv_asqt_ram_16k_t *g_pAsqtRam_16k;

extern uint32 g_agc_aec_shm_ptr;


extern dsp_m4k_share_t *g_p_share_mem;
extern dsp_m4k_share_16k_t *g_p_share_mem_16k;

extern sv_prms_t g_dsp_init_parm;//存放dsp初始化参数
//extern sv_prms_t g_dsp_init_parm_16k;//存放dsp初始化参数

extern int8 g_stub_task_timer_id;//定时器id
extern int8 g_stub_task_timer_id_16k;//定时器id

extern asqt_parm_t g_asqt_parm;
extern asqt_parm_16k_t g_asqt_parm_16k;

extern asqt_peq_addr_t asqt_peq_addr;
//extern asqt_peq_addr_t asqt_peq_addr_16k;
extern uint8 g_real_call_mode_16k;
...
void get_asqt_peq_para_from_pc(asqt_peq_status_t *p_peq_status_t);
...
//extern void init_m4k_dsp_ctl_16k(uint8 flag);
extern sv_configs_t *asqt_set_dsp_init_parm_16k(void);
extern void reset_dsp_16k(uint8 call_mode);
//extern void stub_asqt_send_end_16k(uint8 cmd);
extern void deal_load_simu_mmm_16k(void);
//extern int stub_asqt_down_param_16k(uint32 *stub_buf);
//extern int stub_asqt_get_parameter_16k(uint32 *stub_buf, sv_configs_t *parm);
extern int  reset_aec_asqt_16k(void);
//extern void set_asrc_cpu_clk_16k(void);
extern void btcall_eg_asqt_deal_16k(uint8 stub_opened);
//
//
//extern uint32 stub_asqt_write_data_16k(uint16 cmd, void* buf, uint16 len);
extern void task_timer10ms_16k(void);
//extern void get_asqt_peq_para_from_pc_16k(asqt_peq_status_t* p_peq_status_t)

...
uint32 switch_stub_uart_port(int toUart)
{
    uint32 restore = act_readl(GPIOAOUTEN);
    
    restore = (act_readl(GPIOAOUTEN) >> 16) & 0x81;
    restore = (restore << 16) | ((act_readl(GPIOAINEN) >> 16) & 0x81);
    if(toUart)
    {
        act_writel((act_readl(GPIOAOUTEN) & ~(1 << 23 | 1 << 16)), GPIOAOUTEN);
        act_writel((act_readl(GPIOAINEN) & ~(1 << 23 | 1 << 16)), GPIOAINEN);
        reg_writel((act_readl(MFP_CTL0) & (~MFP_CTL0_GPIOA23_MASK)) | (6 << MFP_CTL0_GPIOA23_SHIFT), MFP_CTL0);
        reg_writel((act_readl(MFP_CTL0) & (~MFP_CTL0_GPIOA16_MASK)) | (1 << MFP_CTL0_GPIOA16_SHIFT), MFP_CTL0);
    }
    
    return restore;
}

void stub_uart_port_restore(uint32 restore)
{
    act_writel((act_readl(GPIOAOUTEN) | ((restore >> 16) & 0xFF)), GPIOAOUTEN);
    act_writel((act_readl(GPIOAINEN) | (restore & 0xFF)), GPIOAINEN);
}

//extern int8 g_smart_timer = -1;
//extern int8 g_smart_rec_buf_timer = -1;

bool globe_data_init_applib(void)
{
    uint8 config_file[12];
    uint8 i, ret;
    uint32 stub_uart_status;
...
//  g_smart_rec_buf_timer = -1;
    ...
    if(ret == TRUE) {
        ...
        if(uart_enable == TRUE) {
            ...
            reg_writel((act_readl(MFP_CTL1) & (~MFP_CTL1_GPIOA3_SHIFT)) | (5 << MFP_CTL1_GPIOA3_SHIFT), MFP_CTL1);
#endif
        }
    }
    ...
    stub_uart_status = switch_stub_uart_port(1);
    /* 检测连接状态 */
    ...
    stub_uart_port_restore(stub_uart_status);

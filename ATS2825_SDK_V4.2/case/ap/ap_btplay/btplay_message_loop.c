...
void __section__(".text.autoplay") btplay_autoplay_deal(void)
{
    ...
    if((g_last_btplay_status == BTPLAY_IDLE) && (g_btplay_cur_info.status != BTPLAY_IDLE)) {
        g_send_avrcp_play_tick = 0;
        g_fake_play_tick = 0;
        g_avrcp_autoplay_flag = 0;      //disable autoplay when bt connected
        g_avrcp_autoplay_try = 0;
...
app_result_e get_message_loop(void)
{
    ...
    while(1) {
        ...
        if(g_app_info_state_all.voice_recongnize_mode != 0) {   //语音识别过程中，暂时不响应系统消息
            if(result == RESULT_BLUETEETH_CALL_BACKGROUND || result == RESULT_POWER_OFF) {
                lp_key_record_stop();
            } else {
                result = RESULT_NULL;
            }

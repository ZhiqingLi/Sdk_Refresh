...
int32 analyse_power_val_valid(uint32 sample_cnt, uint32 *power_val_array, uint32 *p_power_val_left,
                              uint32 *p_power_val_right)
{
    uint32 i;
    uint32 power_val;
...
void act_test_report_channel_result(uint16 test_id, int32 ret_val, void *arg_buffer)
{
    ...
    if(g_test_mode != TEST_MODE_CARD) {
        ...
        int32_to_unicode(channel_test_arg->left_ch_power_threadshold, &(return_data->return_arg[trans_bytes]), &trans_bytes,
                         10);

        return_data->return_arg[trans_bytes++] = 0x002c;

        int32_to_unicode(channel_test_arg->right_ch_power_threadshold, &(return_data->return_arg[trans_bytes]), &trans_bytes,
                         10);

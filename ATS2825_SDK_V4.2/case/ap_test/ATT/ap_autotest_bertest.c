...
int32 ber_test_loop_deal(ber_test_arg_t *ber_arg, int channel)
{
    uint32 ber_val;
    uint32 rssi_val;
    uint32 old_state = 0;
            ...
            if(g_ber_control->rssi_test == TRUE) {
                ...
                if(ret_val == FALSE || (((rssi_val < ber_arg->rssi_thr_low) || (rssi_val > ber_arg->rssi_thr_high)))) {
                    ...
                    if(g_ber_control->test_cnt >= MAX_BER_RETRY_CNT) {
                        print_log("channel %d RSSI_test fail:%d", channel, rssi_val);
                        ret_val = FALSE;
                        break;

                    } else {
                        print_log("channel %d RSSI_val:%d cnt: %d", channel, rssi_val, g_ber_control->test_cnt);
                    ...
                    print_log("Channel %d RSSI_test success:%d", channel, rssi_val);
                ...
                if(ret_val == FALSE) {
                    //如果记录不足，要重新进行ber数值的获取
                    if(g_ber_control->test_cnt >= MAX_BER_RETRY_CNT) {
                        print_log("channel %d ber_test fail:%d", channel, ber_val);
                        ret_val = FALSE;
                        break;

                    } else {
                        print_log("channel %d ber_val:%d cnt: %d", channel, ber_val, g_ber_control->test_cnt);
                    ...
                    if(ber_val < ber_arg->ber_thr_low || ber_val > ber_arg->ber_thr_high) {
                        ...
                        print_log("channel %d ber_test success:%d", channel, ber_val);

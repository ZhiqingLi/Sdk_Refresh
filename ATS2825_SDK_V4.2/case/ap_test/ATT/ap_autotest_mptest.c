...
uint32 att_cfo_test_tx_begin(mp_test_arg_t *mp_arg, uint32 channel)
{
    uint32 ret_val;
...
retry:
    att_cfo_test_tx_begin(mp_arg, channel);
    ...
    */
    if(mp_arg->pwr_test == 1) {
        if((cfo_return_arg->pwr_val < mp_arg->pwr_threshold_low) ||
            ...
            print_log("PWR is wrong : %d", cfo_return_arg->pwr_val);
            ret_val = FALSE;
        }
    }
...
uint32 att_cfo_test(mp_test_arg_t *mp_arg, cfo_return_arg_t *cfo_return_arg, uint32 test_mode, uint32 channel)
{
    int32 cfo_val;
    int32 left;
    int32 right;
...
retry:
    while(left <= right) {
        ...
        ret_val = att_get_cfo_val(mid, mp_arg, cfo_return_arg, test_mode, channel);

...
void gui_box_display(void)
{
	...
	case GUI_BOX_FMFREQ:
		...
		break;
	case GUI_BOX_ALRAM_1:
		gui_display(DISP_ALARM_1);
		break;
	case GUI_BOX_ALRAM_2:
		gui_display(DISP_ALARM_2);
		break;
	case GUI_BOX_POWEROFF:
...
AT(.text.gui)
void gui_box_show_alarm_one(void)
{
    box_cb.sta = GUI_BOX_ALRAM_1;
...
AT(.text.gui)
void gui_box_show_alarm_two(void)
{
	box_cb.sta = GUI_BOX_ALRAM_2;
    box_cb.times = BOX_TIMES*2;
    box_cb.update = 1;
    gui_display(DISP_ALARM_2);
}
...
AT(.text.gui)
void gui_box_poweroff(void)
{
	box_cb.sta = GUI_BOX_POWEROFF;
    box_cb.times = BOX_TIMES;
    box_cb.update = 1;
    gui_display(DISP_OFF);
}

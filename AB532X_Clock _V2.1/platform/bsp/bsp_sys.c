...
void usr_tmr5ms_isr(void)
{
	...
	if((tmr5ms_cnt % 40) == 0){
		sys_cb.flush_200ms = !sys_cb.flush_200ms;
	}

    //500ms timer process
    if ((tmr5ms_cnt % 100) == 0) {
        sys_cb.cm_times++;
		sys_cb.flush_500ms = !sys_cb.flush_500ms;
		if(tm_cb.rtc_show_delay)
			tm_cb.rtc_show_delay--;
#if FUNC_CLOCK_EN
...
//#endif
	...
	user_io.sfr[GPIOxFEN] &= ~BIT(user_io.num);
	user_io.sfr[GPIOxDE] |= BIT(user_io.num);
	user_io.sfr[GPIOxDIR] |= BIT(user_io.num);
	user_io.sfr[GPIOxPD200K] |= BIT(user_io.num);

...
void ledseg_disp_alarm_1(void);
void ledseg_disp_alarm_2(void);
...
void ledseg_disp_bt(void)
{
...
#endif // FUNC_BTHID_EN
    {
		...
		if(bt_get_status() >= BT_STA_CONNECTED){
			ledseg_buf[4] |= ICON_BT;
		}else{
			if(sys_cb.flush_500ms){
				ledseg_buf[4] |= ICON_BT;
			}else{
				ledseg_buf[4] &= ~ICON_BT;
			}
...
AT(.text.display.ledseg)
void ledseg_disp_clock_mode(void)
{
	switch(func_cb.sta){
		case FUNC_BT:
			if(bt_get_status() >= BT_STA_CONNECTED){
				...
				if((sys_cb.flush_500ms) || (box_cb.sta == GUI_BOX_ALRAM_1) || (box_cb.sta == GUI_BOX_ALRAM_2) ||(tm_cb.setting == 1)){
					ledseg_buf[4] |= ICON_BT;
				}else{
					ledseg_buf[4] &= ~ICON_BT;
				}
...
void ledseg_disp_rtctime(void)
{
    ...
    if ((rtc_tm.tm_sec % 4 == 1) || (rtc_tm.tm_sec % 4 == 2)) {
        ledseg_buf[5] |= ICON_DDOT;
    }

	ledseg_disp_clock_mode();
#endif // FUNC_CLOCK_EN
}
...
{
    ledseg_buf[0] = ledseg_num_table[rtc_temp.tm_hour / 10];
    ledseg_buf[1] = ledseg_num_table[rtc_temp.tm_hour % 10];
    ledseg_buf[2] = ledseg_num_table[rtc_temp.tm_min / 10];
    ledseg_buf[3] = ledseg_num_table[rtc_temp.tm_min % 10];

	ledseg_buf[5] |= ICON_DDOT;
	if (sys_cb.flush_500ms) {
		if(tm_cb.rtc_show_delay == 0){
			if (tm_cb.index == 1) {				//分钟闪烁
				ledseg_buf[2] = 0x00;
				ledseg_buf[3] = 0x00;
			} else if (tm_cb.index == 2) {		//小时闪烁
				ledseg_buf[0] = 0x00;
				ledseg_buf[1] = 0x00;
			}
		}
...
void ledseg_disp_alarm_1(void)
{
    ledseg_buf[0] = ledseg_num_table[rtcalm_tm1.tm_hour / 10];
    ledseg_buf[1] = ledseg_num_table[rtcalm_tm1.tm_hour % 10];
    ledseg_buf[2] = ledseg_num_table[rtcalm_tm1.tm_min / 10];
    ledseg_buf[3] = ledseg_num_table[rtcalm_tm1.tm_min % 10];
	ledseg_buf[5] |= ICON_DDOT;
	ledseg_disp_clock_mode();
}
...
void ledseg_disp_alarm_2(void)
{
    ledseg_buf[0] = ledseg_num_table[rtcalm_tm2.tm_hour / 10];
    ledseg_buf[1] = ledseg_num_table[rtcalm_tm2.tm_hour % 10];
    ledseg_buf[2] = ledseg_num_table[rtcalm_tm2.tm_min / 10];
    ledseg_buf[3] = ledseg_num_table[rtcalm_tm2.tm_min % 10];
	ledseg_buf[5] |= ICON_DDOT;
	ledseg_disp_clock_mode();
}

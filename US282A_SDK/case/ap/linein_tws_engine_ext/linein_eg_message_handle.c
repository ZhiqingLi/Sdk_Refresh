...
void set_btstack_linein_ext(uint8 flag)
{
     msg_apps_t msg;
     //消息类型(即消息名称)
     msg.type = MSG_BTSTACK_TWS_LINEIN_EXT;
     //msg.content.addr = &used_len;
     msg.content.data[0] = flag;
     //发送同步消息
     send_sync_msg_btmanager(NULL, &msg, NULL, 0); 	
}

void set_start_time_param(void)
{

    //for sync start time set
    uint8 data[4]; 
    data[0] = TWS_SYNC_START_CNT;
    msg_apps_t msg;
    
    msg.content.addr = data;
    //消息类型(即消息名称)
    msg.type = MSG_BTSTACK_TWS_SET_START_TIME;
    
    //发送同步消息
    send_sync_msg_btmanager(NULL, &msg, NULL, 0);  

    set_notice_func = *(uint32*)( msg.content.addr);
}
/*
 此命令定义两tws设备之间的通讯通道.

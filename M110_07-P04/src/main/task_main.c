...
void GuiTaskEntrance(void)
{
	...
	else
	{
		...
		if(gSys.CurModuleID != MODULE_ID_WIFI)
		{
		   WiFiMicOff();					  //非WiFi模式关闭MIC拾音
		}
		else if(!gWiFi.MicMuteState)
		{
		   WiFiMicOn();					      //进入WiFi模式打开MIC
		}
#endif
#endif
...
//					SysClkDivison(1);	//ljg, call directly
			...
			&& (gSys.CurModuleID != MODULE_ID_BT_HF)) 
		{
			...
			else
			{
				...
				BP_SET_ELEMENT(pSysInfo->Volume, gSys.Volume);
			}
			APP_DBG("pSysInfo->CurModuleId:%d\n", pSysInfo->CurModuleId);
			APP_DBG("pSysInfo->CurSysVolume:%d\n", pSysInfo->Volume);

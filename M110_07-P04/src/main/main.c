...
#if defined(USE_POWERKEY_SOFT_PUSH_BUTTON) || defined(FUNC_GPIO_SOFT_PUSH_EN)
	LED_ALL_MODE_OFF();
#else
	LED_WHITE_MODE_ON();
#endif
...
//	SysGotoPowerDown();
	...
	OsSetDebugFlag(1);
	SwUartTxInit(SWUART_GPIO_PORT_C, 1, 115200);
...
//#endif
	...
	while(1)
	{
	    ...
	    if((!gSys.MuteFlag && gSys.CurModuleID != MODULE_ID_IDLE)
#ifdef FUNC_SOUND_REMIND

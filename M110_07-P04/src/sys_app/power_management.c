...
//以下定义不同的电压检测事件的触发电压(单位mV)，用户根据自身系统电池的特点来配置
   ...
   #define LDOIN_VOLTAGE_LOW			2400
   #define LDOIN_VOLTAGE_OFF			2300	//注意：这里是ADC值，Vol=log/4095*3.41,单位V
#else
...
static bool PowLowDetFlag = FALSE;
#endif
...
//#else
    ...
    //电池电量低报警	
		...
		else if(LdoinLevelAverage > LDOIN_VOLTAGE_MID || !PowLowDetFlag)
		{
...
#define SLIDE_SWITCH_OFFTIME 3000
#define SLIDE_SWITCH_ONTIME 1000
...
#ifdef FUNC_GPIO_SOFT_PUSH_EN
			if(gSys.CurModuleID == MODULE_ID_IDLE)
			{
				gSys.NextModuleID = MODULE_ID_WIFI;
				if(gSys.NextModuleID >= MODULE_ID_USER_PLUS)
				{
					gSys.NextModuleID = MODULE_ID_WIFI;
				}
				APP_DBG("PowerKey  On CurModuleId = %d\n",gSys.NextModuleID);
	...
	else
	{
		if(gSys.CurModuleID != MODULE_ID_IDLE)
		{
			slide_switch_pd_cnt = SLIDE_SWITCH_OFFTIME;
		}
		else
		{
	...
	#define SOFT_SWITCH_OFFTIME 3000
	static uint16_t  soft_switch_pd_cnt = SOFT_SWITCH_OFFTIME;
		...
		)
	{
		...
		if(gSys.NextModuleID != MODULE_ID_POWEROFF && !PowerOffFlag 
			&& (soft_switch_pd_cnt-- == 0))	/*MSG_COMMON_CLOSE only need send once*/
		{	
			/*if use push button, send message, for task's saving info.*/
	...
	{
		soft_switch_pd_cnt = SOFT_SWITCH_OFFTIME;
	}
#endif	

...
#if defined(USE_POWERKEY_SOFT_PUSH_BUTTON) && defined(FUNC_POWERKEY_SOFT_POWERON_EN)
	if(gWakeUpFlag == WAKEUP_FLAG_POR_POWER || gWakeUpFlag == WAKEUP_FLAG_POR_POWERKEY)
	{
		TIMER	WaitTimer;
		
		TimeOutSet(&WaitTimer, 1000);
		...
		gSys.CurModuleID = MODULE_ID_WIFI;
		APP_DBG("Powerkey timeout, will PowerOn\n");
	}
#endif

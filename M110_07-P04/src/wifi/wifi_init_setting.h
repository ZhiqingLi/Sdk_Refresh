...
//****************************************************************************************
//					             客户项目产品的相关定制参数选择
//        根据客户不同要求，会有功能及认证相关参数差异，由以下宏定义区别设置
//****************************************************************************************
//项目定义：M110或者M120,只能打开其中一个
//#define  FUNC_PROJECT_NAME_M110_EN
#define  FUNC_PROJECT_NAME_M120_EN

//客户名称定义：只能打开其中任意一个宏定义
//#define  FUNC_NARUI_M110_EN 
//#define  FUNC_NARUI_M120_EN
//#define  FUNC_MK_COLORS_RHYTHM_EN
//#define  FUNC_SOUNDBOT_SB600_EN
#define  FUNC_MXQ_MXQ_HF30_EN
//#define  FUNC_JIUSHENG_S001_EN
//#define	 FUNC_KNOWBEST_D2_EN
#ifdef 	FUNC_JIUSHENG_S001_EN
//	#define FUNC_BAIDU_SCTEL_PRJECT_EN           //玖胜百度四川电信项目
	#define FUNC_BAIDU_HNTEL_PRJECT_EN           //玖胜百度湖南电信项目
#endif
//****************************************************************************************
//					WiFi产品的相关定制参数设置
//****************************************************************************************
#if defined(FUNC_NARUI_M110_EN)
  //**MCU+CAP+PRJproject_name&:项目名称定义
	#define MCU_CAP_PRJ_PROJECT_NAME	"M110"
	//**MCU+SID+name&:设置设备默认SSID名称
	#ifdef FUNC_WIFI_DUMI_PROJECT_EN
		#define MCU_SSID_NAME	"DuerOS_M110"
	#else
		#define MCU_SSID_NAME	"M110"
	#endif
	
#elif defined(FUNC_NARUI_M120_EN)
  //**MCU+CAP+PRJproject_name&:项目名称定义
	#define MCU_CAP_PRJ_PROJECT_NAME	"M120"
	//**MCU+SID+name&:设置设备默认SSID名称
	#ifdef FUNC_WIFI_DUMI_PROJECT_EN
		#define MCU_SSID_NAME	"DuerOS_M120"
	#else
		#define MCU_SSID_NAME	"M110"
	#endif
	
#elif defined(FUNC_MK_COLORS_RHYTHM_EN)
  //**MCU+CAP+PRJproject_name&:项目名称定义
	#define MCU_CAP_PRJ_PROJECT_NAME	"M110"
	//**MCU+SID+name&:设置设备默认SSID名称
	#define MCU_SSID_NAME	"MK_Colors Rhythm"
	
#elif defined(FUNC_SOUNDBOT_SB600_EN)
	//**MCU+CAP+PRJproject_name&:项目名称定义
	#define MCU_CAP_PRJ_PROJECT_NAME	"SB600"
	//**MCU+SID+name&:设置设备默认SSID名称
	#define MCU_SSID_NAME	"SoundBot-SB600"
	
#elif defined(FUNC_MXQ_MXQ_HF30_EN)
  //**MCU+CAP+PRJproject_name&:项目名称定义
	#define MCU_CAP_PRJ_PROJECT_NAME	"MXQ_HF30"
	//**MCU+SID+name&:设置设备默认SSID名称
	#define MCU_SSID_NAME	"MXQ_HF30"

#elif defined(FUNC_JIUSHENG_S001_EN)
	#if defined(FUNC_BAIDU_SCTEL_PRJECT_EN)
		//**MCU+CAP+PRJproject_name&:项目名称定义
		#define MCU_CAP_PRJ_PROJECT_NAME    "JS_S002"
		//**MCU+SID+name&:设置设备默认SSID名称
		#define MCU_SSID_NAME   "JS02"
	#elif defined(FUNC_BAIDU_HNTEL_PRJECT_EN)
		//**MCU+CAP+PRJproject_name&:项目名称定义
		#define MCU_CAP_PRJ_PROJECT_NAME    "JS_S003"
		//**MCU+SID+name&:设置设备默认SSID名称
		#define MCU_SSID_NAME   "JS03"
	#else
		//**MCU+CAP+PRJproject_name&:项目名称定义
		#define MCU_CAP_PRJ_PROJECT_NAME    "JS_S001"
		//**MCU+SID+name&:设置设备默认SSID名称
		#define MCU_SSID_NAME   "JS01"
	#endif
#elif defined(FUNC_KNOWBEST_D2_EN)
	//**MCU+CAP+PRJproject_name&:项目名称定义
	#define MCU_CAP_PRJ_PROJECT_NAME	"KnowBest_D2"
	//**MCU+SID+name&:设置设备默认SSID名称
	#define MCU_SSID_NAME	"KnowBest_D2"
...
//如果这是一个阿里小智的项目，请回复此项，productid来自阿里提供的prodconf.json文件里面model字段，例如：EDIFIER_ENTERTAINMENT_ATALK_MA1
#if defined(FUNC_MK_COLORS_RHYTHM_EN)
	#define MCU_ALI_PID_PRODUCTID	"FREESON_ENTERTAINMENT_ATALK_MK_"
#elif defined(FUNC_NARUI_M110_EN)
	#define MCU_ALI_PID_PRODUCTID	"NARUI_ENTERTAINMENT_ATALK_M110"	
#elif defined(FUNC_NARUI_M120_EN)
	#define MCU_ALI_PID_PRODUCTID	"NARUI_ENTERTAINMENT_ATALK_M120"
#endif
...
#if defined(FUNC_PROJECT_NAME_M110_EN)
#define MCU_CAP_PLM_BIT1	1	//有无Aux-In，默认0（如果App不需要显示当前处于Aux-In状态，例如切换到Aux状态WiFi已经关闭，则此处不需要回复1）
#elif defined(FUNC_PROJECT_NAME_M120_EN)
#define MCU_CAP_PLM_BIT1	0	//有无Aux-In，默认0（如果App不需要显示当前处于Aux-In状态，例如切换到Aux状态WiFi已经关闭，则此处不需要回复1）
#endif

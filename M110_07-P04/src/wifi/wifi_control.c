...
void WiFiSetMcuVolume(uint8_t Vol)
{
	...
	if((gWiFi.MicState == 2) || (gWiFi.MicState == 3))
	{
		WiFiSetMicState(0);
		SoundRemind(SOUND_UPDATA_SUCC);
	}
#endif
}
  ...
  if((gWiFi.MicState != FALSE) || (gWiFi.VisInfState == TRUE) || gWiFi.MicMuteState)
	{
		...
		else
		{
			...
			{
				TimeOutSet(&WiFiLedBlinkTimer, 200);
				...
				{
					TimeOutSet(&WiFiLedBlinkTimer, 2000);
					LedBlinkCnt = 0;
					LED_RED_MODE_ON();
				}
			}

...
extern uint16_t GpioPowerKeyScan(void);
...
//有些功放Unmute有延时，按键Beep beep声适当延时后在播放
	...
	{	
        ...
        if(((BeepKeyMsg != MSG_MODE) && (PastTimeGet(&BeepDelayMuteTimer) >= 300))
	...
	if(Msg != MSG_NONE)
	{
#ifdef FUNC_WIFI_ALI_PROJECT_EN	      //阿里项目，配网中禁止所有按键功能
		if(((gWiFi.WPSState == WIFI_STATUS_WPS_SCANNING) && (Msg != MSG_WIFI_WPS))

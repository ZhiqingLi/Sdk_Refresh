...
#define BT_SCO_FAR_NOISE_THRESHOID      0x600  //远端环境噪声阈值（低于此阈值便认为是噪声）(0~65535)
...
#define FMRX_RSSI_DEBUG                 0                               //打印每个台的RSSI信息到RAM中

#define FMRX_DNR_EN                     0                               //FM动态降噪使能
#define FMRX_MANUAL_GAIN_EN             0                               //FM手动放大，放大同时会把左右声道反向一下
#define FMRX_MANUAL_GAIN                FM_DIG_P7DB
/*****************************************************************************
 * Module    : FMTX功能配置
 *****************************************************************************/
#define FMTX_EN                         0   //是否使能FM TX功能
#define FMTX_POWER                      1   //发射功率选择，1：大， 0：小
#define FMTX_DAC_EN                     0   //是否打开DAC
...
#define AUX_2_MIC_CH                    0                           //AUX通过MIC通道进入,实现AUX直通音量可调  //AUX需要在外部衰减如(-30DB),再利用MIC前级0~23共24级可调(-6~63DB,step_3db)实现。
///通过配置工具选择检测GPIO
...
#define MAGIC_REC_PLAY                   0           //魔音播放

#define SPIFLASH_ID                      0x40150000  //通过读ID判断FLASH是否在线, 需要改成SPIFLASH对应的ID
#define SPIFALSH_BAUD                    (500000*5)    //SPI波特率500K
...
#define WARING_MAXVOL_FORMAT            WARING_TONE //最大音量提示音可以配置为 WARING_NO(0) 或 WARING_WAV 或 WARING_TONE

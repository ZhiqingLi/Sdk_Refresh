...
void wav_res_play_kick(u32 addr, u32 len,wavres_cb_t *wavres_cb);
void wav_res_dec_process(wavres_cb_t *wavres_cb);
bool wav_res_is_play(wavres_cb_t *wavres_cb);
bool wav_res_stop(wavres_cb_t *wavres_cb);
...
#if		1
		//å·²ç»æœ€å¤§éŸ³é‡æ—¶ï¼Œæç¤ºéŸ³ä¸­å¿½ç•¥éŸ³é‡æŒ‰é”®
...
void wav_res_play(u32 addr, u32 len, wavres_cb_t *wavres_cb)
{
    ...
    dac_dnr_set_sta(0);
#endif
    bsp_aux_wav_res_start();       //AUX直通时播放WAV提示音需要特殊处理下
    wav_res_play_kick(addr, len, wavres_cb);
    while (wav_res_is_play(wavres_cb)) {
        wav_res_dec_process(wavres_cb);
        WDT_CLR();
    }
    wav_res_stop(wavres_cb);
    bsp_aux_wav_res_exit();
#if DAC_DNR_EN
    dac_dnr_set_sta(sta);
#endif

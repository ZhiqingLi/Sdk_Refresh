...
void audio_path_init(u8 path_idx)
{
    ...
    if (path_idx == AUDIO_PATH_AUX) {
        #if AUX_2_MIC_CH        //AUX直通走MIC通道，MIC通道则走AUX.
    ...
    if (path_idx == AUDIO_PATH_BTMIC || path_idx == AUDIO_PATH_KARAOK) {
        ...
        #if AUX_2_MIC_CH        //MIC通道则走AUX. AUX直通走MIC通道，
...
void aux2mic_path_init(u8 enable)  //AUX2MIC 直通配置
{
    printf("AUX_2_MIC_CH\n");
    if (enable) {
        analog_mic2pa_start(btmic_ch_tbl[xcfg_cb.bt_ch_mic], 0);
        aux2mic_set_gain(BT_ANL_GAIN);
    } else {
        aux2mic_set_gain(0);
    }
}

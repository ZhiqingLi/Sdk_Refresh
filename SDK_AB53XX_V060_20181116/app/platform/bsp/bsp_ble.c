...
const struct att_hdl_t att_hdl_tbl[BLE_IDX_END] = {
    ...
    [BLE_IDX_OLD_APP]   = {0x0009,  1},
    [BLE_IDX_APP_SEND]  = {0x000c,  0},
    [BLE_IDX_APP_REV]   = {0x000e,  1},
};
...
bool ble_send_packet(u8 *buf, u8 len)
{
    ...
    printf("BLE TX:");
    print_r(buf, len);
    return le_tx_notify(BLE_IDX_APP_REV, buf, len);
}
...
void bsp_ble_process(void)
{
    ...
    if (handle == 0x0009) {             //兼容旧版APP
        bsp_app_0_process(ptr, len);
    }
#if BT_APP_EN
    else if (handle == 0x000C) {
        bsp_app_process(ptr, len);
    }
#endif
}
#endif

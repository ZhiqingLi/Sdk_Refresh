...
static int codec_write(uint8_t reg_addr, uint8_t value)
{
    ...
    return hal_i2c_send(HAL_I2C_ID_0, TLV32AIC32_I2C_ADDRESS, buf, 1, 1, 0, 0);
}
...
static uint8_t codec_read(uint8_t reg_addr)
{
    ...
    hal_i2c_recv(HAL_I2C_ID_0, TLV32AIC32_I2C_ADDRESS, buf, 1, 1, HAL_I2C_RESTART_AFTER_WRITE, 0, 0);
...
uint32_t tlv32aic32_stream_setup(enum AUD_STREAM_T stream, struct tlv32aic32_config_t *cfg)
{
    ...
    /* adc flag : 36 */
    //TRACE("tlv adc flag 0x%x", codec_read(ADC_FLAG));
    /* left adc pga mute : 15 */
    //TRACE("tlv left adc pga 0x%x", codec_read(LADC_VOL));
    /* right adc pga mute : 16 */
    //TRACE("tlv right adc pga 0x%x", codec_read(RADC_VOL));
    /* input end */

...
void analog_aud_get_dc_calib_value(int16_t *dc_l, int16_t *dc_r)
{
    ...
    static const int16_t SE_MONO_COMPENSATE = -35;
    bool se_mono = false;
#endif
    ...
    static const uint16_t DC_VALUE_MASK = 0x01FF;
    static const uint16_t DC_VALUE_SIGN = 0x0200;
#else
...
#ifdef AUDIO_OUTPUT_DC_CALIB_ANA
    *dc_l = efuse & (DC_VALUE_SIGN | DC_VALUE_MASK);
#else
    dc.reg = efuse;
    *dc_l = dc.val.dc;
#endif
...
#ifdef AUDIO_OUTPUT_DC_CALIB_ANA
    *dc_r = efuse & (DC_VALUE_SIGN | DC_VALUE_MASK);
#else
    dc.reg = efuse;
    *dc_r = dc.val.dc;
#endif
...
#ifdef  EXTERN_PA_ENABLE 
	app_extern_pa_onoff(en);
	osDelay(10);
#endif

#ifdef __ANKER_UI__
    hal_pa_output_ctrl(en ? 1: 0);
#endif
}

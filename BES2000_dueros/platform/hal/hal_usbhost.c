...
#define USBHOST_TRACE(mask, str, ...)           { if (usbhost_trmask & (1 << mask)) { TRACE(str, ##__VA_ARGS__); } }
...
static const uint32_t usbhost_trmask = (1 << 0); //~0; //(1 << 3) | (1 << 4);
...
void hal_usbhost_halt_chan(uint8_t chan)
{
    ...
    USBHOST_TRACE(17, "%s: %d", __FUNCTION__, chan);
...
void hal_usbhost_port_reset(uint32_t ms)
{
    ...
    USBHOST_TRACE(14, "%s: %d", __FUNCTION__, ms);
...
int hal_usbhost_get_chan(uint8_t *chan)
{
    ...
    USBHOST_TRACE(13, "%s: %d", __FUNCTION__, i);
    ...
    *chan = i;
    ...
    if (usbhost_timer[i] == NULL) {
        USBHOST_TRACE(0, "%s: WARNING: Failed to alloc hwtimer for chan=%d", __FUNCTION__, i);
...
{
    USBHOST_TRACE(12, "%s: %d", __FUNCTION__, chan);
...
{
    USBHOST_TRACE(11, "%s: chan=%d mps=%d ep=%d in=%d type=%d addr=%d",
...
{
    USBHOST_TRACE(10, "%s: chan=%d dev_addr=%d", __FUNCTION__, chan, dev_addr);
...
{
    USBHOST_TRACE(9, "%s: chan=%d mps=%d", __FUNCTION__, chan, mps);
...
int hal_usbhost_start_xfer(uint8_t chan, const struct HAL_USBHOST_XFER_T *xfer)
{
    ...
    USBHOST_TRACE(7, "%s: chan=%d size=%u mc=%d pid=%d",
...
static void hal_usbhost_setup_xfer_handler(uint8_t chan, uint8_t *buf, uint32_t len, enum HAL_USBHOST_XFER_ERR_T error)
{
    ...
    USBHOST_TRACE(6, "%s: chan=%d cur=%d next=%d error=%d",
        ...
        case HAL_USBHOST_SETUP_STATUS_OUT:
            if (cur_setup_stage == HAL_USBHOST_SETUP_DATA_IN) {
                if (setup_xfer.setup_pkt.wLength != len) {
                    USBHOST_TRACE(0, "Invalid setup data length: %d expect=%d", len, setup_xfer.setup_pkt.wLength);
                    // Update received len
...
int hal_usbhost_start_setup_xfer(const struct HAL_USBHOST_SETUP_XFER_T *setup, uint32_t *recv_len)
{
    ...
    USBHOST_TRACE(6, "%s: out=%d in=%d type=0x%02x req=0x%02x wlen=%d next_stage=%d",
...
static void hal_usbhost_port_handler(void)
{
    ...
    USBHOST_TRACE(4, "%s: 0x%08x", __FUNCTION__, prt);
...
static void hal_usbhost_retry_chan(uint8_t chan, uint32_t size)
{
    ...
    USBHOST_TRACE(20, "%s: chan=%d size=%u", __FUNCTION__, chan, size);
...
static void hal_usbhost_chan_n_handler(uint8_t chan)
{
    ...
    USBHOST_TRACE(3, "%s: %d", __FUNCTION__, chan);
    ...
    USBHOST_TRACE(18, "%s: chan=%d HCINTn=0x%08x xfer=%u", __FUNCTION__, chan, raw_irq, xfer);
    ...
    if (error == HAL_USBHOST_XFER_ERR_QTY) {
        ...
        USBHOST_TRACE(19, "%s: Got unknown IRQ chan=%d irq=0x%08x", __FUNCTION__, chan, irq);
    } else {
        // Stop xfer timer
        hwtimer_stop(usbhost_timer[chan]);
...
static void hal_usbhost_irq_handler(void)
{
    ...
    USBHOST_TRACE(1, "%s: 0x%08x", __FUNCTION__, status);
...
static void hal_usbhost_timeout(void *param)
{
    ...
    USBHOST_TRACE(21, "%s: %d", __FUNCTION__, chan);
...
{
    USBHOST_TRACE(24, "%s: %d", __FUNCTION__, status);

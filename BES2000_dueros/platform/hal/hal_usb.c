...
void hal_usb_unstall_ep(enum EP_DIR dir, uint8_t ep)
{
    ...
    if (dir == EP_OUT) {
        _disable_out_ep(ep, USBC_SNAK, USBC_STALL);
    } else {
        _disable_in_ep(ep, USBC_SNAK, USBC_STALL);
    }
}
...
#ifdef USB_ISO
#define RXFIFOSIZE      ((4 * CTRL_EPNUM + 6) + 1 + (2 * (USB_MAX_PACKET_SIZE_ISO_DRV / 4 + 1)) + (EPNUM * 2))
#else
...
#ifdef USB_HIGH_SPEED
uint32_t hal_usb_calc_ep_mps(uint32_t pkt_size)
{
...
void hal_usb_activate_epn(enum EP_DIR dir, uint8_t ep, uint8_t type, uint16_t mps)
{
    ...
    if (dir == EP_OUT) {
        ...
        if (type == E_ISOCHRONOUS || type == E_INTERRUPT) {
            if (mps > USB_MAX_PACKET_SIZE_ISO_DRV) {
                fifo_mps = USB_MAX_PACKET_SIZE_ISO_DRV;
            }
            mps = hal_usb_calc_ep_mps(mps);
        }
#endif
        // Stop ep in
...
void hal_usb_update_recv_epn_mps(uint8_t ep, uint16_t mps)
{
    uint8_t type;
    ...
    if (usbc->DOEPnCONFIG[ep - 1].DOEPCTL & USBC_USBACTEP) {
...
static int hal_usb_ep0_setup_stage(uint32_t statusEp)
{
    ...
    ASSERT(ep0_transfer.setup_pkt.wLength || ep0_transfer.setup_pkt.bmRequestType.direction == EP_OUT,
        "Ep0 dir should be out if wLength=0");
...
{
    USB_TRACE(18, "[%X] %s: DSTS=0x%08x", hal_sys_timer_get(), __FUNCTION__, usbc->DSTS);
...
static void hal_usb_sleep(enum USB_SLEEP_T cause)
{
    ...
    if (callbacks.state_change) {
        callbacks.state_change((cause == USB_SLEEP_SUSPEND) ? HAL_USB_EVENT_SUSPEND : HAL_USB_EVENT_L1_DEEP_SLEEP);
    }

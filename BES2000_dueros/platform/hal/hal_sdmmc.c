...
static void sdmmcip0_ext_dma_rx_handler(uint8_t chan, uint32_t remain_tsize, uint32_t error, struct HAL_DMA_DESC_T *lli)
{
    ...
    HAL_SDMMC_TRACE("%s:%d\n", __func__, __LINE__);
    ip_raw_int_status = sdmmcip_readl(host, SDMMCIP_REG_RINTSTS);
    HAL_SDMMC_TRACE("%s:%d, ip_raw_int_status 0x%x\n", __func__, __LINE__, ip_raw_int_status);
    if (ip_raw_int_status & (SDMMCIP_REG_DATA_ERR|SDMMCIP_REG_DATA_TOUT)) {
        HAL_SDMMC_TRACE("%s:%d, sdmmcip0 rx dma error 0x%x\n", __func__, __LINE__, ip_raw_int_status);
    }
                   ...
                   void *bounce_buffer)
{
    ...
    if (data->flags & MMC_DATA_READ) {
        HAL_SDMMC_TRACE("sdmmc use dma read\n");
        dma_cfg.dst = (uint32_t)data->dest;
        //HAL_SDMMC_TRACE("sddma :len %d, buf 0x%x\n", data->blocksize*data->blocks, dma_cfg.dst);
        if ((dma_cfg.dst % 4) != 0) {
        ...
        HAL_SDMMC_TRACE("sdmmc use dma tsize %d\n", dma_cfg.src_tsize);
        ...
        HAL_SDMMC_TRACE("sdmmc use dma get ch %d\n", dma_cfg.ch);
    }
    else {
        HAL_SDMMC_TRACE("sdmmc use dma write\n");
        ...
        struct mmc_data *data)
{
    ...
    if (mask & SDMMCIP_REG_INTMSK_RTO) {
         ...
         */
        HAL_SDMMC_TRACE("%s: Response Timeout.\n", __func__);
        return TIMEOUT;
    } else if (mask & SDMMCIP_REG_INTMSK_RE) {
        HAL_SDMMC_TRACE("%s: Response Error.\n", __func__);
        return -1;
    }
    ...
    if (data) {
        ...
        while(1) {
            ...
            if (mask & (SDMMCIP_REG_DATA_ERR | SDMMCIP_REG_DATA_TOUT)) {
                HAL_SDMMC_TRACE("%s: READ DATA ERROR!\n", __func__);
                ret = -1;
                goto out;
            }
            ...
            if (data->flags == MMC_DATA_READ) {
                if (status & SDMMCIP_REG_FIFO_COUNT_MASK) {
                    ...
                    //HAL_SDMMC_TRACE("%s: count %d, read -> 0x%x\n", __func__, i, fifo_data);
                    ...
                    if (i < data->blocks*data->blocksize) {
                        ...
                        HAL_SDMMC_TRACE("%s: fifo data too much\n", __func__);
                        ret = -1;
                        goto out;
                    }
                }
                /* nothing to read from fifo and DTO is set */
                    ...
                    if(i != data->blocks*data->blocksize) {
                        HAL_SDMMC_TRACE("%s: need to read %d, actually read %d\n", __func__, data->blocks*data->blocksize, i);
                    }
                    ret = 0;
                    goto out;
                }
                ...
                if(mask & SDMMCIP_REG_INTMSK_DTO) {
                    ...
                    if(i != data->blocks*data->blocksize) {
                        HAL_SDMMC_TRACE("%s: need to write %d, actually written %d\n", __func__, data->blocks*data->blocksize, i);
                    }
                    ret = 0;
                    goto out;
                }
	...
	if (data) {
        ...
        if (data->flags & MMC_DATA_READ) {
            for (int ccc = 0; ccc < 400; ++ccc) {
                HAL_SDMMC_TRACE("%d-0x%x ", ccc, data->dest[ccc]);
            }
            HAL_SDMMC_TRACE("\n");
        }
#endif
...
static int sdmmcip_setup_bus(struct sdmmcip_host *host, uint32_t freq)
{
    ...
    if (host->get_mmc_clk)
        ...
        HAL_SDMMC_TRACE("%s: Didn't get source clock value.\n", __func__);
        return -EINVAL;
    }
    ...
    HAL_SDMMC_TRACE("%s : freq %d, sclk %ld, div %d\n", __func__, freq, sclk, div);
...
static void sdmmcip_set_ios(struct mmc *mmc)
{
    ...
    HAL_SDMMC_TRACE("Buswidth = %d, clock: %d\n", mmc->bus_width, mmc->clock);
...
int mmc_send_cmd(struct mmc *mmc, struct mmc_cmd *cmd, struct mmc_data *data)
{
    ...
    HAL_SDMMC_TRACE("CMD_SEND:%d\n", cmd->cmdidx);
    HAL_SDMMC_TRACE("\t\tARG\t\t\t 0x%08X\n", cmd->cmdarg);
    ret = mmc->cfg->ops->send_cmd(mmc, cmd, data);
    HAL_SDMMC_TRACE("send cmd end...");
    switch (cmd->resp_type) {
        case MMC_RSP_NONE:
            HAL_SDMMC_TRACE("\t\tMMC_RSP_NONE\n");
            break;
        case MMC_RSP_R1:
            HAL_SDMMC_TRACE("\t\tMMC_RSP_R1,5,6,7 \t 0x%08X \n",
                    cmd->response[0]);
            break;
        case MMC_RSP_R1b:
            HAL_SDMMC_TRACE("\t\tMMC_RSP_R1b\t\t 0x%08X \n",
                    cmd->response[0]);
            break;
        case MMC_RSP_R2:
            HAL_SDMMC_TRACE("\t\tMMC_RSP_R2\t\t 0x%08X \n",
                    cmd->response[0]);
            HAL_SDMMC_TRACE("\t\t          \t\t 0x%08X \n",
                    cmd->response[1]);
            HAL_SDMMC_TRACE("\t\t          \t\t 0x%08X \n",
                    cmd->response[2]);
            HAL_SDMMC_TRACE("\t\t          \t\t 0x%08X \n",
                    cmd->response[3]);
            HAL_SDMMC_TRACE("\n");
            HAL_SDMMC_TRACE("\t\t\t\t\tDUMPING DATA\n");
            for (i = 0; i < 4; i++) {
                int j;
                HAL_SDMMC_TRACE("\t\t\t\t\t%03d - ", i*4);
                ptr = (uint8_t *)&cmd->response[i];
                ptr += 3;
                for (j = 0; j < 4; j++)
                    HAL_SDMMC_TRACE("%02X ", *ptr--);
                HAL_SDMMC_TRACE("\n");
            }
            break;
        case MMC_RSP_R3:
            HAL_SDMMC_TRACE("\t\tMMC_RSP_R3,4\t\t 0x%08X \n",
                    cmd->response[0]);
            break;
        default:
            HAL_SDMMC_TRACE("\t\tERROR MMC rsp not supported\n");
            break;
    }
    return ret;
}
        ...
        uint32_t blkcnt, const void *src)
{
    ...
    if (mmc_send_cmd(mmc, &cmd, &data)) {
        HAL_SDMMC_TRACE("mmc write failed\n");
        return 0;
    }
    ...
    if (!sdmmc_host_is_spi(mmc) && blkcnt > 1) {
        ...
        cmd.resp_type = MMC_RSP_R1b;
        if (mmc_send_cmd(mmc, &cmd, NULL)) {
            HAL_SDMMC_TRACE("mmc fail to send stop cmd\n");
            return 0;
        }
    }
...
static int mmc_send_ext_csd(struct mmc *mmc, uint8_t *ext_csd)
{
    ...
    HAL_SDMMC_TRACE("[%s] start...", __func__);
    err = mmc_send_cmd(mmc, &cmd, &data);
    HAL_SDMMC_TRACE("[%s] err = %d", __func__, err);
...
static int mmc_startup(struct mmc *mmc)
{
    ...
    if ((mmc->dsr_imp) && (0xffffffff != mmc->dsr)) {
        ...
        cmd.resp_type = MMC_RSP_NONE;
        if (mmc_send_cmd(mmc, &cmd, NULL)) {
            HAL_SDMMC_TRACE("MMC: SET_DSR failed\n");
        }
    }
    ...
    sprintf(mmc->block_dev.vendor, "Man %06lx Snr %04lx%04lx",
    ...
    sprintf(mmc->block_dev.product, "%c%c%c%c%c%c", (char)mmc->cid[0],
            ...
            (char)(mmc->cid[2] >> 24));
    sprintf(mmc->block_dev.revision, "%ld.%ld", (mmc->cid[2] >> 20) & 0xf,
            (mmc->cid[2] >> 16) & 0xf);
#else
...
int mmc_start_init(struct mmc *mmc)
{
    ...
    if (mmc_getcd(mmc) == 0 || mmc->cfg->ops->init == NULL) {
        mmc->has_init = 0;
        HAL_SDMMC_TRACE("%s : no card present\n", __func__);
        return NO_CARD_ERR;
    }
    ...
    if (err == TIMEOUT) {
        ...
        if (err) {
#if !defined(CONFIG_SPL_BUILD) || defined(CONFIG_SPL_LIBCOMMON_SUPPORT)
            HAL_SDMMC_TRACE("%s : Card did not respond to voltage select!\n", __func__);
#endif
            return UNUSABLE_ERR;
        }
    }
...
void hal_sdmmc_irq_handler(void)
{
	...
	if (mask & SDMMCIP_REG_INTMSK_CD){
		...
		HAL_SDMMC_TRACE("%s cdetect:%d mask:%x\n", __func__, cdetect, mask);
...
void hal_sdmmc_dump(enum HAL_SDMMC_ID_T id)
{
    ...
    HAL_SDMMC_TRACE("-----------hal_sdmmc_dump-------------\n");
    HAL_SDMMC_TRACE("    [sdmmc id]                       : %d\n", id);
    HAL_SDMMC_TRACE("    [io register address]            : 0x%x\n", sdmmc_ip_base[id]);
    HAL_SDMMC_TRACE("    [ddr mode]                       : %d\n", mmc->ddr_mode);
    switch(mmc->version)
    {
        case SD_VERSION_1_0:  HAL_SDMMC_TRACE("    [version]                        : SD_VERSION_1_0\n");  break;
        case SD_VERSION_1_10: HAL_SDMMC_TRACE("    [version]                        : SD_VERSION_1_10\n"); break;
        case SD_VERSION_2:    HAL_SDMMC_TRACE("    [version]                        : SD_VERSION_2\n");    break;
        case SD_VERSION_3:    HAL_SDMMC_TRACE("    [version]                        : SD_VERSION_3\n");    break;
        case MMC_VERSION_5_0: HAL_SDMMC_TRACE("    [version]                        : MMC_VERSION_5_0\n"); break;
        case MMC_VERSION_4_5: HAL_SDMMC_TRACE("    [version]                        : MMC_VERSION_4_5\n"); break;
        case MMC_VERSION_4_41:HAL_SDMMC_TRACE("    [version]                        : MMC_VERSION_4_41\n");break;
        case MMC_VERSION_4_3: HAL_SDMMC_TRACE("    [version]                        : MMC_VERSION_4_3\n"); break;
        case MMC_VERSION_4_2: HAL_SDMMC_TRACE("    [version]                        : MMC_VERSION_4_2\n"); break;
        case MMC_VERSION_4_1: HAL_SDMMC_TRACE("    [version]                        : MMC_VERSION_4_1\n"); break;
        case MMC_VERSION_4:   HAL_SDMMC_TRACE("    [version]                        : MMC_VERSION_4\n");   break;
        case MMC_VERSION_3:   HAL_SDMMC_TRACE("    [version]                        : MMC_VERSION_3\n");   break;
        case MMC_VERSION_2_2: HAL_SDMMC_TRACE("    [version]                        : MMC_VERSION_2_2\n");   break;
        case MMC_VERSION_1_4: HAL_SDMMC_TRACE("    [version]                        : MMC_VERSION_1_4\n");   break;
        case MMC_VERSION_1_2: HAL_SDMMC_TRACE("    [version]                        : MMC_VERSION_1_2\n");   break;
        default:              HAL_SDMMC_TRACE("    [version]                        : unkown version\n");   break;
    }
    HAL_SDMMC_TRACE("    [is SD card]                     : 0x%x\n", IS_SD(mmc));
    HAL_SDMMC_TRACE("    [high_capacity]                  : 0x%x\n", mmc->high_capacity);
    HAL_SDMMC_TRACE("    [bus_width]                      : 0x%x\n", mmc->bus_width);
    HAL_SDMMC_TRACE("    [clock]                          : %d\n", mmc->clock);
    HAL_SDMMC_TRACE("    [card_caps]                      : 0x%x\n", mmc->card_caps);
    HAL_SDMMC_TRACE("    [ocr]                            : 0x%x\n", mmc->ocr);
    HAL_SDMMC_TRACE("    [dsr]                            : 0x%x\n", mmc->dsr);
    HAL_SDMMC_TRACE("    [capacity_user/1024]             : %d(K)\n", (uint32_t)(mmc->capacity_user/1024));
    HAL_SDMMC_TRACE("    [capacity_user/1024/1024]        : %d(M)\n", (uint32_t)(mmc->capacity_user/1024/1024));
    HAL_SDMMC_TRACE("    [capacity_user/1024/1024/1024]   : %d(G)\n", (uint32_t)(mmc->capacity_user/1024/1024/1024));
    HAL_SDMMC_TRACE("    [read_bl_len]                    : %d\n", mmc->read_bl_len);
    HAL_SDMMC_TRACE("    [write_bl_len]                   : %d\n", mmc->write_bl_len);
    HAL_SDMMC_TRACE("--------------------------------------\n");
}
void hal_sdmmc_info(enum HAL_SDMMC_ID_T id, uint32_t *sector_count, uint32_t *sector_size)
{
    ...
    if (sector_size) {
        *sector_size = mmc->read_bl_len;
        HAL_SDMMC_TRACE("[sdmmc] sector_size %d\n", *sector_size);
    }
    if (sector_count) {
        if (mmc->read_bl_len != 0)
            ...
            *sector_count = 0;
        HAL_SDMMC_TRACE("[sdmmc] sector_count %d\n", *sector_count);
    }
}

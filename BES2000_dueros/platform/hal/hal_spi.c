...
#ifndef SPI_ROM_ONLY
static uint8_t spi_cs_map[HAL_SPI_ID_QTY];
...
int hal_spi_init_ctrl(const struct HAL_SPI_CFG_T *cfg, struct HAL_SPI_CTRL_T *ctrl)
{
...
#ifdef SPI_ROM_ONLY
    // Assume 26M crystal -- Never access global versatile data to ensure reentrance
    mod_freq = 26000000;
#else
    mod_freq = hal_cmu_get_crystal_freq();
#endif

    SPI_ASSERT(cfg->rate <= mod_freq / (MIN_CPSDVSR * (1 + MIN_SCR)), "SPI rate too large: %u", cfg->rate);
    SPI_ASSERT(cfg->rate >= mod_freq / (MAX_CPSDVSR * (1 + MAX_SCR)), "SPI rate too small: %u", cfg->rate);
    ...
    div = (mod_freq + cfg->rate - 1) / cfg->rate;
...
static int NOINLINE hal_spi_open_id(enum HAL_SPI_ID_T id, const struct HAL_SPI_CFG_T *cfg)
{
    int ret;
    ...
    ret = hal_spi_init_ctrl(cfg, &ctrl_regs);
    if (ret) {
        return ret;
    }
    ...
    if (0) {
        ...
        ctrl = &spi0_ctrl[cfg->cs];
#endif
#ifdef CHIP_HAS_SPILCD
    } else if (id == HAL_SPI_ID_SLCD) {
        ctrl = &spilcd_ctrl[cfg->cs];
#endif
    }
    if (ctrl) {
        memcpy(ctrl, &ctrl_regs, sizeof(ctrl_regs));
    }
    ...
    hal_spi_enable_id(id, &ctrl_regs, HAL_SPI_XFER_TYPE_SEND);
...
static void POSSIBLY_UNUSED hal_spi_close_id(enum HAL_SPI_ID_T id, uint32_t cs)
{
    bool cfg_clk = true;
...
int hal_spi_activate_cs_id(enum HAL_SPI_ID_T id, uint32_t cs)
{
    struct HAL_SPI_CTRL_T *ctrl = NULL;
...
int hal_spi_open(const struct HAL_SPI_CFG_T *cfg)
{
    ...
    ret = hal_spi_open_id(HAL_SPI_ID_0, cfg);
...
{
    hal_spi_close_id(HAL_SPI_ID_0, cs);
}
...
int hal_spilcd_open(const struct HAL_SPI_CFG_T *cfg)
{
    ...
    ret = hal_spi_open_id(HAL_SPI_ID_SLCD, cfg);
...
{
    hal_spi_close_id(HAL_SPI_ID_SLCD, cs);
}
...
{
    hal_spi_close_id(HAL_SPI_ID_USBPHY, HAL_SPI_CS_0);
}

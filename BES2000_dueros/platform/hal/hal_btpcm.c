...
int hal_btpcm_open(enum HAL_BTPCM_ID_T id)
{
    uint32_t reg_base;
...
int hal_btpcm_close(enum HAL_BTPCM_ID_T id)
{
    uint32_t reg_base;
    ASSERT(id < HAL_BTPCM_ID_NUM, invalid_id, id);
    reg_base = _btpcm_get_reg_base(id);
...
int hal_btpcm_send(enum HAL_BTPCM_ID_T id, uint8_t *value, uint32_t value_len)
{
    ...
    for (i = 0; i < value_len; i += 4) {
        while (!(btpcmip_r_int_status(reg_base) & BTPCMIP_INT_STATUS_TX_FIFO_EMPTY_MASK));

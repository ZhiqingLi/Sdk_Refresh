...
#endif
#define HAL_KEY_TRACE(s,...) //TRACE("[%u]" s, TICKS_TO_MS(hal_sys_timer_get()), ##__VA_ARGS__)
...
#define CFG_SW_KEY_REPEAT_THRESH_MS         200
#endif
...
void hal_key_debounce_timer_restart(void)
{
    uint32_t lock;
    bool set = false;
...
static inline void hal_adckey_enable_press_int(void)
{
    hal_adckey_set_irq(HAL_ADCKEY_IRQ_PRESSED);
}

static inline void hal_adckey_enable_release_int(void)
{
    hal_adckey_set_irq(HAL_ADCKEY_IRQ_RELEASED);
}

static inline void hal_adckey_enable_adc_int(void)
{
    hal_gpadc_open(HAL_GPADC_CHAN_ADCKEY, HAL_GPADC_ATP_NULL, NULL);
}

static inline void hal_adckey_disable_adc_int(void)
{
    hal_gpadc_close(HAL_GPADC_CHAN_ADCKEY);
}

static inline void hal_adckey_disable_allint(void)
{
    hal_gpadc_close(HAL_GPADC_CHAN_ADCKEY);
    hal_adckey_set_irq(HAL_ADCKEY_IRQ_NONE);
}

static inline void hal_adckey_reset(void)
{
...
static void hal_adckey_irqhandler(uint16_t read_val, HAL_GPADC_MV_T vol)
{
    ...
    if (read_val & ((1 << 11) | (1 << 12))) {
        HAL_KEY_TRACE("irq,adckey err 0x%04X", read_val);
    ...
    if(read_val & (1 << 7)) {
        code = hal_adckey_findkey(vol);
        HAL_KEY_TRACE("irq,adckey cur:0x%X pre:0x%X volt:%d", code, adc_key.code_debounce, vol);
...
static inline void hal_pwrkey_enable_riseedge_int(void)
{
    hal_pwrkey_set_irq(HAL_PWRKEY_IRQ_RISING_EDGE);
}

static inline void hal_pwrkey_enable_falledge_int(void)
{
    hal_pwrkey_set_irq(HAL_PWRKEY_IRQ_FALLING_EDGE);
}

static inline void hal_pwrkey_enable_bothedge_int(void)
{
...
static void hal_pwrkey_handle_irq_state(enum HAL_PWRKEY_IRQ_T state)
{
    ...
    if (state & HAL_PWRKEY_IRQ_FALLING_EDGE) {
        HAL_KEY_TRACE("pwr_key irq down");
#ifdef CHIP_BEST1000
        if (hal_get_chip_metal_id() < HAL_CHIP_METAL_ID_2) {
            pwr_key.debounce = true;
...
{
    enum HAL_PWRKEY_IRQ_T state = hal_pwrkey_get_irq_state();

    HAL_KEY_TRACE("%s: %08x", __func__, state);
...
static void hal_pwrkey_open(void)
{
    ...
    pmu_set_irq_handler(PMU_IRQ_TYPE_PWRKEY, hal_pwrkey_irqhandler);
#else
...
#ifdef CHIP_HAS_EXT_PMU
    pmu_set_irq_handler(PMU_IRQ_TYPE_PWRKEY, NULL);
#else
...
static void hal_gpiokey_irqhandler(enum HAL_GPIO_PIN_T pin)
{
    ...
    HAL_KEY_TRACE("gpio_key trig=%d pressed=%d", pin, pressed);
...
static inline void hal_gpiokey_enable_allint(void)
{
    ...
    for (i = 0; i < CFG_HW_GPIOKEY_NUM; i++){
        ...
        hal_gpiokey_enable_irq(cfg_hw_gpio_key_cfg[i].key_config.pin, polarity);
    }
}
...
static inline void hal_gpiokey_disable_allint(void)
{
    ...
    for (i = 0; i < CFG_HW_GPIOKEY_NUM; i++) {
        ...
        hal_gpiokey_disable_irq(cfg_hw_gpio_key_cfg[i].key_config.pin);
    }
}
...
static void hal_key_boot_handler(void *param)
{
    ...
    if (pwr_key.debounce || pwr_key.dither || pwr_key.pressed) {
        ...
        //HAL_KEY_TRACE("keyBoot: dbn=%d dither=%d pressed=%d/%d event=%d", pwr_key.debounce, pwr_key.dither, pwr_key.pressed, pressed, key_status.event);
        ...
        if (pwr_key.pressed) {
            if (key_status.event == HAL_KEY_EVENT_NONE) {
                if (time - key_status.time_updown >= KEY_INIT_DOWN_THRESHOLD) {
                    ...
                    key_detected_callback(HAL_KEY_CODE_PWR, key_status.event);
                }

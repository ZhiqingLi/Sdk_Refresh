	...
	while(list)
	{
		if (list->record.audio_id == id)
		{
			...
			*type = list->record.type;
			return (U8*) (gpAudioDataBase + list->record.offset);
		}
		list = list->next;
	}	
...
U8* aud_get_reouce_chinese(AUD_ID_ENUM id, UINT32* leng, UINT16* type)
{
	...
	*leng = 0, *type = 0;
	...
	while(list)
	{
		if (list->record.audio_id == id)
		{
			*leng = list->record.size;
			*type = list->record.type;
			return (U8*) (gpAudioDataBase_CN + list->record.offset);
		}
		list = list->next;
	}	
...
/********************************************************/
	...
	TRACE("%s, *pcrc: %x ,  crc: %x",__func__, *pcrc, crc);
	if (*pcrc != crc)
	    return;
	...
	while((UINT32)pRes < (UINT32)gpResourceBase +gReserver1 && pRes->MARK == AUDIO_TOOL_RES_MARK)
	{
		if (pRes->ID == RES_ENGLISH_ID)
		{
			...
			TRACE("%s,  english count: %d, leng: %d",__func__, pRes->count, pRes->total_size);
			...
			TRACE("%s,  english list: %x",__func__, list);
		...
		else if (pRes->ID == RES_CHINESE_ID)
		{
			...
			AUDIO_List * list;
			TRACE("%s, AUDIO_TOOL_RES_MARK: %d",__func__, pRes->count);
			...
			TRACE("%s, chinese count: %d, leng: %d",__func__, pRes->count, pRes->total_size);
		...
		TRACE("%s, %x,  pRes->total_size: %x",__func__,res_addr, pRes->total_size);
	...
	TRACE("%s, pRes add: %x",__func__, pRes);
...
void test_resource_load()
{
	...
	TRACE("====english_audio, leng = %d, type = %d, data[0] = %x, data[1]= %x, data[2]= %x, data[3]= %x", leng, type, data[0], data[1], data[2], data[3]);

	data = aud_get_reouce_chinese(AUD_ID_BT_CLEAR_SUCCESS, &leng, &type);

	TRACE("====chinese_audio, leng = %d, type = %d, data[0] = %x, data[1]= %x, data[2]= %x, data[3]= %x", leng,type, data[0], data[1], data[2], data[3]);

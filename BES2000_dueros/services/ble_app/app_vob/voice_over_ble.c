...
osThreadDef(ble_app_thread, osPriorityNormal, 512);
...
static int ble_resample_iter(uint8_t *buf, uint32_t len)
{
    ...
    {
        TRACE("Start encoded data caching again.");
...
uint32_t a2dp_audio_prepare_pcm_data(uint8_t *buf, uint32_t len)
{
	if (stream_a2dp_data_management.isCachingDone)
	{
...
#endif
        {
    		...
    		if (acquiredPCMDataLength < len)
    		{
    			TRACE("Need %d PCM data, but the queue encoded data can only provide %d bytes", 
...
int encoded_data_received(unsigned char *buf, unsigned int len)
{
	...
	if (CQ_ERR == ret)
	{
		TRACE("Encoded data cache is overflow.");
	}
...
static void vob_stop_voice_stream(void)
{
    ...
    {
       TRACE("Time-out happens when waiting the response of stopping voice stream from DST.");
    }
...
void vob_start_voice_stream_rsp_handler(BLE_CUSTOM_CMD_RET_STATUS_E retStatus, uint8_t* ptrParam, uint32_t paramLen)
{
	...
	{
		TRACE("voice stream is successfully started!"
	...
	{
		TRACE("starting voice stream failed!"
	}
}
...
void vob_connected_evt_handler(void)
{
	if (VOB_ROLE_SRC == vob_role)
	{
		app_datapath_server_update_conn_parameter(VOB_CONNECTION_INTERVAL_IN_MS, 
...
int ble_app_init(void)
{
	...
	if (ble_app_tid == NULL)  {
        TRACE("Failed to Create ble_app_thread\n");
		return 0;
	}
	return 0;
}

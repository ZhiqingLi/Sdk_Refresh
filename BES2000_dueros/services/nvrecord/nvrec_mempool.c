...
#include "nvrec_mempool_config.h"
#include "cmsis.h"
#else
...
{
    buffer_alloc_ctx *addr = NULL;

    addr = (buffer_alloc_ctx *)&heap;       
    return addr;
}

void * (*pool_malloc)( size_t ) = NULL;
void (*pool_free)( void * )     = NULL;

int platform_set_malloc_free( void * (*malloc_func)( size_t ),
                              void (*free_func)( void * ) )
{
    pool_malloc = malloc_func;
    pool_free = free_func;
    return( 0 );
}
...
static void buffer_alloc_free( void *ptr )
{
    ...
    {
#if defined(POLARSSL_MEMORY_DEBUG)
        polarssl_fprintf( stderr, "FATAL: pool_free() outside of managed "
                                  "space\n" );
#endif
        exit( 1 );
    }
    ...
    {
#if defined(POLARSSL_MEMORY_DEBUG)
        polarssl_fprintf( stderr, "FATAL: pool_free() on unallocated "
                                  "data\n" );
#endif
        exit( 1 );
    }
...
static void *buffer_alloc_malloc_irqlock(size_t len)
{
    void *buf;
...
static void buffer_alloc_free_irqlock(void *ptr)
{
    uint32_t lock = int_lock();
    buffer_alloc_free(ptr);
    int_unlock(lock);   
...
{
    platform_set_malloc_free(buffer_alloc_malloc_irqlock,buffer_alloc_free_irqlock);
}

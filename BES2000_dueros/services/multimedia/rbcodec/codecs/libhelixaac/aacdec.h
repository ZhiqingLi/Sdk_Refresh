/* ***** BEGIN LICENSE BLOCK *****  
 * Source last modified: $Id: aacdec.h,v 1.6 2005/07/05 21:08:13 ehyche Exp $ 
 *   
...
#define AAC_MAINBUF_SIZE	        (1024)//(768 * AAC_MAX_NCHANS)
...
enum {
	...
	ERR_AAC_INVALID_LOAS_HEADER			  = -23,
...
/* public C API */
int GetAACinfoSize(void);
void SetAACInfoBase(void* info_base);
HAACDecoder AACInitDecoder(void);
void AACSetLOASFormat(HAACDecoder hAACDecoder);
...
#ifdef _WIN32
#define scnprintf(buf, len, fmt, ...)   _snprintf_s(buf, len, 1, fmt, __VA_ARGS__)
#define INLINE __inline
#define true                            1
#define false                           0
#else
extern int hal_trace_printf(const char *fmt, ...);
#define scnprintf(buf, len, fmt, ...)   snprintf(buf, len, fmt, __VA_ARGS__)

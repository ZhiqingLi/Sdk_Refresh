...
typedef struct {
  unsigned int frameLengthType;
  unsigned int frameLength;
  unsigned int latmBufferFullness;
  unsigned int streamID;
  unsigned int frameLengthInBits;
  unsigned int MuxSlotLengthBytes;
  unsigned int MuxSlotLengthCoded;
} LATM_LAYER_INFO;

typedef struct _LOASHeader {
	/* fixed */
	...
	short numPrograms;
	short numLayer;
        unsigned char useSameConfig;
	int sync_extension;
	int asclen;
        unsigned char * buf_start;

        unsigned char otherDataPresent;
        unsigned char otherDataLenBits;

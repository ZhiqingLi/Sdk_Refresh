									  ...
									  ke_task_id_t const src_id)
{
	...
	datapathps_env->isNotificationEnabled = false;
...
{	
    ...
    if (datapathps_env != NULL)
    {
        ...
        if (param->handle == (datapathps_env->shdl + DATAPATHPS_IDX_TX_NTF_CFG))
        {
			...
			{
				datapathps_env->isNotificationEnabled = true;
			}
            else
            {
            ...
            if (status == GAP_ERR_NO_ERROR)
            {
                ...
                ind->isNotificationEnabled = datapathps_env->isNotificationEnabled;
...
{	
    ...
    {
		cfm = KE_MSG_ALLOC_DYN(GATTC_READ_CFM, src_id, dest_id, gattc_read_cfm, sizeof(custom_tx_desc));
	...
	{
		cfm = KE_MSG_ALLOC_DYN(GATTC_READ_CFM, src_id, dest_id, gattc_read_cfm, sizeof(custom_rx_desc));
	...
	else if (param->handle == (datapathps_env->shdl + DATAPATHPS_IDX_TX_NTF_CFG))
	{
		cfm = KE_MSG_ALLOC_DYN(GATTC_READ_CFM, src_id, dest_id, gattc_read_cfm, sizeof(custom_rx_desc));
		uint16_t notify_ccc;
		if (datapathps_env->isNotificationEnabled)
		{
			notify_ccc = 1;
		}
		else
                                      ...
                                      ke_task_id_t const src_id)
{
	...
	prf_gatt_write(&(datapathps_env->prf_env), KE_IDX_GET(dest_id), datapathps_env->shdl + DATAPATHPS_IDX_TX_NTF_CFG,

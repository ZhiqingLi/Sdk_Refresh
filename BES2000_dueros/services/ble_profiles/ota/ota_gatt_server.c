 ...
 ****************************************************************************************
 */
#define ota_service_uuid_128_content  			{0x66, 0x66, 0x66, 0x66, 0x66, 0x66, 0x66, 0x66, 0x66, 0x66, 0x66, 0x66, 0x66, 0x66, 0x66, 0x66 }
...
static const uint8_t OTA_SERVICE_UUID_128[ATT_UUID_128_LEN]	= ota_service_uuid_128_content;  
...
const struct attm_desc_128 ota_att_db[OTA_IDX_NB] =
{
    ...
    [OTA_IDX_SVC] 	=   {ATT_DECL_PRIMARY_SERVICE_UUID, PERM(RD, ENABLE), 0, 0},
	// OTA Characteristic Declaration
	[OTA_IDX_CHAR]    =  {ATT_DECL_CHARACTERISTIC_UUID, PERM(RD, ENABLE), 0, 0},
	// OTA service
    [OTA_IDX_VAL]     =  {ota_val_char_val_uuid_128_content, PERM(WRITE_COMMAND, ENABLE) | PERM(WRITE_REQ, ENABLE) | PERM(NTF, ENABLE), PERM(RI, ENABLE) | PERM_VAL(UUID_LEN, PERM_UUID_128), OTA_MAX_LEN},
	// OTA Characteristic
	[OTA_IDX_NTF_CFG]	=   {ATT_DESC_CLIENT_CHAR_CFG_UUID, PERM(RD, ENABLE) | PERM(WRITE_REQ, ENABLE), 0, 0},
...
{
	struct ota_env_tag* ota_env = (struct ota_env_tag*) env->env;
	struct prf_svc ota_otac = {ota_env->shdl, ota_env->shdl + OTA_IDX_NB};
	prf_register_atthdl2gatt(env->env, conidx, &ota_otac);
}
									  ...
									  ke_task_id_t const src_id)
{
	...
	ota_env->isNotificationEnabled = false;
...
{	
    ...
    if (ota_env != NULL)
    {
		if (param->handle == (ota_env->shdl + OTA_IDX_NTF_CFG))
		{
			...
			{
				ota_env->isNotificationEnabled = true;
			}
            else
            {
            ...
            if (status == GAP_ERR_NO_ERROR)
            {
                ...
                ind->isNotificationEnabled = ota_env->isNotificationEnabled;
...
{	
    ...
    if (param->handle == (ota_env->shdl + OTA_IDX_NTF_CFG))
	{
		...
		if (ota_env->isNotificationEnabled)
		{
			notify_ccc = 1;
		}
		else
...
static void send_notifiction(uint8_t conidx, const uint8_t* ptrData, uint32_t length)
{
	...
	TRACE("Send ota notificationto handle offset %d:", handle_offset);

...
static BtHandler a2dp_source_handler;
...
static void bt_a2dp_source_call_back(const BtEvent* event)
{
    ...
    TRACE("SRC app_bt_golbal_handle evt = %d",event->eType);
...
{
    A2DP_SuspendStream(&app_bt_device.a2dp_stream[BT_DEVICE_ID_1]);
}
...
static SbcStreamInfo StreamInfo = {0};

static SbcEncoder sbc_encoder;
...
//    sbcPacket->frameSize = len/frame_num;
    ...
    if(status == BT_STATUS_PENDING)
        ...
        TRACE("SEND ERROR CHECK IT!,status:%d",status);
}
#endif

//#define BT_A2DP_SOURCE_LINEIN_BUFF_SIZE    	(512*5*2*2)
#define BT_A2DP_SOURCE_LINEIN_BUFF_SIZE    	(10*1024)
...
osThreadDef(send_thread, osPriorityHigh, 1024*2);

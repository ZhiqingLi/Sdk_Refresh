...
int app_factorymode_languageswitch_proc(void)
{
	...
	APP_FACTORY_TRACE("languages old:%d new:%d",lan ,new_lan);
...
void app_factorymode_key_init(void)
{
    const APP_KEY_HANDLE  app_factorymode_handle_cfg[] = {
        ...
        {{APP_KEY_CODE_PWR,APP_KEY_EVENT_CLICK},"bt i2c key",app_factorymode_i2c_switch, NULL},
#else
...
static void app_factorymode_audioloopswitch(APP_KEY_STATUS *status, void *param)
{
    ...
    if (onaudioloop)
        app_audio_sendrequest((uint8_t)APP_FACTORYMODE_AUDIO_LOOP, (uint8_t)APP_BT_SETTING_OPEN, 0);
    else
        app_audio_sendrequest((uint8_t)APP_FACTORYMODE_AUDIO_LOOP, (uint8_t)APP_BT_SETTING_CLOSE, 0);
}
...
static int app_factorymode_send_code (uint8_t progress)
{
    APP_MESSAGE_BLOCK msg;
...
static int app_factorymode_msg_send_code(uint8_t code)
{
    int ret;
    ...
    send_msg.hdr.seq = send_seq++;
    send_msg.hdr.len = 2;
    send_msg.data[0] = 0xff;
    send_msg.data[1] = code;
    send_msg.data[2] = ~app_factorymode_msg_check_sum((unsigned char *)&send_msg, MSG_TOTAL_LEN(&send_msg) - 1);
...
static int app_factorymode_process(APP_MESSAGE_BODY *msg_body)
{
    ...
    if (msg_body->message_id == 3){
        app_factorymode_msg_send_code(msg_body->message_Param0);
    }
    return 0;
}
...
int app_factorymode_init(uint32_t factorymode)
{
	...
	struct nvrecord_env_t *nvrecord_env;
    APP_FACTORY_TRACE("app_factorymode_init mode:%x\n", factorymode);

...
//    TRACE("%s\n len:%d", __func__, len);
	...
	if(pcm_res != FR_OK)
	{
		TRACE("[%s]:error-->res = %d", __func__, pcm_res);
	}
    return 0;
}
...
void  ad_tester(bool run)
{
    ...
    if (run){
        ...
        if (pcm_res) {
        	TRACE("[%s]:Cannot creat test2.bin...%d",__func__, pcm_res);
            return;
        }
...
//        ASSERT(pcm_res == FR_OK,"[%s]:Write dump.bin failed, res = %d", __func__, pcm_res);
    ...
    if(res == FR_OK)
    {
        TRACE("[%s] SUCESS: opt = %d, res = %d",__func__, opt, res);
    }
    else
    {
        TRACE("[%s] ERROR: opt = %d, res = %d",__func__, opt, res);
    }
}

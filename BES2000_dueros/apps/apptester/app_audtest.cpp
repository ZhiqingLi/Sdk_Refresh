...
uint32_t pcm_cvsd_data_capture(uint8_t *buf, uint32_t len)
{
    ...
    etime = hal_sys_timer_get();
    TRACE("%s exit irqDur:%d fsSpend:%d, add:%d", __func__, TICKS_TO_MS(stime - preIrqTime), TICKS_TO_MS(etime - stime), len);
    preIrqTime = stime;
    return 0;
}
...
void app_anc_usb_init(void)
{
    ...
    usb_audio_app_init(app_test_playback_buff, USB_AUDIO_PLAYBACK_BUFF_SIZE,
        app_test_capture_buff, USB_AUDIO_CAPTURE_BUFF_SIZE,
        app_test_recv_buff, USB_AUDIO_RECV_BUFF_SIZE,
        app_test_send_buff, USB_AUDIO_SEND_BUFF_SIZE,
        NULL, 0,
        NULL, 0);

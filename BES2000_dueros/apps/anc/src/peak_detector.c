...
void peak_detector_init(void)
{
    ...
    // TRACE("[%s] pkd_alphaR = %f, pkd_alphaA = %f", __func__, (double)pkd_alphaR, (double)pkd_alphaA);
}
...
static void peak_detector_run_16bits(int16_t *buf, uint32_t len, float vol_multiple)
{
    ...
    //  test_num = 0;
    //  TRACE("START>>>");
    //  TRACE("vol_level = %d, pkd_vol_multiple = %f", vol_level, pkd_vol_multiple[vol_level]);
    //  TRACE("buf = %d, pkd_alphaR = %f, pkd_alphaA = %f", buf[len-1], pkd_alphaR, pkd_alphaA);
    //  TRACE("pkd_factor1 = %f, pkd_factor2 = %f, normal_rate = %f, tgt_rate = %f", pkd_factor1, pkd_factor2, normal_rate, tgt_rate);
    //  TRACE("END<<<");
    //  // TRACE("[%s] buf = %d, pkd_alphaR = %f, pkd_alphaA = %f, pkd_factor1 = %f, pkd_factor2 = %f, normal_rate = %f", __func__, buf[len-1], pkd_alphaR, pkd_alphaA, pkd_factor1, pkd_factor2, (1.0 - pkd_factor2/32768));
    // }
    ...
    TRACE("Max = %10d, Min = %10d",sample_max, sample_min);
#endif
}

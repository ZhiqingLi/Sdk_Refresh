...
osThreadDef(rb_ctl_thread, osPriorityHigh, 1024 * 4);
...
void rb_ctl_set_vol(uint32_t vol)
{
    ...
    _LOG_DBG("%s set vol as :%d , ",__func__,vol);
        ...
        case RB_MODULE_EVT_PLAY:
            if(rb_ctl_context.status != RB_CTL_IDLE) {
                if(rb_ctl_context.status == RB_CTL_SUSPEND)
                    ...
                    _LOG_DBG("%s  st  %d not in idle \n",__func__,rb_ctl_context.status);
                break;
            }
			_LOG_DBG(" %s start %d/%d ",__func__, rb_ctl_context.curr_song_idx ,sd_playlist.total_songs);
            if(sd_playlist.total_songs > 0 ) {
                ...
                if(it == NULL) {
                    _LOG_DBG(" %s get item fail idx %d",__func__,rb_ctl_context.curr_song_idx);
                }

                _LOG_DBG("%s  start songidx %d \n",__func__,rb_ctl_context.curr_song_idx);
...
{
    _LOG_DBG(" %s array_subidx %d, ",__FUNCTION__,array_subidx);
...
{
    _LOG_DBG(" %s inc %d, ",__FUNCTION__,inc);
...
{
    _LOG_DBG(" %s onoff %d, get status:%d",__FUNCTION__,onoff, rb_ctl_get_status());
...
{
    _LOG_DBG(" %s get status:%d",__func__, rb_ctl_get_status());
...
static int rb_ctl_mailbox_init(void)
{
    ...
    if (rb_ctl_mailbox == NULL)  {
        TRACE("Failed to Create rb_ctl_mailbox\n");
        return -1;
    }
    return 0;
}
...
static void rb_ctl_thread(void const *argument)
{
    osEvent evt;
#if 1
    app_sysfreq_req(APP_SYSFREQ_USER_APP_2, APP_SYSFREQ_208M);
    ...
    while(1) {
        ...
        if (!rb_ctl_mailbox_get(&msg_p)) {
            app_sysfreq_req(APP_SYSFREQ_USER_APP_2, APP_SYSFREQ_104M);
...
int rb_ctl_init(void)
{
	...
	if (rb_ctl_tid == NULL)  {
		TRACE("Failed to Create rb_ctl_thread\n");
		return 0;
	}
	_LOG_DBG("Leave %s \n",__func__);
	return 0;

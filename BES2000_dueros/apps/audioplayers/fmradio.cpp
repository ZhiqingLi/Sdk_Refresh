...
static void fm_handler(uint8_t chan, uint32_t remains, uint32_t error, struct HAL_DMA_DESC_T *lli)
{
...
#ifdef FM_DEBUG
    {
        ...
        TRACE("[fm_handler]       diff=%d add:%d remain:%d input", diff_ticks, (FM_SAMPLE_NUM/9)<<1, app_audio_pcmbuff_length());
    }
#endif
}
...
uint32_t fm_pcm_more_data(uint8_t *buf, uint32_t len)
{
...
#ifdef FM_DEBUG
    {
        ...
        TRACE("[fm_pcm_more_data] diff=%d get:%d remain:%d output isr:0x%08x cnt:%d", diff_ticks, len/2, app_audio_pcmbuff_length(), FM_DIGITAL_REG(0x40160020), FM_DIGITAL_REG(0x40160028));
    }
#endif
    return 0;
}
...
int fm_radio_player(bool on)
{
    ...
    TRACE("fm_radio_player work:%d op:%d", isRun, on);
    if (isRun==on)
        return 0;

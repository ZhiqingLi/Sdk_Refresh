...
archive_target :=
ifeq ($(GEN_LIB),1)
archive_target := $(addprefix $(srctree)/$(src)/lib/,$(notdir $(archive-src-y)))
endif

__build: $(builtin-target) $(lib-target) $(buildextra-y) $(buildsubdir-y) $(lst_target) $(archive_target) $(always)
	@:
...
define archive-src-rule
$(1): $(addprefix $(obj)/,$(patsubst %/,%/$(notdir $(builtin-target)),$($(notdir $(1:.a=-y)))))
endef

$(foreach m, $(archive_target), $(eval $(call archive-src-rule,$(m))))

$(archive_target): FORCE
	$(call cmd,gen_lib_file)

targets += $(archive_target)
...
cmd_link_multi-y = $(LD) -Wl,$(subst $(space),$(comma),$(strip $(ld_flags) -r)) -nostdlib -nostartfiles $(LINK_CFLAGS) -o $@ $(link_multi_deps)

...
#define printf(...)                     hal_trace_printf(__VA_ARGS__)
...
extern void print_hex_dump(const char *prefix_str,
                           int prefix_type, unsigned int rowsize, unsigned int groupsize,
...
void hex_dump_to_buffer(const void *buf, size_t len, unsigned int rowsize,
                        unsigned int groupsize, char *linebuf, size_t linebuflen,
                        bool ascii)
{
    const u8 *ptr = buf;
    u8 ch;
    ...
    switch (groupsize) {
        case 8: {
            const u64 *ptr8 = buf;
            unsigned int ngroups = len / groupsize;

...
static void hwtimer_handler(uint32_t elapsed)
{
    ...
    if (hwtimer_list.active == NULL) {
        err_irq_active_null++;
        TRACE("HWTIMER irq when active is null: might be deleted? %u", err_irq_active_null);
    ...
    if (hwtimer_list.active->time > elapsed + HAL_TIMER_LOAD_DELTA) {
        err_irq_early++;
        TRACE("HWTIMER irq occurred early: old active timer might be deleted? %u", err_irq_early);
...
static void timer_stop_test(int id)
{
    ...
    TRACE("[%u] Stop %d / %d", hal_sys_timer_get(), id, ret);
    hwtimer_dump();
}
...
static void timer_callback(void *param)
{
    ...
    TRACE("[%u] TIMER-CALLBACK: %d", hal_sys_timer_get(), id);
...
void hwtimer_test(void)
{
    ...
    for (i = 0; i < HWTIMER_TEST_NUM; i++) {
        ...
        TRACE("[%u] START-TIMER: %u / %p / %d", hal_sys_timer_get(), i, test_id[i], ret);
    }

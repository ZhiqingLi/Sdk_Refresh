	...
	if((FsContext->gFsInfo.FolderSum == 1) && (UpgradeFileFound != 1)) //只搜索根目录是否有升级文件，注释这句则全盘搜索
	{
		if(memcmp(FsContext->gCurrentEntry->ExtName, "MVA", 3) == 0)
		{
			...
			if( (MVAbuf[0] == MVA[0]) && (MVAbuf[1] == MVA[1]) && (MVAbuf[2] == MVA[2]) && (MVAbuf[3] == MVA[3]))
			{
				...
				if(gSys.UpgradeDoneDeleteFile == TRUE)
				{
					FsUnLock(FsContext);
					FileOpen( &MVA_File, FileName, FA_READ);
					if(FileDelete(&MVA_File))
					{
						DBG("%s Has Deleted Done!!!\n",FileName);
					}
					FileClose(&MVA_File);
					UpgradeFileFound = 0;
				}
...
uint8_t FileGetType(FS_CONTEXT* FsContext)
{
	...
	else if((ExtFileName[0] == 'A') && (ExtFileName[1] == 'B') && (ExtFileName[2] == 'C'))
    {
        return FILE_TYPE_MP3;
    }
#endif
...
#if (defined(FUNC_SPI_UPDATE_EN) && defined(FUNC_UPDATE_DONE_DELETE_FILE))
#ifdef FUNC_BREAKPOINT_EN	
		//文件统计完成。判断下次开机是否需要删除升级文件
		//如果是，将标志位保存到断点记忆。
		{
			BP_SYS_INFO *pSysInfo;
			pSysInfo = (BP_SYS_INFO *)BP_GetInfo(BP_SYS_INFO_TYPE);
			if(UpgradeFileFound == 1)
			{
				BP_SET_ELEMENT(pSysInfo->UpgradeDoneDeleteFile, TRUE);
			}
			else
			{
				BP_SET_ELEMENT(pSysInfo->UpgradeDoneDeleteFile, FALSE);
			}
...
#endif
			gSys.UpgradeDoneDeleteFile = pSysInfo->UpgradeDoneDeleteFile;
			APP_DBG("Save to BpInfo UpgradeDoneDeleteFile:%d\n", gSys.UpgradeDoneDeleteFile);
		}
#endif
#endif
		//end = OSSysTickGet();

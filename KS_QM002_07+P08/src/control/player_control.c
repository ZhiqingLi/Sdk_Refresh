...
//底层解码时回调，实现用户自定义加密文件播放
void FileReadCallBack(void* buffer, uint32_t size)
{
//	APP_DBG("FileReadCallBack = %d;\n", size);
}
#endif
...
static bool PlayerParseLrc(void)
{
	...
	if((gPlayContrl->LrcRow.MaxLrcLen = (int16_t)GetLrcTextLength(SeekTime)) >= 0)
	{
		...
		if(gPlayContrl->LrcRow.StartTime + gPlayContrl->LrcRow.Duration == LrcStartTime)//LrcStartTime =
		{
			return FALSE;
		}
		...
		LrcStartTime = gPlayContrl->LrcRow.StartTime + gPlayContrl->LrcRow.Duration;
		return TRUE;
	}
...
bool AudioPlayerInitialize(int32_t FileIndex, uint32_t FolderIndex, bool IsNeedInitFs)
{
	...
	if(gPlayContrl->CurPlayMode >= PLAY_MODE_SUM)
	{
		gPlayContrl->CurPlayMode = PLAY_MODE_REPEAT_ALL;
	}
	...
	if(gPlayContrl->LrcFlag > 1)
	{
		gPlayContrl->LrcFlag = TRUE;
	}
#endif
	gSys.Eq = BP_GET_ELEMENT(pBpSysInfo->Eq);
	if(gSys.Eq >= EQ_STYLE_CNT)
	{
		gSys.Eq = EQ_STYLE_CNT - 1;
	}
...
void AudioPlayerSwitchMode(void)
{
	...
	if(gPlayContrl->CurPlayMode == PLAY_MODE_REPEAT_ONE)
	{
		gPlayContrl->CurPlayMode = PLAY_MODE_REPEAT_ALL;
		SoundRemind(SOUND_REP_ALL);
	}
	else
	{
		gPlayContrl->CurPlayMode = PLAY_MODE_REPEAT_ONE;
		SoundRemind(SOUND_REP_1);
	}
...
{		
	...
	{			
		...
		case MSG_BT_PAIR:
#ifdef TEST_CLEAR_BT_PAIRED_INFO
			{
		...
		case MSG_NUM_0:
		case MSG_NUM_1:
			MixerSoundRemind(SOUND_KS_KNOCK_SOUND_RING1);
			return TRUE;
		
		case MSG_NUM_2:
			MixerSoundRemind(SOUND_KS_KNOCK_SOUND_RING2);
			return TRUE;
		
		case MSG_NUM_3:
			MixerSoundRemind(SOUND_KS_KNOCK_SOUND_RING3);
			return TRUE;
		
		case MSG_NUM_4:
			MixerSoundRemind(SOUND_KS_KNOCK_SOUND_RING4);
			return TRUE;
		
		case MSG_NUM_5:
			MixerSoundRemind(SOUND_KS_KNOCK_SOUND_RING5);
			return TRUE;
		
		case MSG_NUM_6:
			MixerSoundRemind(SOUND_KS_KNOCK_SOUND_RING6);
			return TRUE;
		
		case MSG_NUM_7:
			MixerSoundRemind(SOUND_KS_KNOCK_SOUND_RING7);
			return TRUE;
		
		case MSG_NUM_8:
			MixerSoundRemind(SOUND_KS_KNOCK_SOUND_RING8);
			return TRUE;
		
		case MSG_NUM_9:
			MixerSoundRemind(SOUND_KS_KNOCK_SOUND_RING9);
			return TRUE;
...
#define  PLAY_LED_BLINK_CB_TIME        150
//播放状态下LED状态处理,
void PlayerStateLedCB(void* Param)
{
...
#endif
	{
		if((gSys.CurModuleID == MODULE_ID_BLUETOOTH) && (GetNumberOfConnectedProfile() == FALSE))
		{
			if(LedBlinkCnt >= 2)
			{
				BlueLedOn();
...
{
	uint16_t Msg  = 0;
	SW_TIMER	PlayLedTimer;			// 播放状态LED状态定时请
	...
	InitTimer(&PlayLedTimer, PLAY_LED_BLINK_CB_TIME, PlayerStateLedCB);
	StartTimer(&PlayLedTimer);

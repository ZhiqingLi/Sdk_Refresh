...
void SysVarInit(void)
{
	...
	gSys.UpgradeDoneDeleteFile = FALSE;    
#endif
#ifdef USE_POWERKEY_SOFT_PUSH_BUTTON
	//系统上电标志，用于开关机控制及关机提示音播放
...
#ifdef FUNC_BREAKPOINT_EN
{
	...
	if(gSys.HfVolume > MAX_BTHF_VOLUME)
	{
		gSys.HfVolume = MAX_BTHF_VOLUME;
	}    
#endif

#if 0//def FUNC_SOUND_REMIND
...
#ifdef FUNC_UPDATE_DONE_DELETE_FILE
	gSys.UpgradeDoneDeleteFile = BP_GET_ELEMENT(pBpSysInfo->UpgradeDoneDeleteFile);    
	APP_DBG("gSys.UpgradeDoneDeleteFile = %d;\n", gSys.UpgradeDoneDeleteFile);
	if(gSys.UpgradeDoneDeleteFile)
	{

...
#define 	POWER_KEY_JITTER_TIME		50			//消抖时间，该时间和软开关开关机硬件时间有关
#define 	POWER_KEY_CP_TIME			500			
...
bool 			PowerKeyJitterFlag;
										
static const uint16_t PowerKeyEvent[2] = {MSG_NONE/*按键开始*/, MSG_PLAY_PAUSE/*短按松开*/};	
...
{
	//防止开机长按电源键产生短按消息
	if(PowerkeyGetOnkeyReg() && PowerKeyJitterFlag)
	{
		return MSG_NONE;
	}
	else if(gSys.CurModuleID < MODULE_ID_IDLE )
	{
		PowerKeyJitterFlag = FALSE;
	}

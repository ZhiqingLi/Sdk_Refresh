 ...
 * @param  nlp Non-linear Processing on/off. On:1, Off:0
 * @param  ns_level Noise suppression level (0~5). 0: off, 1: minimum suppression, ..., 5: maximum suppression. In order to use noise suppression, non-linear processing (NLP) must be set ON.
 * @return 0 (always success)
 */
int32_t blue_aec_init(BlueAECContext* aecct, int32_t nlp, int32_t ns_level);
